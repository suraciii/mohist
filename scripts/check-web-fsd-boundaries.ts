...
const layerRank = new Map<string, number>(layers.map((layer, index) => [layer, index]))
...
const knownFiles = new Set<string>(allSourceFiles)
...
const moduleGraph = new Map<string, Set<string>>(sourceFiles.map((filePath) => [filePath, new Set<string>()]))
...
const traversalIndexes = new Map<string, number>()
