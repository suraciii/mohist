...
test('parseArgs: focused with both arguments resolves the request', () => {
  const args = parseArgs([
    'focused',
    'packages/server/tests/Mohist.Server.Tests/Mohist.Server.Tests.csproj',
    'Mohist.Server.Tests.Skills.SkillsContentTests',
  ])
  assert.equal(args.mode, 'focused')
  assert.equal(args.focused?.csproj, 'packages/server/tests/Mohist.Server.Tests/Mohist.Server.Tests.csproj')
  assert.equal(args.focused?.className, 'Mohist.Server.Tests.Skills.SkillsContentTests')
})
...
test('commandFor explicitly selects the execution-ledger reporter without environmental auto-reporters', () => {
  const track: TrackConfig = {
    id: 'cli',
    kind: 'dotnet-apphost',
    csproj: 'packages/server/tests/Mohist.Server.Tests/Mohist.Server.Tests.csproj',
...
test('parseArgs: focused with only the csproj leaves the request unresolved', () => {
  const args = parseArgs(['focused', 'packages/server/tests/Mohist.Server.Tests/Mohist.Server.Tests.csproj'])
  assert.equal(args.mode, 'focused')
  assert.equal(args.focused, undefined)
})
...
test('main: partially provided focused arguments (csproj only) fail explicitly', async () => {
  const stderr = captureStderr()
  try {
    const code = await main(['focused', 'packages/server/tests/Mohist.Server.Tests/Mohist.Server.Tests.csproj'])
    assert.equal(code, 2)
    assert.match(stderr.calls(), /usage: guard focused <csproj> <ClassName\.FQN>/)
  } finally {
    stderr.restore()
...
test('main: unreadable csproj fails explicitly with exit 2 instead of an unhandled exception', async () => {
  const stderr = captureStderr()
  try {
    const code = await main(['focused', 'no/such/project/Mohist.Server.Tests.csproj', 'X.Y.Z'])
    assert.equal(code, 2)
