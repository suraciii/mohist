...
function applyDurationMeasurementPhase(
  ...
  return planned.map((plan) => {
    const measurementIndex =
      plan.policyTrack === undefined ? -1 : durationMeasurementTracks.indexOf(plan.policyTrack.id)
...
function startLane(
  ...
  const runStage = async (
    ...
    currentChild = child
    const stageResult = await runWithDeadline({
      start: () => child.done,
      kill: async () => {
    ...
    if (stageResult.status === 'passed') {
      stdout = readFileSync(stageEvidence.stdoutPath, 'utf8')
    }
    return { ...stageResult, stdout }
  }
    ...
    try {
      ...
      if (executionTrack && plan.partition !== undefined) {
        ...
        ;({ command, args } = specPartitionCommand([
          ...
          manifestDir,
          plan.reportPath!,
          ...(executionTrack.partitionMaxThreads === undefined ? [] : [String(executionTrack.partitionMaxThreads)]),
        ]))
    ...
    },
    {
      ...
      readDiscovery: async () => {
        ...
        try {
          ...
          return readFileSync(evidence.stdoutPath, 'utf8')
        } finally {
          timer.cancel()
        }
      },
