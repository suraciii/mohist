...
function usage() {
  return [
    'Usage:',
    '  node scripts/compare-vitest-results.mjs --before <report.json> --after <report.json> [--manifest <changes.json>]',
...
function parseArguments(arguments_) {
  const options = { before: undefined, after: undefined, manifest: undefined, selfTest: false, help: false }

  for (let index = 0; index < arguments_.length; index += 1) {
    const argument = arguments_[index]
    if (argument === '--self-test') {
      if (options.selfTest) throw new Error('Option --self-test may only be provided once.')
  ...
  if (options.selfTest) {
    if (options.before !== undefined || options.after !== undefined || options.manifest !== undefined) {
      throw new Error('--self-test does not accept report or manifest paths.')
    }
    return options
  }
...
function makeReport(assertions, testResultStatus = 'passed') {
  return {
    testResults: [{
...
function expectComparisonFailure(name, action, messagePart) {
  try {
    action()
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error)
    assert.ok(message.includes(messagePart), `${name} failed for an unexpected reason: ${message}`)
    return
  }
...
function runSelfTest() {
  const baseline = makeReport(['alpha keeps behavior', 'beta keeps behavior'])
  const equal = compareVitestResults(baseline, makeReport(['alpha keeps behavior', 'beta keeps behavior']))
  assert.equal(equal.unchangedAssertions, 2)

  const addition = compareVitestResults(baseline, makeReport([
    'alpha keeps behavior',
    'beta keeps behavior',
    'gamma is added',
  ]))
  assert.equal(addition.additions, 1)

  expectComparisonFailure(
    'duplicate identity loss',
    () => compareVitestResults(makeReport(['duplicate', 'duplicate']), makeReport(['duplicate'])),
    'Missing baseline test identities',
  )

  expectComparisonFailure(
    'unlisted rename',
    () => compareVitestResults(makeReport(['old behavior']), makeReport(['new behavior'])),
    'Missing baseline test identities',
  )
  const renamed = compareVitestResults(
    makeReport(['old behavior']),
    makeReport(['new behavior']),
    { renames: [{ from: 'old behavior', to: 'new behavior' }], removals: [] },
  )
  assert.equal(renamed.renames, 1)

  expectComparisonFailure(
    'unlisted removal',
    () => compareVitestResults(makeReport(['duplicate behavior']), { testResults: [] }),
    'Missing baseline test identities',
  )
  const removed = compareVitestResults(
    makeReport(['duplicate behavior']),
    { testResults: [] },
    { renames: [], removals: [{ fullName: 'duplicate behavior', reason: 'duplicate coverage' }] },
  )
  assert.equal(removed.removals, 1)

  expectComparisonFailure(
    'failed test',
    () => compareVitestResults(
      makeReport([{ fullName: 'failing behavior', status: 'failed' }], 'failed'),
      makeReport([{ fullName: 'failing behavior', status: 'failed' }], 'failed'),
    ),
  ...
  expectComparisonFailure(
    'skipped test',
    () => compareVitestResults(
      makeReport([{ fullName: 'skipped behavior', status: 'skipped' }], 'skipped'),
      makeReport([{ fullName: 'skipped behavior', status: 'skipped' }], 'skipped'),
    ),
    'After report contains non-passed results',
  )
  expectComparisonFailure(
    'failed report',
    () => compareVitestResults(
      { ...makeReport(['interrupted behavior']), success: true },
      { ...makeReport(['interrupted behavior']), success: false },
    ),
    'After report contains non-passed results',
  )
  expectComparisonFailure(
    'missing test',
    () => compareVitestResults(makeReport(['required behavior']), { testResults: [] }),
    'Missing baseline test identities',
  )
}
...
function main() {
  ...
  if (options.selfTest) {
    runSelfTest()
    console.log('Vitest result comparison self-test passed.')
    return
  }
