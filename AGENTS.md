...
默认 CI workflow 必须在 5 分钟内完成并通过，job、step 和命令级 `timeout` 均不得超过 5 分钟；不得通过增加 `timeout`、`retry`、`skip`、弱化断言或加入可选慢 gate 掩盖超时。`AgentJob` focused 测试若仍有需要，只能走本地或专门验证，不得进入默认 CI gate。
