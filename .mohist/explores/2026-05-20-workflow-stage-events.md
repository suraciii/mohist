...
## 下一步
- 在 domain type 中为 `TaskResultInput` / `StageTaskResult` 增加 `events?: string[]`。
- 修改 WorkflowRun：task completion invalidation 只根据 `result.events` 应用，不再因 `definition.emits` 自动触发。
- 保留 runtime rebase 的内置 shaChanged policy，直到它也能通过 task result events 表达。
- 更新默认 workflow：Plan 增加 `plan.artifacts.changed`，`fix-plan-review` emit 后重置 `self-review` 与 `self-review-passed`。
- 更新测试，覆盖“声明但未实际抛出 event 不触发 reset”和“实际抛出 event 才触发 reset”。
