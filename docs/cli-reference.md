...
- `mo slack status` and the Slack guides exit 0 for every completed read,
  including a projection that reports a definite failure. The nonzero exit on a
  definite failure is not implemented yet.
