...
- `mo slack message send --conversation <conversation-id> [--reply-to <thread-root-ts>] [--connection <connection-id>] [--triggering-message <message-id>] [--dispatch-ref <reply-anchor-ref>] --text <body> [--image <url> | --file <path>]`
  ...
  replies pass `--connection`, `--triggering-message`, and `--dispatch-ref`;
  general sends may omit all three.
