...
- `mo agent launch <agent>` creates an AgentJob, AgentSession, first
  ...
  configuration without creating work. It returns stable Agent, Job, Session,
  Input, Turn, Workspace, and target identities, plus the immutable execution
  record, canonical Session URL, transcript URL, and composite observation URL.
  `--workspace <name>` binds the new Session to an existing Workspace. Without
  it, the command binds the current Project default Workspace. See
  [Workspace](#workspace). The command accepts `--idempotency-key`. When
  ...
  record even after saved defaults or catalog metadata change.
- `mo agent create/edit` configures an Agent with typed `--runtime`, `--model`,
  ...
  Clearing Reasoning effort saves the selected Runtime catalog's default effort.
...
- `mo agent job list <agent>` and `mo agent job view <job-id>` read work state
  and results.
- `mo agent model list --runtime <runtime>` reads the static versioned catalog
...
- Saved Agent Reasoning effort defaults will be statically validated and remain
  independent from Variant.[^433]
- One-job Model and Reasoning effort override, dry-run resolution, and immutable
  execution-record readback will follow after that saved-default contract.[^434]
...
- Agent create and edit use typed Runtime, model, reasoning-effort, variant,
  avatar, Skill, and concurrency flags. Readiness reports the resolved configuration, and the old
