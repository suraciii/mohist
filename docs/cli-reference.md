...
- The Slack setup guide still reads `~/.mohist/slack-credentials.json` when
  `--credentials-file` is absent, offers no hidden terminal input, and accepts
  no `--workspace-team` selector. The Slack grammar above is the target
  contract.
