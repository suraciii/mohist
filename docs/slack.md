...
`mo slack` 覆盖 Slack 接入的全部操作。它的交互模型是一条循环：**CLI 先程序化完成当前
所有能完成的事项**——创建与配置 App、登记记录、校验返回的身份与凭据；只有遇到平台不
开放接口的事（生成 Configuration Token 或 App-level token、浏览器里的安装授权点击）
才停下来，说清要做什么、在哪做、为什么需要，然后等待；用户交付后立即校验并继续自动
推进，直到就绪或遇到下一个缺口。用户不需要自己拼装步骤或记住上次停在哪里——
`mo slack status` 与安装记录随时给出当前状态与唯一下一步。
