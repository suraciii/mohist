...
里与它对话，完成工作区接入、安装与调整 Agent、查看状态、创建 Agent 等管理动作。
...
字段也不会进入审计记录。Slack 的安装授权仍只用于确认实际的工作区与 Agent App，不代表 Mohist
调用者身份。

## 安装模型

一个 Slack 工作区只需要安装一次 **Mohist App**。它是这个工作区的安装与运维入口：把工作区
与一个 Mohist 部署安全绑定，之后为每个要接入的 Agent 创建**属于它自己的 Slack App 与独立
Bot 身份**，而不是让多个 Agent 共用一个 Bot。

因此用户看到谁，就确定调用谁：每个 Agent 仍是原生可 `@` 的独立身份，Agent 的回复也始终
由它自己的 Bot 发出，绝不由 Mohist App 代发。Mohist App 只负责发现、安装、续装、诊断、停用和
卸载这些 Agent 身份，不代替 Agent 执行工作，也不扩大或削减 Agent 已配置的能力。

Slack 仍是交互入口，Mohist 仍是 Agent、工作、会话和调用权限的事实来源。把工作区连接到
Mohist App、或让某个 Slack 成员成为 Owner，都不会自动获得 Mohist 管理权限。

### 本机 Socket Mode

Mohist 的 Slack 集成是本机自托管能力。Mohist App 与每个 Agent App 都由本机
`mohist-slack` 服务通过 Socket Mode 主动连接 Slack；用户不需要给 Mohist 配置公共域名、
公网回调服务或 Slack 登录会话。一个本机服务可以承载多个 App 的连接，但每个 Agent 仍有
自己的 App、Bot 和凭据。

Slack 安装授权必须由有权安装 App 的成员确认；工作区策略要求时还要等待管理员审批。
Socket Mode 还要求每个 App 有一枚只授予 `connections:write` 的 App-level token。Slack 的
公开 App 管理接口可以创建和配置 App，却不在响应中返回可供 Mohist 使用的 App-level token；
官方流程仍要求在 App 设置页生成。因此这一步不能由 Mohist 代替用户。

同一个 Agent 在一个工作区只产生一个 App、一个 Bot；中断或失败的安装恢复同一条安装记录和
同一个 App，不会因为重跑命令而重复创建 Bot。

### App 供给凭据

Mohist 要为工作区创建并持续维护 Mohist App 与各 Agent App，需要一组**工作区级 App 供给
凭据**（Slack Configuration access token 与配套 refresh token）。它把整个接入过程里需要用户
手工处理的凭据压缩到最少：
...
- **运行凭据按 App 提供**：有了供给凭据，Mohist App 与 Agent App 的创建、manifest 配置和
  ...
  Slack 可访问的 HTTPS OAuth callback，Mohist 不伪装成能自动接收这两个结果。

设计原则：**Mohist 自动完成平台允许自动化的部分，用户只完成授权和必要的结果回填**。
引导必须自包含：不假设用户装有 Slack CLI，不索取 Slack 登录 session；每一步的入口、要做
什么、为什么需要，以及完成后如何继续，都由 `mo slack` 直接给出。凭据不得通过 Mohist App
对话、命令参数、日志或 Session transcript 传递。

## 与 Mohist App 对话

Mohist App 不仅完成一次性安装，也是 Slack 里的常驻管理入口。用户在它的私聊中
用自然语言完成全部日常管理动作：

- **安装 Agent**：「把 review-bot 安装到 Slack」。Mohist App 建立可恢复的 Connection
  记录、创建 Agent App 并发回安装授权链接。需要提交 Bot Token 或 App-level token 时，它只
  引导用户在 Mohist 主机运行 `mo slack install-agent review-bot` 继续，不在 Slack 对话里接收
  secret。整个过程是同一条安装记录；中断后重跑同一命令即可从当前步骤继续。
- **查看与诊断**：「现在接了哪些 Agent」「review-bot 什么状态」。Mohist App 逐项给出
  当前状态与唯一下一步，与 Web、CLI 看到的同一批事实。
- **调整与生命周期**：调整访问策略、停用或启用一个接入、发起 Owner 转移。永久删除
  Slack App 不进入对话——它需要二次确认与审计，只在 Web 与 CLI 操作。
- **创建 Agent**：「帮我建一个每天盯 CI 失败的 Agent」。Mohist App 最多追问两件事——
  名字和日常做什么——其余使用默认配置：它替用户起草 Instructions、选用默认 Runtime
  与 Model，不逐项追问技术配置。创建完成后立即引导安装到 Slack。
...
谁能驱动 Mohist App 与谁能管理对应资源是同一批人：Mohist App 只对有权管理目标 Connection
...
- 一个接入对应一个 Slack 工作区中的一个 Bot 身份；这个 Bot 由 Mohist App 为该 Agent 创建的
  独立 Agent App 产生。
...
把一个工作区连接到 Mohist App 需要一个有权在该工作区创建和安装 App 的 Slack 成员完成一次
授权，并一次性提供 App 供给凭据（见「App 供给凭据」）。**很多工作区默认禁止成员自行安装
App**，这时 Mohist App 的安装本身要过一次管理员审批；
之后 Mohist App 为各 Agent 创建的 Agent App，以及这些 Agent App 的安装授权，仍可能各自需要审批。
先确认工作区的 App 安装策略与 plan 是否支持这套方式，再决定要把哪几个 Agent 接进来。

每个 Agent 的安装是一个**可恢复的流程**：它有稳定的身份，中断、超时、用户取消或管理员
待审批后都能回到同一个 Agent App 继续下一步，不会重新创建一个 Bot。Mohist App 每一步只展示
一个当前状态和唯一的下一步动作，不让用户自己拼装结论。

`mohist-slack` 服务可以稍后安装或恢复。未就绪时 Connection 保留在对应等待状态，而不是让用户
重新开始。Agent Readiness 与 Connection
安装相互独立，因此可以并行完成。
...
安装可以由 `mo slack install-agent <agent>` 发起，也可以在 Mohist App 对话中发起；两者调用
同一条 Server 侧安装流程，并在 Mohist 与 Slack 两边留下可恢复的进度。涉及 secret 的步骤
只能回到本机 CLI 完成。每一步的当前状态与唯一下一步都由同一条安装记录决定。

### 先安装 Mohist App

1. 用户在 Mohist 主机运行 `mo slack setup`。若还没有 App 供给凭据，CLI 引导用户在自己的
   浏览器打开 Slack App 管理页，生成 Configuration access/refresh token pair，再通过隐藏输入
   或受保护文件提交。CLI 不登录 Slack，也不索取浏览器 session。
2. Mohist 校验凭据所属 workspace，生成固定的 Mohist App 配置并创建 App，然后显示 Slack
   安装链接。用户在自己的浏览器确认 **Allow**；需要管理员审批时，setup 保留进度并等待。
3. 授权完成后，CLI 引导用户从 **OAuth & Permissions** 提供 Bot User OAuth Token，并从
   **Basic Information → App-Level Tokens** 生成只含 `connections:write` 的 App-level token。
4. `mo slack setup` 验证 workspace、App、Bot 与 Socket，安全保存凭据并启动或恢复本机
   `mohist-slack`。只有 Socket 连接建立且 Mohist App 身份确认后，它才显示就绪。任何中断都
   通过重跑 `mo slack setup` 继续同一个 App。

### 选择 Agent 并准备安装

1. 在 CLI 或 Mohist App 中，安装者看到自己**有权管理 Connection** 的 active Agent 列表，每项给出当前
...
2. 安装者选择一个 Agent，确认将出现在 Slack 的 Bot 名称、头像与说明、该 Agent 需要的 Slack
   ...
   键；出现重名时 Mohist App 先给出一个带稳定后缀的备选，再创建。
3. Mohist 建立一个**可恢复的 Connection 与安装记录**：固定目标 Agent 与目标工作区，
   但此刻还没有 Slack App 或 Bot。之后所有进度都落到这条记录上，中断后回到它继续。

### 创建 Agent App 并完成安装授权

4. Mohist 用已供给的 App 供给凭据，为这个 Agent 生成一份版本化的 App 配置，并创建一个
   独立 Slack App。创建可能因超时或网络中断而结果未知；此时安装记录进入**结果未知**状态，
   **不会自动再创建一个 App**，必须先与 Slack 核对或由人工裁决，确认同一个 Agent 仍只对应
   一个 App。
5. CLI 或 Mohist App 引导安装者完成这个 Agent App 的 Slack 安装授权。工作区开启审批时，等待管理员
   批准——这一步不能绕过。用户取消授权、授权过期或待审批时，Mohist 都保留同一个 App，
   下一次继续，不新建 Bot。
6. 安装授权完成后，Mohist 校验返回的工作区、App 与 Bot 身份确实和预期一致；任何不匹配都
   不保存凭据、不绑定 Connection，并给出明确的下一步。

### 取得本机运行凭据

7. 安装者在该 Agent App 的 **OAuth & Permissions** 页面复制 Bot User OAuth Token，并在
   **Basic Information → App-Level Tokens** 生成一枚只含 `connections:write` 的 App-level
   token。`mo slack install-agent <agent>` 通过隐藏输入或受保护的凭据文件接收两者，立即验证
   workspace、App、Bot 与 Socket Mode 能力；不匹配时不保存、不绑定。两枚凭据齐备前，
   Connection 不进入就绪状态。凭据不出现在 Agent Instructions、消息、日志或 Session transcript。
...
8. 身份与凭据验证完成后，进入 **Claim owner**。安装者选择 **Generate owner code**，Mohist
   ...
   收取私聊并回复。认领成功后，Bot 在同一私聊中确认认领，并给出简短的自包含首次使用
   引导：在私聊里直接发任务派活、把 Bot 拉进频道后 `@` 它派活、在 Bot 回复的 thread 里
   继续追问。引导不要求用户阅读文档；它只在首次认领成功时出现，Owner 转移成功后不重复
   引导。
...
Mohist 无法在没有平台证据时知道用户是否完成了外部 Slack 步骤，因此不伪造一个没有凭据或授权
...
`mo slack` 覆盖 Slack 接入的全部操作。安装命令是幂等、可续跑的向导：CLI 每次先推进所有
可自动化步骤；只有遇到 Slack 必须由用户完成的授权或 token 生成时才停下，给出精确页面、
动作和继续命令。重跑同一命令读取持久进度，不产生第二个 App。

- `mo slack setup` 把工作区级 Mohist App 安装到 Slack，并让本机 `mohist-slack` 建立它的
  Socket Mode 连接。它负责取得 Configuration token pair、创建/配置 App、引导安装和收集运行
  凭据；已 setup 的工作区会进入检查、修复或安全轮换，不重复创建 Mohist App。
- `mo slack install-agent <agent> [--workspace-team <team-id>]` 把一个已经存在的 Mohist Agent
  安装到 Slack。命令创建或恢复接入记录和专属 Agent App，推进安装、凭据验证、连接上线与
...
产出的记录同样由 `install-agent` 查询和继续。Slack App 创建、manifest 更新、凭据提交
和投递恢复是安装流程内部步骤，不拆成要求用户编排的常规命令。
...
| Slack workspace | Bot 所在工作区；由 Mohist App 安装结果确认，不靠用户手填名称判断 |
...
| 运行方式 | 本机 Socket Mode；每个 Mohist App 与 Agent App 都有独立 Bot Token 和 App-level token |
...
Socket Mode 不要求 Mohist 暴露公共入站地址，但 Slack 对未确认消息只保留有限
...
| 创建 Agent App 超时或结果未知 | 进入「结果未知」，禁止自动再创建；必须先与 Slack 核对或人工裁决，确认同一 Agent 仍只对应一个 App |
| 安装授权取消、过期或待审批 | 保留同一个 Agent App，恢复后继续，不新建 Bot |
| 安装授权返回的身份与预期不符 | 不保存凭据、不绑定 Connection；给出明确下一步 |
| Slack 凭据失效 | 接入进入 Degraded 并停止接受新输入；重跑 `mo slack install-agent` 修复，必须仍验证为原 workspace、App 与 Bot，否则按新安装处理 |
| 同一 Agent 已接入该 Slack workspace | 不覆盖已有 Connection；指向已有接入，由用户决定是否删除重复项并在 Slack 卸载多余 App |
| Socket Mode 暂时不可用 | 已创建的 Connection 与安装进度保留；新消息由 Slack 在其重试窗口内重投，超出平台保留窗口的消息可能需要用户重新发送 |
...
- Mohist App 不代替 Agent 发送回复，也不是多个 Agent 共享的执行身份。
...
- 本机安装不承诺「一句话零步骤全自动」：安装者要完成 Slack 安装授权，工作区策略要求时
  还要过管理员审批，并为每个 App 提供 Slack 页面显示的 Bot Token 和手工生成的 App-level
  token。Mohist 必须自动完成除此之外的 App 创建、配置、校验和运行接入。
