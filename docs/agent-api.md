...
The Server task-first route uses the same Project key space:
`POST /api/projects/{projectRef}/agent-tasks`. Its closed request body accepts
only `prompt`, `attachments`, `context`, `name`, `runtime`, `model`, and
`variant`; the `context` object uses the definition-first launch reference
fields. It requires `Idempotency-Key`, derives and materializes a complete
Agent definition, and returns the standard Agent, Job, Session, Input, Turn,
workspace, attachment, and canonical URL projection.

Replay is caller-visible and stable. The same key and the same prompt, context,
attachments, name, and execution hints return the original identities. Changing
any of those fields, including adding, changing, or removing `model` or
`variant`, returns `409 launch_idempotency_conflict`. A plan still converging
returns `503 launch_setup_pending` with the key; retry that key to resume it.
Recorded rejection outcomes are replayed without creating an Agent. A key
reused between task-first and definition-first launch surfaces is compared in
the same coordinator space and conflicts when its fingerprint differs.
