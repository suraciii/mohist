...
能力声明、启动即知、`mo agent spawn`、终态回报、`mo session tree`、级联停止与
detach 已按本篇落地（交付增量 1–3）。仍未落地的是：

- **受管隔离工作空间**（交付增量 4）：子会话目前仍只继承父会话的工作目录，不能
  请求 Project 管理的隔离工作空间，也没有 `--worktree` 之类的路径输入。
- **定时输入**（到点唤醒一段会话）：没有它，「子会话卡住、永远不到终态」只能靠人
  查看会话树兜底。方向已记录，由后续 issue 落地。
