...
一次 spawn 的重试只会收敛为一个结果：子会话被接纳，或明确拒绝。父子关系尚未建立前，
检查不通过不会留下子会话；建立过程中的条件变化会让这次委托明确拒绝，子会话不会开始
执行。关系一旦建立，子会话就是普通的工作，之后的停止或取消按其正常生命周期处理，
不会因为父会话后来变化而改写成拒绝。同一个 key 重试始终得到同一个已接纳子会话或同一
拒绝结果；被拒绝后即使条件恢复，也要用新的 key 再次委托。
