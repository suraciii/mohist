...
需要让子会话在独立工作空间里干活时，加 `--workspace worktree`（默认 `--workspace inherit`
为与父共享工作目录）；隔离工作空间由系统创建，不接受路径。
...
- **工作目录：共享或隔离**：子会话随父会话当前可用的工作目录开始，有两种选择——**共享**
  （默认，与父会话用同一个工作目录）或**隔离**（由系统从父会话当前工作进度派生一个独立的
  工作空间，子会话在里面独立工作）。调用方不能指定路径：工作目录必须是父会话当前可用的
  工作目录，隔离工作空间由系统创建，不是调用方给的路径。隔离工作空间要求父会话当时正工作在
  项目声明的仓库上；父会话不在项目仓库上时，隔离请求会被明确拒绝。父会话没有这种工作目录或可用
  执行环境时，spawn 会明确拒绝，不会悄悄换地方执行。当前通过 Agent 接入启动的会话没有可供子会话
  延续的工作目录和执行环境，因此不能作为父会话 spawn；以后由它自己的会话具备这些条件后才会
  开放。文件冲突仍由父的分单纪律避免，git 是最终仲裁。
...
detach 与定时输入已按本篇落地（交付增量 1–3 与 5）。受管隔离工作空间（交付增量 4）
也已按本篇落地：`mo agent spawn --workspace worktree` 会从父会话当前的项目仓库派生
隔离工作空间并完成子会话创建；不再被使用的隔离工作空间会按会话的空闲与活动情况自动
回收，找不到对应会话的隔离工作空间也会在多次复核确认后安全回收，父会话仍在使用的同一
仓库下的子会话工作空间不会被误删，显式释放也照常立即回收。
