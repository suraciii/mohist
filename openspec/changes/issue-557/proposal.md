...
- Extend Agent create/edit/read surfaces so Web, CLI, and API users can set, clear, and see the final `runtime`, `model`, `reasoningEffort`, and `variant` values. Agent list/detail and launch/job result surfaces must use the same vocabulary and projection.
...
- Resolve the Agent's final execution tuple before creating work, then freeze `runtime`, `model`, `reasoningEffort`, and `variant` into the AgentJob, AgentSession, and Runner dispatch snapshot. The Runner must deliver effort and variant independently, and later Agent edits must not change an accepted Job.
...
- **Web and CLI:** Agent editor controls, model/effort compatibility messaging, list/detail summaries, launch observation/result views, typed Agent create/edit flags, and table/JSON output.
