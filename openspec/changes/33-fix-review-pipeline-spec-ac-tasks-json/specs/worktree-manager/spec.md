...
### Requirement: mergeBack SHALL include openspec/changes/ in commit
The mergeBack operation SHALL NOT exclude `openspec/changes/` from the staging step. The `git add` pathspec SHALL stage all changes including openspec/changes/ so that tasks.json updates committed by ralph are preserved through merge.

#### Scenario: mergeBack includes tasks.json from openspec/changes/
- **WHEN** mergeBack stages uncommitted changes
- **THEN** the system SHALL use `git add` without the `:!openspec/changes/` exclusion
- **AND** tasks.json updates from ralph (committed in the worktree) SHALL be included in the merge commit
- **AND** the `:!.opencode/` exclusion SHALL remain to skip opencode config files

#### Scenario: mergeBack preserves ralph's tasks.json commits
- **WHEN** ralph has committed tasks.json updates in the worktree during build
- **AND** mergeBack is called
- **THEN** those commits SHALL be part of the merge
- **AND** the final tasks.json on the base branch SHALL reflect all task status updates
