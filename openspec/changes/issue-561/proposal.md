...
- Build and install Server and Runner artifacts from that snapshot into stable, self-contained, versioned managed runtime locations. Compiler intermediates and web/Runner output stay outside the read-only snapshot, and service units switch to absolute active artifacts instead of depending on an arbitrary worktree or relative build output. **BREAKING** for installations or custom units that rely on source-bound working directories and relative entrypoints.
...
- **Managed services and local filesystem:** systemd and Windows launcher generation, fixed per-user runtime roots, versioned self-contained runtime storage, activation links, and rollback state change from source-bound execution to verified installed artifacts. No external dependency is required.
