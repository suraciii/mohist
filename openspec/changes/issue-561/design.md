...
6. `Cancelling`: cancellation cleanup or rollback is in progress.
7. `RollingBack`: previous target restoration is in progress.
8. `RolledBack`: previous verified target was restored and verified; the update still returns failure.
9. `Cancelled`: cancellation cleanup or rollback completed; the command returns exit code 130 and never success.
10. `NoVerifiedRuntime`: candidate was stopped/removed because no verified target existed.
11. `RecoveryFailed`: candidate services are stopped/disabled, `active.json` has `status: "none"`, and neither candidate nor previous target could be verified.

For a full update, `CandidateStaged` means that the CLI, Server, Runner, web assets, dependency closure, launch metadata, and identity metadata are complete. No CLI-slot replacement, service stop, service-unit change, or active-record publication occurs before that state is durable. After `CandidateStaged`, the stable launcher selects the candidate entries by atomically publishing `active.json`; service units and the CLI wrapper remain stable and resolve only that record. The coordinator then records `CandidateActivated` and `Verifying`, restarts only the requested scope, and commits only after identity checks pass. A crash in `Prepared` before `CandidateStaged` cleans the incomplete candidate without changing the previous active record. A crash after `CandidateStaged` but before activation leaves the previous record authoritative and cleans the unused candidate.
...
`RuntimeTransactionReconciler` runs before a new update and from the stable managed launcher. For every nonterminal record it compares the active record with the recorded candidate and previous sets. A `Prepared` record without `CandidateStaged` is cleaned and terminally marked; an unapplied candidate is cleaned and terminally marked; an active candidate is bounded-verified and either committed or rolled back; `RollingBack` and `Cancelling` records resume restoration; a missing or corrupt active record attempts the previous target and ends in `RecoveryFailed` only after stopping candidate services and atomically publishing `active.json` with `status: "none"` when that target cannot be verified. Recovery has a bounded timeout using `TimeProvider`; it does not poll without a deadline, and no concurrent update can start while a record remains unresolved.

The transaction store uses one OS-backed lock file per managed runtime root. The lock file is created with exclusive creation and contains the schema version, transaction ID, owner process ID, owner process-start token, and creation time; the file contents and containing directory are flushed before `Prepared` is durable. The process-local semaphore is only an optimization and never proves ownership. A lock is stale only when the recorded owner process-start token is no longer live or the transaction is already terminal; an ambiguous owner fails closed. Reconciliation must resolve the transaction before removing a stale lock. The lock remains held until the terminal state is durably written; a release failure sets `lockReleasePending` in that terminal record and is retried before the next update. Transaction files and `active.json` are written to same-directory temporary files, flushed through the injected atomic-file boundary, and replaced with write-through semantics; Unix directory entries are flushed after rename and Windows uses the platform replace operation with write-through semantics. Kill-point tests cover each state, target, service, CLI-slot, and lock boundary.

The selected CLI slot is part of `active.json`. `Committed` and `RolledBack` select a verified release CLI slot. `NoVerifiedRuntime`, `Cancelled` after failed candidate activation, and `RecoveryFailed` set the Server/Runner target set to none but select the last trusted recovery CLI slot; the stable `launchers/cli` wrapper executes only that slot and prints `bootstrap_required` rather than executing the candidate when no trusted recovery slot exists. The supported manual repair command is `mo install cli`, which recreates the recovery slot, stable launchers, and runtime-root metadata before another `mo update`.
...
The persisted `SystemUpdateJobState` schema is versioned and records `owner` (`web` or `cli`), `status`, `scope`, `sourceMode`, target identity, sequence, and `lockReleasePending`. Existing records without `owner` are classified as `web` because all records written by the current Server are web-owned; startup writes the schema version and then applies the quarantine rule. `rejected` is a terminal status reserved for quarantined web jobs. The registered `SystemUpdateStartupReconciler` replaces `SystemUpdateRecoveryService` and runs as a direct hosted service before the HTTP listener accepts requests. It persists the rejected state, completion time, reason `UpdateMutationOwnedByCli`, and log entry before releasing the stale web lock. If release fails it persists `lockReleasePending: true`, leaves the job rejected, and retries the release on the next startup; it never resumes the web job or changes it back to `failed`.

A CLI transaction creates `transactions/<job-id>/projection-lease.json` before its first outcome. The lease binds the job ID, a cryptographically random nonce hash, scope, source mode, target identity, and last accepted sequence, and the local transaction record must exist with owner `cli`. `SystemUpdateOutcomeRequest` requires the job ID, nonce, sequence, status, stage, outcome, scope, source mode, and target facts. The Server reads the lease through `IUpdateProjectionLeaseStore`, rejects an unknown job, missing or mismatched lease, changed target facts, invalid transition, or non-increasing sequence, and never creates a projection for an arbitrary job ID. A repeated sequence with the identical payload is idempotent. A successful projection is accepted only when the local transaction record is durably `Committed` with matching identity evidence; the status endpoint never advances that record.
