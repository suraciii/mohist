...
- Add the `mohist/agent` Workflow Action, with required static Agent `name` (or id) and template-renderable workflow `prompt`, plus optional `session` and `timeout` inputs, for both tasks and checks. `agent_*` references resolve by id only; all other references resolve by name before an id fallback.
...
- `docs/actions/agent.md` expands its current task-only product contract to cover checks and documents static-name validation and resolver precedence.
