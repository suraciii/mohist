...
- **THEN** a fifth recognized OTLP trace request SHALL receive HTTP `429` with `Retry-After` before the receiver reads its body
...
For every committed write block, the receiver SHALL persist each accepted Span idempotently and SHALL maintain the associated Trace summary with the correct span count, earliest start time, latest end time, and first stored service name. The aggregate-maintenance work for an ingestion request SHALL grow linearly with its accepted Span count and MUST NOT rescan all Spans of a Trace once for each received Span.
...
- **THEN** the receiver SHALL return HTTP `429` with `Retry-After` and `application/x-protobuf`
- **AND** the response body SHALL decode as the applicable OTLP error response without a JSON fallback
