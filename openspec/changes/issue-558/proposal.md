...
- The Agent page history becomes result-bearing:
  - Each session row identifies the task (first-input excerpt), its origin and context references (Issue, Epic, repository, workspace, Slack), start and end times with elapsed duration, accumulated cost, and the current Activity as its own separate signal.
...
- The Agent-scoped session list read model carries the facts the history needs: a first-input subject excerpt, the terminal Turn result facts, the session's accumulated cost, and its derived end time (elapsed computed from the recorded lifecycle anchors) per session. The existing AgentJob read surface and launch observation are reused rather than duplicated.
- The Session view explains results in its first viewport:
  ...
  - For launch-origin sessions — direct launches and Slack connection sessions alike, both of which are AgentJob launches — the first AgentJob result is presented as the launch result and stays distinct from later Turn results; later Turns never rewrite it.
...
- `agent-history-results`: The Agent page history explains each execution — per-session task subject, origin and context references, start/end/elapsed and cost, current Activity, and execution outcomes (first AgentJob result and latest Turn result) presented with result vocabulary including honest unknown handling, and the session-list read-model facts it consumes.
