...
Before a write transaction, `TraceIngester` makes one complete classification pass over the materialized request and creates an immutable `PreparedIngestBatch`. Every identifiable Span attempt is provisionally classified as parsed-for-write, malformed dropped, protection rejected, or another non-retryable drop. An `IngestOutcomeBuilder` with a private-result constructor combines that classification with exactly one write result: `not_attempted`, `committed`, `rolled_back` with bounded reason text, or `cancelled`. It derives the response disposition, all four counters, and degradation changes, so callers cannot provide contradictory aggregates.

The write result has batch-level precedence. When the accepted subset commits, or when no accepted subset exists and no write is attempted, the provisional non-retryable classifications become final: committed parsed Spans are saved and rejected/dropped attempts produce HTTP 200 partial success. When an accepted subset rolls back because of a storage failure, the whole request instead returns retryable HTTP 503. That attempt records every successfully parsed Span as received, records zero saved/rejected/dropped, activates only `storage_write`, and neither activates nor refreshes `ingest_protection`; provisional malformed/rejected/drop classifications do not become non-retryable loss while the exporter is being asked to retry the whole request. Repeated rollback retries can therefore add received attempts but cannot repeatedly add rejected/dropped counters.

If the request cancellation token is signaled after the transaction begins, `TraceIngester` rolls back, records the batch's single outcome with write result `cancelled`, and rethrows the same `OperationCanceledException`; `OtlpTraceResponseWriter` does not synthesize an OTLP response. A cancelled outcome records every successfully parsed Span as received and records zero saved/rejected/dropped because no non-retryable result reached the exporter. It neither activates nor clears `storage_write` and neither activates nor refreshes `ingest_protection`, so client cancellation cannot create a storage failure or recover an existing one. A transaction exception is classified as `cancelled` only when the request token is signaled; other rollback exceptions remain storage failures. A transaction-gate fake exposes an awaitable started signal so tests cancel after begin without scheduler or wall-clock waits.

Only `committed` from a transaction that actually entered the production write path clears `storage_write`; an empty, wholly rejected, wholly dropped, parse-failed, or cancelled request cannot establish or restore write readiness. The same atomic `RecordIngest` call refreshes protection only when final rejected or dropped counts are non-zero.
...
Move startup orchestration out of top-level statements into an internal `MohistHostRunner`. It accepts `IMohistHostFactory`, `IOtelBindFailureClassifier`, and `IMohistDatabaseInitializer`. `IMohistHostFactory.CreatePrimary` and `CreateAlternate` return an `IMohistHost` exposing `Services`, `StartAsync`, `StopAsync`, `DisposeAsync`, and `WaitForShutdownAsync`; the production adapter wraps `WebApplication`, while tests use signal-controlled fake hosts. `MohistDatabaseInitializer` is the production implementation and invokes the existing `DatabaseInitializer.InitializeAsync(services, cancellationToken)`, which applies EF migrations and then the repository data upgrade. Keeping initialization as a runner dependency makes the ordering testable without starting or adapting a real `WebApplication`; the constructor requires production composition to supply the concrete initializer. The production factory creates an immutable `MohistHostPlan` containing the shared `RuntimeEpoch`, initial `CollectorResult`, enabled intent, and listener intents. Primary and alternate both pass that plan through one `Build(plan)` composition path that always configures the same Orleans, routes, DI, and one sampler registration; alternate is derived only by removing the OTLP listener intent and replacing the initial collector result. No alternate-only builder path may duplicate those registrations. The classifier wraps `OtelBindFailureDetector` and receives the startup exception plus the collector listener intent containing `BindHost` and `Port`. It never receives or interprets the outbound trace exporter's `Endpoint`; that URI can differ from the built-in listener address without changing bind-failure classification. Top-level `Program` only builds production adapters, captures the epoch, constructs the runner with `MohistDatabaseInitializer`, and invokes it.

For every host attempt, the runner invokes `IMohistDatabaseInitializer.InitializeAsync(host.Services, cancellationToken)` exactly once after construction and before `StartAsync`; the primary therefore migrates/upgrades before its start attempt, and a fallback alternate repeats the idempotent initialization before its own start, preserving both current startup paths. An initialization failure is terminal and is never passed to the bind-failure classifier: `StartAsync` is not called, no alternate is created for a primary initialization failure, and the runner always attempts to dispose the unstarted host. If initialization and disposal both fail, it throws an `AggregateException` preserving initialization then disposal order.
...
Only after clean shutdown does `CreateAlternate` receive the same epoch plus `collector_bind_failed`. The alternate plan omits only the OTLP listener, preserves `Mohist:Otel:Enabled`, and constructs its new `RuntimeObservability` with the explicit ordered seed list `[storage_unverified, collector_bind_failed]` before `StartAsync`; the constructor follows D6's ordered-seed rule, so the alternate's initial snapshot exposes `collector_bind_failed` as `latest_degradation`. The alternate never publishes a normal `collector` recovery, so the collector source remains active and status remains degraded. Later process, storage, or protection events follow D6 normally and can replace the latest reason without clearing the collector failure. The failed primary can publish its initial process sample but never receives `ApplicationStarted`; only the alternate enters post-start sampling, so exactly one storage probe occurs across both host attempts.

Runner unit tests use fake factory/hosts/classifier/database initializer and fake application-start signals to trigger primary/alternate initialization failure, generic start failure, classified bind failure, stop failure, dispose failure, and dual failure without ports. They own lifecycle-boundary assertions: the initializer receives each host's service provider before its start, a failed initialization prevents that start and is disposed without fallback, initialization/disposal failures retain order, stop/dispose ordering is preserved, no alternate follows shutdown failure, at most one started host remains, the primary performs zero storage probes, the alternate performs one immediate probe, and the alternate snapshot projects degraded with `collector_online=false`. Extend the existing `DatabaseInitializationSpecs` and `DatabaseInitializerRepositoryUpgradeSpecs` to invoke `MohistDatabaseInitializer` rather than bypassing it; together their permitted database-initialization fixtures prove that the concrete production dependency applies migrations and then upgrades repository data. Runner tests do not issue HTTP requests or claim that a production silo was started.

A separate non-starting production composition test owns only static graph assertions. It compares primary and alternate `MohistHostPlan` values, verifies that epoch/enabled/application composition are identical while only OTLP listener intent and initial collector result differ, and inspects service descriptors to prove one sampler registration and the shared Orleans/API registration markers per plan. It does not call `Build`, invoke the database initializer, call `StartAsync`, resolve physical filesystem adapters, open listeners, or claim runtime probe/silo facts. Because the production adapter has one `Build(plan)` implementation and no alternate composition branch, the inspected plan and registrations are the exact inputs consumed by production construction.
