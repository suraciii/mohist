...
- Add per-request candidate, processed, transcript/database, and downstream-call counts to the existing project-scoped agent status and activity responses. Add `/api/agent/status` and `/api/agent/activity` compatibility routes that require an explicit project selector and never aggregate all projects.
...
- **Agent read paths** (`packages/server/src/Mohist.Server/Api/AgentRoutes.cs` and AgentOps assemblers/queriers): add response-local amplification counts while preserving canonical project routes, plus explicit-project compatibility routes at the literal paths.
