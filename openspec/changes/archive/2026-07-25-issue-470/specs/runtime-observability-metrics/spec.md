...
Telemetry outcomes SHALL count Span attempts. `received` SHALL count each Span attempt that is successfully parsed at the ingest boundary. `saved` SHALL count each parsed Span whose write commits, including a duplicate upsert that commits successfully. `rejected` SHALL count each parsed Span intentionally refused by admission or storage protection with a non-retryable OTLP partial-success response. `dropped` SHALL count each malformed or otherwise lost Span for which the response does not request a retry. Provisional Span classifications SHALL become rejected or dropped outcomes only when the batch response is non-retryable. If the accepted subset's write rolls back because of storage failure, the retryable result SHALL take precedence for the entire request: every parsed Span attempt SHALL increment `received`, while `saved`, `rejected` and `dropped` SHALL not increment, and only storage-write degradation SHALL activate. If the request cancellation token is signaled after the write transaction begins, the batch SHALL publish one cancelled outcome and propagate the original cancellation without an OTLP response: every parsed Span attempt SHALL increment `received`, `saved`/`rejected`/`dropped` SHALL not increment, `storage_write` SHALL remain unchanged, and `ingest_protection` SHALL neither activate nor refresh. The ordinary time-based protection-expiry evaluation still applies to this publication. The four counters MUST NOT be required to satisfy a conservation equation.
...
#### Scenario: A request is cancelled inside its write transaction

- **WHEN** an awaitable transaction test boundary confirms that a batch has begun writing and its request cancellation token is then signaled
- **THEN** exactly one cancelled ingest outcome SHALL be published and the original cancellation SHALL propagate without an OTLP response
- **AND** every parsed Span attempt SHALL increment `received`, while `saved`, `rejected` and `dropped` SHALL NOT increment
- **AND** cancellation SHALL neither activate nor clear storage-write degradation and SHALL NOT activate or refresh ingestion protection; ordinary time-based protection expiry SHALL still apply
- **AND** verification SHALL use signals rather than scheduler or wall-clock waits
...
Whole-body malformed JSON or protobuf SHALL return HTTP 400 with `google.rpc.Status` code 3 (`INVALID_ARGUMENT`) in the recognized request encoding and SHALL NOT publish an ingest outcome. A retryable storage-failure rollback SHALL return HTTP 503 with `google.rpc.Status` code 14 (`UNAVAILABLE`) in the request encoding. This retryable response SHALL take precedence over partial success when the same batch also contains provisionally rejected or dropped attempts. A request-token cancellation after transaction start SHALL propagate unchanged and SHALL NOT be mapped to HTTP 503 or any other OTLP response. Error messages SHALL be bounded to 256 characters and `details` SHALL be absent. When no supported request encoding can be selected, the route SHALL return HTTP 415 with a JSON `google.rpc.Status`.
