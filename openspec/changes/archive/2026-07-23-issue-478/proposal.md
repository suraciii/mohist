...
- **Runner** (`packages/runner`): update the `setVars` patch URL (`server/connection.ts`) to the renamed Run variable route.
- **Web** (`packages/web`): migrate the production variable API clients to the clean `/variables` path — `entities/settings/api/client.ts` (Settings AI variables) and `entities/issue/api/client.ts` (Issue workflow variables) — plus their MSW handlers and Web tests, since T-001 removes the `workflow-profile/variables` routes they currently call.
- **Tests** (`packages/cli/tests`, server tests, Web tests): new specs for the shared key-path / `--stage` / value-type rules across all three scopes, Run `--effective`, target resolution, write-boundary rejection, and no-remote local usage failures; preserve the attempt-snapshot invariant (accepted attempts keep their context, new attempts use the latest Variables); update server path-contract specs and Web specs to the renamed routes.
