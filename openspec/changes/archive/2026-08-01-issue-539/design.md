...
**Rationale.** ETag is bumped by `WorkflowRunStore.StageRunAsync` on every save (a save-count, not a content hash — see D3), and is the version signal this cache keys on. The aggregate is the single thing whose reconstruction is the proven LOH source (the full typed `JSON.Deserialize<WorkflowRun>`). Definition and artifacts are *not* versioned by ETag and must stay live-fresh; by leaving them per-call we inherit their existing freshness semantics for free and avoid inventing version signals for them. `BuildStatusView` and `AttachArtifactSummariesAsync` only *read* the aggregate and build new view objects (they mutate the view, not the run), so the cached aggregate is safely shareable as a read-only snapshot. `Hydrate` also applies `WorkflowRunLineage.RestoreStoredEpicNumber(run, row.EpicNumber)`; EpicNumber is written in the same save transaction as State, so the ETag covers it too — the cached entry stores the post-restoration aggregate.

**Scope note (residual `State` reads on a hit).** Because definition resolution runs per call, `WorkflowDefinitionResolver.LoadTemplateAsync` still touches `State` on a hit: `ResolveRunContextAsync` loads the full row (`WorkflowDefinitionResolver.cs:220-221`) and `LoadBoundProfileIdAsync` does `Select(x => x.State)` + a pooled-buffer `JsonDocument.Parse` (`:234-241`). Those are lightweight metadata reads, *not* the LOH-allocating typed deserialize, and they preserve live profile-edit freshness. This change targets only the typed deserialize (the profiled dominant cost); eliminating the definition-resolution `State` reads on the hot path is a separate follow-up (see Open Questions). The spec's hit-guarantee is scoped accordingly.
...
Each call reads the current `ETag` and compares. There is **no time-based expiry**: an entry is valid iff its stored `ETag` equals the row's current `ETag`. Staleness is impossible as long as each call re-reads the ETag.

**ETag is a save-count, not a content hash.** `WorkflowRunStore.StageRunAsync` re-serializes `State` and bumps `ETag` unconditionally on every update save (`WorkflowRunStore.cs:160-166`), even when the serialized content is byte-identical to the prior write. (The byte-idempotency rule in `design/workflow/run-state.md:64-66` applies only to cold-start data upgraders, not the runtime store.) Consequence: a redundant grain save invalidates the cache and triggers one redundant deserialize even though `State` did not change. Correctness is unaffected (the spec's "ETag differs → deserialize once" still holds); only the hit-rate ceiling depends on the grain not saving redundantly. The cache treats ETag as an opaque per-save revision number, which is exactly how the store maintains it.
...
- **Eliminate the definition-resolution `State` reads on a hit.** On a cache hit the cached `WorkflowRun` already carries `WorkflowProfileId`, so the bound-profile-id extraction in `LoadTemplateAsync` (which reads `State` twice — see D1 scope note) could be bypassed by resolving the definition straight from the cached aggregate. This is a deliberate non-goal for this change (it refactors the querier↔definition-resolver coupling and risks live profile-edit freshness); adopt only if profiling shows those residual `State` reads are a meaningful LOH contributor after the typed deserialize is eliminated.
