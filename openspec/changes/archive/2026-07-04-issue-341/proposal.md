...
- `issue-detail-reference-rail`: The reference column — it holds only metadata, low-frequency configuration, and non-runtime issue actions (details, model, workflow-profile control, prerequisites, the non-runtime IssueActionsCard), renders as a right rail on desktop and as collapsed sections on narrow screens, defaults low-frequency items (drift, convergence) to collapsed, and carries the lightest visual weight.
