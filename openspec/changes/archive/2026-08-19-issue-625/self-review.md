...
- **M-4 — fixed properly.** `proposal.md:9`, `design.md:13,36`, `tasks.json:54,57,60,70`, and `specs/verification-lanes/spec.md:55-68` put `export DOTNET_ROOT=/home/user/.dotnet` in each `verify-dotnet` lane before the unchanged `dotnet test Mohist.sln --nologo -m:1 -p:UseSharedCompilation=false` command. This matches the live project variable and accounts for each `core/script` task's fresh shell.
