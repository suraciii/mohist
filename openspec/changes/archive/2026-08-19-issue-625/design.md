...
- Replace aggregate verification in both built-in profiles with six ordered, independently reportable lanes:
  1. `npm ci`
  2. `export DOTNET_ROOT=/home/user/.dotnet` followed by `dotnet test Mohist.sln --nologo -m:1 -p:UseSharedCompilation=false` in the same lane shell
...
1. **Use ordinary workflow tasks as the lane boundary.**
   ...
   The `verify-dotnet` run body carries the existing `export DOTNET_ROOT=/home/user/.dotnet` prelude before its unchanged test command. This is required because each `core/script` task runs in a fresh shell; exporting the variable in `verify-install` would not reach the .NET lane. The prelude is setup, not an additional verification command, and must be present in both built-in profiles. This uses the existing task ordering, claim, report, and retry machinery. A single new `verification-lanes` Action was considered, but it would have to duplicate durable child identities, partial-result persistence, ordered scheduling, timeout attribution, and recovery fencing inside the Runner. Six workflow tasks make those facts control-plane state instead.
...
6. **Keep the CI contract explicit and testable.**
   ...
   Profile-definition tests will inspect both built-in YAML definitions for the six IDs, exact command lines, order, finite per-lane budgets, the per-lane profile-specific `fix-ci` recovery declaration, absence of the old aggregate timeout, absence of resource or slot-policy settings, and the `verify-dotnet` `export DOTNET_ROOT=/home/user/.dotnet` prelude before the unchanged `dotnet test` command. Clean-run contract tests will execute the lanes in fresh task shells and prove that the .NET lane sees the export; the representative clean run must complete all six lanes for both built-in profiles. Server tests will cover lane classification including recovery scheduling envelopes, durable projection, gate behavior, stale reports, first-non-passing selection, and recovery preservation. Runner tests will cover command timeout propagation, process-group termination, journal fencing, and report retry. End-to-end workflow tests will cover a timeout after an earlier pass, recovery from that lane, and one-time downstream execution.
...
3. Update `mohist-local.workflow.yaml` and `mohist-github-pr.workflow.yaml` to the six ordered tasks, put the live `export DOTNET_ROOT=/home/user/.dotnet` prelude in each `verify-dotnet` run body before the unchanged .NET command, remove the aggregate `verify` task and enclosing timeout, and update profile and clean-run contract tests. New runs initialized after profile activation become lane-enabled from their stored snapshots.
