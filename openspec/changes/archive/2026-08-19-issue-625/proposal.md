...
- Preserve the existing strict verification requirements and live command mapping, including the project-profile `export DOTNET_ROOT=/home/user/.dotnet` prelude required by the .NET apphosts: `npm ci`; `dotnet test Mohist.sln --nologo -m:1 -p:UseSharedCompilation=false`; Web typecheck; Web `test:run`; Runner typecheck; and Runner tests with `--no-file-parallelism`. The `.NET` lane carries that prelude because each `core/script` task runs in its own shell.
