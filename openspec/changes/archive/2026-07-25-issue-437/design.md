...
### D1. The maintenance seam realizes time and size eviction across ordered callbacks

The time/size/reclaim/arbitrate work is implemented as three `IOtelMaintenanceCallback` implementations registered in DI so `OtelDiagnosticsSampler` invokes them in registration order on every enabled 10s tick and on the immediate first post-start probe: `OtelStorageRecoveryMaintenance` (one-shot startup recovery, D5), then `OtelRetentionMaintenance` (time eviction), then `OtelStorageMaintenance` (size eviction + reclaim + arbitrate). Recovery is registered first so an oversized database is rebuilt on the first tick before retention/storage waste work evicting from a store that is about to be discarded; recovery is a one-shot gate, so on every subsequent tick the effective order is time eviction → size eviction → reclamation → admission arbitration, exactly the per-invocation sequence below. All work executes under the loop's existing `SuppressInstrumentationScope` and `_enabled` gate, so no new self-observation or off-state-execution risk is introduced.
...
Recovery is the first callback in the registration order (D1), so on the first tick it rebuilds before retention/storage eviction touch the oversized store. Immediately after rebuilding, recovery re-probes the fresh empty store and re-arbitrates admission so the write path opens and any stale `storage_budget_exhausted` reason clears; without this, the store would be reported over-budget for up to a tick despite being empty, and the first post-rebuild write would be refused (blocking `storage_data_reset` from clearing, since it needs a committed write). A re-probe failure here is non-fatal: the rebuild itself succeeded, and the next tick's storage callback re-derives the watermark.
...
- **Restart recovery**: a fake probe returning usage above the budget drives the rebuild decision; a fake `IFileSystem` + connection-pool seam asserts the old files are removed and a fresh DB is initialized, with core reachability asserted independently. The rebuild re-arbitration is asserted by driving admission closed, rebuilding, and asserting admission reopens and `storage_budget_exhausted` clears. **Accepted limitation**: the in-memory shared-cache DB is kept alive by a keeper connection and cannot model the real on-disk file-absent window during which a concurrent read-only query would fail, so the "query overlapping the rebuild window fails and surfaces through the existing `StorageRead`/`storage_read_failed` path" behaviour relies on SQLite file locking and is accepted as verified-by-design (the no-corruption half — schema intact and a fresh read-only open succeeds after rebuild — is asserted directly).
