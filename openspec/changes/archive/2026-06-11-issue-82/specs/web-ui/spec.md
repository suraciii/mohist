...
#### Scenario: 成功删除项目
- **WHEN** 用户在 Header 项目下拉菜单点击 "Delete Project"
...
#### Scenario: Idempotent re-invocation on reconnect
- **WHEN** the Web invokes `SetSubscriptionsAsync` twice with the same list (initial connect and reconnect)
- **THEN** the server's per-connection subscription set reflects the list exactly once per call
- **AND** the behaviour SHALL remain correct across many reconnects
...
### Requirement: LiveTaskProvider switch handles both legacy and reverse-DNS names

The `LiveTaskProvider` switch SHALL handle both the legacy snake_case event names (for any unmigrated producer) and the new `com.mohist.*` reverse-DNS event names emitted by the server. The `EventMap` and `AgentDetailEventMap` types SHALL include the reverse-DNS names so the TypeScript switch exhausts the new arms. New switch arms for reverse-DNS names SHALL map to the same cache invalidation and toast logic that the legacy arms already use.

#### Scenario: Stage start reaches the Web under both names
- **WHEN** `com.mohist.workflow.stage.started` arrives at a subscribed connection
...
- **THEN** the Web invalidates `issues` and `agent-activity`
- **AND** the approval toast surfaces as it does for the legacy `approval_requested` arm
...
- **AND** the same toast pattern for pause/error applies
...
### Requirement: ActivityPage Waiting list receives waiting issues

The `ActivityPage` Waiting section SHALL display issues that the server returns in the `waiting` array of `GET /api/projects/{ref}/agent/activity`. The list and section rendering SHALL NOT change structurally; once the server populates the array, the `WaitingCard` components SHALL render for each entry.

#### Scenario: Waiting issues appear in the activity page
- **WHEN** the activity response's `waiting` array contains entries
- **THEN** the `ActivityPage` Waiting section displays one `WaitingCard` per entry
- **AND** the section count matches `waiting.length`

#### Scenario: Empty waiting section
- **WHEN** the activity response's `waiting` array is empty
- **THEN** the `ActivityPage` Waiting section shows the empty state message

### Requirement: 5-second polls remain as a fallback

The 5-second `agent/activity`, `agent/status`, and `workflow/status` polls SHALL NOT be removed. They remain the safety net when SignalR is unavailable, the subscription set is still being negotiated, or any case where the realtime push has not yet caught up.
