...
Set `TranscriptPartTypes.SessionClosed = "session.closed"` (remove the underscore variant entirely). `TranscriptAccumulator.ToTranscriptPartType` then persists `session.closed` for new closure parts. Every reader references the single constant. The literal `"session_closed"` comparison sites — the `|| "session_closed"` branch (`ReadTerminalStateAsync:445`), `SessionTranscriptBuilder.cs:77`, and `AgentSessionSummaryBuilder.cs:79,114` — are replaced by the constant. Sites that already reference the `TranscriptPartTypes.SessionClosed` constant (`TerminalFact.FromTranscript`, `LoadTerminalFactsAsync`, `TranscriptEventSummaryProjector`) need no edit: they pick up the dot token automatically once the constant flips.
