...
- **[Behavior drift during the reverse-DNS outcome decoupling (D3)]** → Mitigation: D2 test-first pins all four outcome arms + fallthrough on the old side-effecting code before any move. The four sinks the caller applies (`invalidateQueries`, `setRebaseConflict`, `dispatchRebaseEvent`, `toast`) are mutually independent — none awaits another, and none reads a value produced by another in the same call (`setRebaseConflict` schedules an async React update that a synchronously-dispatched rebase event cannot observe anyway). The legacy per-arm order was *not* uniform across arms (rebase arms invalidated last; merge arms invalidated first), so the caller applies the declarative result in one canonical order rather than reproducing a per-arm order that was already inconsistent; because the sinks are independent, this preserves observable behavior.
