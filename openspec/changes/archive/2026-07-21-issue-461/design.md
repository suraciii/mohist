...
`RunnerSignalRClient` will retain the `() => OpenCodeRuntime | null` accessor supplied by `RunnerHost` rather than resolving it during handler registration. Follow-up and cancel handler dependencies receive that accessor and call it for each command. A follow-up captures one ready runtime, requires outbox health, resolves its Session target, durably enqueues input, and invokes that captured instance; a missing/unready runtime or unhealthy outbox returns `unavailable` before enqueue. Cancel uses the same invocation-time runtime lookup but does not consult outbox health, so runtime initialization or replacement after client construction is visible and cancellation remains available during local persistence recovery.

The host target resolver is narrowed to binding-only validation: project ownership, `runtime: opencode`, runner ownership, `runtimeSessionId`, and `workDir`. It never calls a runtime or claim-readiness predicate. Admission is owned by each caller: the worker claim gate checks current runtime plus outbox health, follow-up checks its captured runtime plus outbox health, and cancel checks only its captured runtime. This prevents target resolution from observing a second runtime during replacement and prevents outbox failure from disabling cancel.
...
- Resolve the OpenCode runtime while registering SignalR handlers: rejected because `RunnerHost` constructs the client before runtime initialization and rebuilds the runtime later; registration-time resolution permanently captures null or a stale instance.
- Push runtime updates into already-registered handlers through a setter: rejected because retaining the existing accessor is smaller and makes every command use the current host-owned value without another synchronization path.
- Keep runtime/outbox readiness inside the shared target resolver: rejected because it can observe a different runtime than the handler captured and would incorrectly gate cancel on outbox health.
...
Follow-up handling resolves and verifies the current runtime at command invocation, then awaits `enqueueBeforeExecution` for input before invoking the captured `runtime.followup`. If runtime lookup or local write fails, the handler returns `unavailable`, creates no orphan input when runtime is absent, and does not invoke the runtime, allowing command delivery to be retried. Server upload remains detached. Terminal callbacks use `enqueueProducedFact` without invoking the runtime again. A local write failure after runtime execution is observable and marks the outbox unhealthy, but does not replace the runtime result.
...
- [SignalR handlers are registered before OpenCode runtime initialization or replacement] -> Retain the host accessor and resolve a ready runtime per command; never snapshot the runtime during client construction.
- [Shared target resolution accidentally couples cancel to outbox health or a second runtime lookup] -> Keep target resolution binding-only and apply distinct claim/follow-up/cancel admission checks at their callers.
