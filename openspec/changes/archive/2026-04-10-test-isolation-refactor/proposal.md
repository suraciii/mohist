...
4. **Update affected test files** (6 test files)
    - Replace `resetDatabase()` with `new DatabaseManager({ inMemory: true })`
    - Replace `new StateManager()` with `new StateManager(db)`
    - Replace `closeDatabase()` with `db.close()`
    - Ensure proper cleanup in `afterEach` blocks
    - Note: 18 other test files don't touch database singletons and require no changes
...
- 6 test files that use `resetDatabase`/`closeDatabase`/`new StateManager()`

**Files NOT affected (verified):**
- 18 test files that don't import database singletons (event-formatter, resolve-model, config-loader, detector, sse-client, etc.)
...
**Testing impact:**
- 6 test files must be updated before merge
