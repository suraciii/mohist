...
The Dashboard page SHALL expose exactly four zone mount-point slots with stable identities: `Attention`, `Pulse`, `Productivity`, and `Digest`. Each slot SHALL render as an empty placeholder with no zone content. The slots SHALL serve as the composition contract that downstream issues (E/F/G/H) fill; this issue SHALL NOT implement any zone content.

#### Scenario: Four zone slots render as empty placeholders

- **WHEN** the Dashboard page renders
- **THEN** the page SHALL contain four zone mount-point placeholders named `Attention`, `Pulse`, `Productivity`, and `Digest`
- **AND** each placeholder SHALL be empty
...
- **AND** a slot SHALL be addressable by its identity as a mount point for future zone content

#### Scenario: No zone content is implemented

- **WHEN** the Dashboard page renders for any project state
- **THEN** none of the four slots SHALL render Attention, Pulse, Productivity, or Digest zone content
- **AND** zone content SHALL remain the responsibility of downstream issues
