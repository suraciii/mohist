...
- [ID: item-W4]
  ...
  Evidence: Live run on this machine: `du -sh /home/mohist/.mohist` (36 GB) consistently takes > 2 s on a cold cache, so the 2 s per-collector timeout fires and `dataDir.size` is rendered as `<unknown>`. This is a fail-safe (per spec) but the issue body explicitly calls out `< 1 s` as a target and the live environment violates it. Other collectors are well under their budget.
