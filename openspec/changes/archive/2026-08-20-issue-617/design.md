...
- [Risk] A fail-closed Runner rejection makes a Slack turn unavailable when the Server and Runner contracts are out of sync. -> Mitigation: use a bounded dual-read rollout gate. While strict validation is disabled, the upgraded Runner accepts only pre-existing source-less payloads through an explicitly legacy path and never relabels them as non-Slack; it still rejects explicit source/context mismatches. Enable Server emission before strict validation, route version-1 dispatches only to upgraded Runners, drain or reconcile legacy work, then activate strict rejection and monitor diagnostics without logging anchor payloads or secrets.
...
This is a wire-contract and embedded-asset change, not a relational database schema migration. The append-only source discriminator, effective bound-root provenance, and context fields live with existing AgentJob/Session durable state; existing Agent definitions and Session queue/turn semantics remain unchanged. The rollout has an explicit compatibility window so a strict consumer is never placed in front of source-less producers.
...
2. Deploy an upgraded Runner to every Runner eligible for Agent work with `strictExecutionSourceValidation` disabled. Its shared parser accepts a missing source only for a bounded, instrumented legacy path that preserves the pre-change dispatch behavior; it does not normalize an omitted source to `non-slack`. Any explicit source is validated immediately: `slack` still requires a complete context, and `non-slack` still requires no context. The upgraded Runner advertises a capability used for version-1 routing. Old Runners remain eligible only for legacy work and cannot receive explicit version-1 Slack dispatches.
3. After all target Runners support the dual-read contract, deploy Server producer changes and enable `emitExecutionSourceV1` across all Server instances. Every new AgentJob and follow-up, including non-Slack work, now persists and emits an explicit source. New version-1 dispatches are routed only to the upgraded Runner capability; a scheduler must hold or retry rather than send an explicit Slack context to an old Runner that would ignore it. During the rolling deployment, source-less payloads from an old Server remain serviceable only through the bounded legacy path.
4. Reconcile and drain pre-existing source-less work before enabling strict validation. For a recovered record, the Server materializes the source only from trusted durable origin data. It constructs the complete Slack context for records with Slack origin and rejects records with Slack evidence but missing context, bound root, or representative provenance; it never upgrades a malformed Slack record to ordinary work. Legacy non-Slack work drains through the compatibility path without changing its prompt or envelope.
5. Enable `strictExecutionSourceValidation` on the upgraded Runners only after all producers have emitted explicit markers for a full drain window, no pending source-less dispatch remains, and capability routing excludes old Runners. From this point, an omitted or unknown source is invalid at both Runner boundaries, `slack` without context and `non-slack` with context fail closed, and no unmarked dispatch is treated as non-Slack. Keep the feature gate and rejection metrics until the drain and replay checks are complete.
6. Verify mixed-version behavior explicitly: an old Server to an upgraded Runner is accepted only by the bounded legacy path; a new Server to an old Runner is held or rejected by capability routing; an explicit new source/context pair reaches the upgraded Runner; and strict mode rejects source-less payloads. Also verify initial/follow-up parity, replay stability, DM and batched anchors, source/context rejection, non-Slack prompt preservation, and the absence of delivery or reply-authorship changes. Monitor validation failures and Runner availability without recording context contents.

Rollback is ordered through the same gate. First pause strict validation on upgraded Runners, stop assigning new version-1 work to incompatible Runners, and let explicit-context work drain on upgraded Runners. Only after no explicit version-1 dispatch remains may the Server stop emitting the new fields and return to the legacy path. Do not roll back only the Runner while strict validation is active or while the Server emits version-1 contexts. Reverting the embedded asset or contract without a versioned replacement is not a valid rollback because it could change the digest of already-created payloads.
