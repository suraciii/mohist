...
- `[Risk] Process fails after Session open/LaunchReady but before Runner acceptance` -> Keep the recovery reminder while Pending and retry dispatch until `Running` with Runner acceptance is durably saved.
...
2. Add the ownership-validating routing launch-context resolver and AgentJob-owned prepared-launch recovery fence, then switch routed launch to register reminder -> persist plan -> idempotent Session open -> persist launch-ready -> retry dispatch until durable Runner acceptance, or enter terminal transition. Keep the runner workspace requirement unchanged.
...
Rollback requires the server to report no outstanding prepared plans awaiting Session open, no `LaunchReady`/pending-dispatch plans awaiting Runner acceptance, no `PendingSessionClose` records, and no `agent-job-recovery` reminders before the old binary is restored. Executable recovery clears only after `Running` with Runner acceptance is durable; terminal recovery clears only after durable Session-close acknowledgement. If any obligation cannot drain, deploy a compatibility drain build rather than rolling directly to code that cannot advance it. Once drained, rollback is code-only: additive state fields and terminal facts remain readable, and removing the issue-feed projection and summary field requires no data cleanup. The runner requires no deployment or rollback change.
