...
`mo workflow delete` SHALL refuse to delete a custom Profile that is referenced by the Project default, by any Issue in that Project including a terminal Issue, or by any active WorkflowRun. The refusal MUST identify each blocking reference relationship. An unreferenced custom Profile or one referenced only by terminal WorkflowRuns SHALL be deletable. `WorkflowProfileReferenceCoordinator` SHALL serialize deletion with Project-default writes and WorkflowRun bindings. `IssueRepositoryCoordinatorGrain` SHALL continue to serialize Issue creation, repository lifecycle commands, and Issue explicit selection; Issue creation SHALL commit its repository binding and Profile selection together in its one Issue transaction. For custom Profiles, Project-default, Issue-selection, and active Run-binding persistence rows SHALL carry a nullable custom-Profile key backing column with a restrictive `(ProjectId, ProfileId)` foreign key to the custom Profile record. It SHALL be populated only for custom IDs and remain null for built-ins, while the public and domain reference remains the single Profile ID. When a WorkflowRun becomes terminal, its own transaction SHALL clear only its backing key while retaining its public Profile ID and historical state. This final transactional constraint SHALL make a concurrent deletion and Issue selection resolve as either a committed reference that blocks deletion or a retryable `workflow-profile-not-found` conflict, never a dangling reference. Built-in Profiles need no foreign key because they are read-only and cannot be deleted.
...
#### Scenario: Delete a Profile referenced only by a terminal Run
- **WHEN** a custom Profile is referenced only by a terminal WorkflowRun
- **THEN** the Profile SHALL be removed, while the Run retains its public Profile ID and all existing history
