...
- **[`StripJsoncComments` is `public`; external callers may exist]** → verified: the only references in `src/` and `tests/` are the two `ConfigService` call sites (D2). The class is internal server infrastructure, not a public API.
