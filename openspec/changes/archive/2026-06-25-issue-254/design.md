...
| 类型 | 内容 | 依赖 |
|---|---|---|
| `InfoCollector`（保留瘦身） | `CollectAsync`/`CollectVerboseAsync`、全部 `Get*Async` 采集、`CheckServerConnectivityAsync`、`AttachConnectivity`（在 `CollectAsync` L138 组装 `InfoResult` 时把 connectivity 挂到 runner 记录上，属采集期数据装配，非渲染）、`InspectSourceAsync`、`ComputeDiskUsageAsync`、`IsSystemdAvailable`、`TryGetBinaryPath`、路径/进程启发式（`ResolveSourcePath`/`ExtractProjectPath`/`TokenizeExecStart` 等，作私有静态保留）、`WithTimeout`/`SafeAsync` | `IFileSystem`、`ICommandExecutor`、`IEnvironmentVariableProvider`、`MohistCliApi`、`SkillAssetService` |
| `InfoRenderer.cs`（新增） | `RenderDefault`/`RenderVerbose`/`RenderJson`、全部 `Build*Json`、`WriteIndentedList`、全部 `Build*Line`、`CompactJsonOptions`、`BuildSkillLines`/`BuildEnvVarLines`/`BuildDiskCategoryLines`/`BuildOriginUrl` | 仅 `TextWriter` + `InfoResult`/`InfoVerbose` |
