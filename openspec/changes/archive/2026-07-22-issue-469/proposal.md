...
- **Web UI**: no current consumer of `/otel/api/query` exists today, so no UI change is required by this change; any future consumer must surface truncation when the indicator is present (placement decided per-consumer).
