...
- Send Pi input, assistant/reasoning text, tool activity, model observations, provider retries, compaction, token usage, cost, and a terminal turn fact through the existing physical-binding-validated AgentSession runtime-event route. The Runner validates explicit AgentSession acceptance before success; this change adds no Action stream, outbox, cursor, inventory, checkpoint, or restart replay protocol.
...
- **Server** (`packages/server`): accepts `pi` on Workflow Session open/attach, preserves guarded physical-binding and lineage rules, widens the two persisted physical-ID projections for absolute Pi paths, exposes runtime-event acceptance already returned by AgentSession, recognizes `mohist/pi` as UserFacing, and carries Pi cache-write/provider-retry facts through existing Session contracts.
