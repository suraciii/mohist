...
- **Web** (`packages/web/src`):
  ...
  - Sibling sessions: `widgets/coder-session/ui/SiblingSessionsSidebar` (within `SessionDetailShell`) — repositioned as a stable reference region.
...
- **Server / runner / CLI**: none. No API, DTO, query, or event-recording changes; both views consume existing projections and endpoints. The non-goal "do not change how events or session transcripts are recorded" is respected.
- **Dependencies**: none added.
- **Tests** (`packages/web`): `pages/activity/ui/ActivityPage.test.tsx` updated for event-type distinction and priority surfacing; `pages/session/ui/SessionDetailShell.*.test.tsx` updated for the new layout hierarchy; new spec tests cover event-type identification, needs-attention/failure prominence, stable session layout regions, navigation orientation, and light/dark status consistency. Existing `data-testid` anchors preserved where still valid.
- **Risk (medium)**: this changes diagnostic views that users rely on during failures and quality-gate decisions. Mitigated by consuming existing data projections unchanged, reusing the shared status baseline from issue 398, and asserting every event-type and layout path against the new evidence-view contract.
