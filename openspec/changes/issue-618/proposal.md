...
- Issue a short-lived Manager capability credential for each execution. Bind it to the immutable Slack origin, authenticated actor, workspace enrollment, Session, and expiry; inject it only into the execution environment and reissue it for every turn or recovered execution.
- Expose a small allowlist of Manager operations through the existing `mo` command capability surface. Commands reuse the existing application services and CLI/Web semantics for status, Agent and Connection inspection, routine lifecycle changes, owner operations, and diagnostics.
...
- **Runner and command execution:** the Runner execution environment and Manager command bridge will need to expose only the approved `mo` capability and prevent credentials or unrestricted management access from reaching model-visible context.
