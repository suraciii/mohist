...
- Issue a new short-lived capability for each initial, follow-up, recovered, or replacement execution, invalidate every prior grant at each Server deployment/process restart and expiry boundary, and reauthorize every invocation against current state.
...
A `ManagerExecutionCapabilityIssuer` will mint two cryptographically random opaque bearer values for every execution: a management lease and a separate reply lease. Each non-secret lease record contains the capability id, scope, immutable Slack origin, actor, Enrollment, Session, dispatch or execution identity, issue time, expiry, deployment epoch, and active state. The Server stores only hashes of the bearer values and their binding metadata in a short-lived, revocable lease store; it never persists either plaintext value. All Server instances that can serve Manager work use the same lease store and shared deployment epoch. Before a Server process starts accepting Manager polls, it advances that epoch atomically; validators reject every lease from an older epoch. A single-Server deployment uses the same epoch boundary, and loss or unavailability of its lease store invalidates all leases and blocks Manager work. Graceful shutdown revokes active leases before draining; an ungraceful restart invalidates them through the new epoch before any work is served. Completion, cancellation, replacement, recovery, expiry, or epoch change invalidates both leases. A new follow-up, expiry recovery, or recovered execution always receives new values.

The leases are transported in a one-shot `managerExecutionGrant` field on the in-memory Runner poll response, deliberately separate from `AgentJobInput`, `WorkDispatch.with`, the serialized active-work ledger, the Slack execution context, and all durable Session or job state. The Server creates the grant only after loading the durable dispatch and before serializing the HTTP response; it contains the two plaintext bearers, the execution identity, expiry, and current deployment epoch. `WorkDispatchResponse`/poll response is the only wire carrier, and request/response logging must redact and never persist that field. The Runner keeps the grant only in an in-memory `BoundManagerExecution` wrapper keyed by the work key; it never copies the grant into `DispatchWorkItem`, the result journal, `inFlight`, `awaitingAck`, recovery receipts, or terminal reports. A re-poll or replacement execution gets a new grant, and a recovered runtime does not reuse the old one. Every Runner heartbeat/poll carries the epoch it is executing; an epoch change causes the Runner to revoke its local Manager wrappers and destroy their brokers before it claims replacement work. The Server-side lease check remains authoritative during the gap before that notification, so an old broker cannot perform a valid invocation.

### 5. Give Pi and OpenCode the same real command boundary

For each Manager execution the Runner creates a private, one-shot CLI broker and a per-execution `mo` launcher placed in a private directory at the front of `PATH`. On Linux the broker is a mode-0600 Unix-domain socket; on Windows it is a user/Runner-ACL named pipe. The launcher has only a broker handle, not a bearer. When and only when the command resolves to the Manager `mo` launcher, it obtains the needed bearer from the broker and `exec`s the real `mo` child with the bearer in a private process environment/header. The bearer is never in the parent model shell, `printenv`, generic command arguments, prompt, transcript, or output. A broker locator or handle may be inherited by the isolated runtime as a non-secret implementation detail, but the broker protocol never returns a raw bearer to a generic command and rejects requests without the launcher boundary.

At Runner dispatch selection, `AgentJobExecutor`/the Runner host recognizes the Manager execution contract and installs the scoped boundary before selecting the runtime. Pi Manager turns use the existing Runner command-executor resources, so the actual `mo` child spawned by Pi receives the private environment only for that invocation. OpenCode Manager turns take a Manager-specific path and never call the shared `OpenCodeRuntime.runTurn` against its shared `state.server`/client. The Runner starts one isolated OpenCode server process and client for each Manager execution through a Manager-specific `OpencodeServerFactory`/process adapter that accepts the scoped process environment and private `PATH`; the server's model-shell children therefore inherit the execution boundary that the current SDK-managed shared server cannot receive. The isolated server attaches to the durable logical Session/runtime binding for that execution, is never shared with another Manager or ordinary Agent execution, and is torn down after the turn. The existing shared OpenCode runtime remains unchanged for ordinary Agent work. If the pinned SDK cannot start an isolated server with an explicit environment, the process adapter must own that spawn/lifecycle boundary; capability gating rejects Manager OpenCode dispatch rather than falling back to the shared server or putting a bearer in its base environment. Manager execution admission likewise rejects any Pi/OpenCode executor that cannot install this boundary.

Different Manager executions have distinct brokers, launchers, OpenCode processes, and environment scopes. Manager turns for one Session are serialized by the ordinary Session dispatch contract; different Sessions may run concurrently without sharing any of those objects. The management bearer is accepted only by the allowlisted management route and the reply bearer only by `POST /api/slack-manager/reply`; neither can be used as an operator or ordinary Connection credential.

The broker, launcher, and isolated OpenCode process tree are destroyed in `finally` paths when the execution ends, is cancelled, expires, or an epoch change is observed. The Runner closes them immediately on an epoch-change control event and on the next heartbeat/poll if the event was missed. The Server revokes both lease hashes on completion, cancellation, replacement, recovery, expiry, or epoch change; an expiry sweeper removes expired runtime records, and lease-store shutdown revokes remaining active records. A lease-store, process-boundary, broker, or redactor failure fails closed. Server validation checks the requested scope, lease, deployment epoch, expiry, execution and full-origin binding, current actor and Enrollment state, and current target authorization before calling an application service. It never broadens the target, retries with an operator credential, or falls back to a less restrictive authorization path. Runner and Server redactors register both bearers before execution and redact them from line capture, aggregate command results, exceptions, runtime events, task logs, work reports, audit data, outbox payloads, and terminal events.
...
### 6. Keep existing application services authoritative for CLI results
...
### 7. Drive Manager liveness through the shared durable Slack projection
...
### 8. Remove the retired protocol as a breaking change
...
- [A Server or Runner restarts while a capability is live] -> Advance the shared deployment epoch before serving Manager polls, revoke active lease hashes, notify connected Runners, and have every Runner discard its old grant and destroy its broker/process boundary. Crash recovery observes the same epoch mismatch on the next heartbeat/poll. Durable recovery creates a fresh execution and lease from the immutable Slack origin; it never reuses the old grant.
...
- [Capability validation is inconsistent across Server instances] -> Use the shared short-lived lease store and deployment epoch for every Server topology, including a single-Server installation. Fail closed when the issuer, epoch, or store cannot validate either scoped lease; never fall back to a durable plaintext token.
...
1. Add the non-durable `managerExecutionGrant` poll-response field, the two scoped lease validators, the shared deployment epoch, the Manager CLI mode, and the dedicated Manager reply route additively to the Server, Runner, and CLI contracts. Add Runner capability/version gating so a Manager execution is dispatched only to a Runner that supports the grant, epoch invalidation, private broker/`mo` launcher, isolated OpenCode process, Pi command executor boundary, and output redaction. Existing non-Manager dispatches continue to use their current contracts.
2. Implement shared Session routing and liveness convergence while retaining the old Manager parser behind no new call sites. Add focused Server, Runner, CLI, and `mohist-slack` integration tests for replay, replacement Session, credential boundaries, Server restart epochs, Runner crash recovery, expiry during an active turn, fresh credentials after each transition, current-state authorization, reply ownership, loop prevention, and all terminal outcomes.
...
- What capability TTL and clock-skew allowance are appropriate for the longest supported Manager CLI operation? The lifecycle is fixed: expiry closes the current execution and starts an explicit non-replaying recovery/new-turn transition with fresh credentials; it is never renewed in place.
