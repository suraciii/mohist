...
Workflow creation uses the same snapshot boundary. `IssueGrain` passes the issue's `LineageVersion` with the initial workflow metadata; `WorkflowRunStore` conditionally locks that version in the transaction that appends `workflow.run.started`. A committed link/unlink either precedes that transaction (the version check rejects the stale input and Issue reloads it) or follows it (the initial event is correctly before the affiliation change). A persisted `DispatchActivated = false` gate survives stage materialization: initialization can fill tasks and checks but cannot advance the workflow state or make it assignable. After Issue has persisted its workflow reference, `ActivateAsync` verifies that binding, synchronizes the workflow scalar snapshot, then derives the dispatchable, approval, or terminal state. Runner poll performs the same activation recovery for a bound gated run after a restart. This is a creation handoff, not a workflow-store query for stamping: after the transaction begins, the envelope still derives only from the metadata that its own run owns.
