...
- Approval feedback completed by a `core/process` task continues to derive its resolution summary from the command text, now at `output.stdout`; arbitrary output objects are not serialized into human-facing summary text, and output without an explicit text adapter yields no summary.
...
- `action-output`: The end-to-end contract for Action success output: object-or-null production, reporting, persistence, task-detail presentation, `setVars` projection, `tasks.<id>.outputs.*` references, recovery matching, and explicit text adaptation for approval-feedback summaries; includes the `{ stdout, exitCode }` contract for `core/process` and explicit task failure for invalid output shapes or missing projected fields.
