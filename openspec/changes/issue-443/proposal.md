...
- `setVars`, `${{ tasks.<id>.outputs.* }}`, declared output capture, and recovery `when: output.*` read the same structured output object. A missing `setVars` source path continues to fail the task and names the missing path.
