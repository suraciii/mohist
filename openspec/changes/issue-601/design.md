...
- `committed-clean`: no recoverable hold is created. The task transitions from `Running` to `Completed`; the existing stage/run advancement path then marks the containing stage `Completed` and the run `Completed` when this was the final stage, or leaves the stage/run in their normal `Running`/`Ready` dispatch state for subsequent work. Artifact binding/events remain idempotent and ordered before task completion.
