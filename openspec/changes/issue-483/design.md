...
- [A Runner upgrades after the initial deployment] -> Retain the version-2-only, idempotent `migration.followup.resolve` command indefinitely; it resolves from durable transcript evidence and never accepts a legacy event name.
...
4. Enable new Runner registration and Session traffic only after REST registration/heartbeat and the RunnerHub query all report version 2. New producers and consumers use only `turn.*` and `followup.*`; legacy names are rejected. A later upgraded Runner may still submit its version-2-only `migration.followup.resolve` records until its converted outbox is acknowledged.
