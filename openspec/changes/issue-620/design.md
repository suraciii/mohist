...
Introduce a dedicated `SlackRetryOperations` persistence boundary rather than overloading `SlackProviderInboxRows`. The inbox records receipt of a provider interaction; it does not retain the operation outcome or provide recovery after a dispatch response is lost.

A Retry operation is keyed by a stable hash of the signed action value (or its nonce plus canonical payload) and stores the failed target identity, a retry-specific dispatch key, state, outcome, and resulting session/input/turn identifiers when known. The unique action key and a conditional state transition provide the concurrency fence:

1. Verify the action and re-read the authoritative session/turn and original Slack provenance.
2. Reject expired, unauthorized, wrong-context, non-failed, superseded, or unavailable targets before creating work.
...
4. Commit the operation identity and retry dispatch key before invoking the session dispatcher.
5. Dispatch with that stable key. If the caller dies after the commit, a later replay/recovery call repeats the same idempotent dispatch key rather than creating another attempt.
6. Persist the accepted/dispatched outcome and enqueue one durable update for the original Slack status target.

The fresh attempt is a new input/turn in the existing AgentSession so the failed turn remains immutable and the retry preserves session, Connection, actor, message, conversation, thread, and reply-routing context. The retry source is copied from authoritative session/input state, including normalized prompt, provenance, and accepted attachments; action-submitted text is never used. The same existing follow-up dispatcher is used for a root failure and a threaded failure, with a retry-specific idempotency key. If the current session cannot accept a new attempt, the operation returns `unavailable` without dispatch.
...
Add Server tests for signing canonicalization, credential omission, expiry, actor/context authorization, terminal-state revalidation, retry operation concurrency/replay, fresh input/turn creation, single-winner prompt selection, candidate invalidation, original-context routing, and durable presentation updates. Add migration/store tests for prompt and operation uniqueness and recovery after a write-before-dispatch failure.
...
- [A process crash after an operation commit but before runtime dispatch can leave work apparently accepted] -> Store a stable dispatch key before dispatch, make the session/launch path idempotent on that key, and allow a later replay or recovery attempt to resume the same operation.
...
1. Add an additive database migration for `SlackRetryOperations` and extend `SlackAmbiguousPrompts` with source, actor, action, expiry, candidate descriptor, selection, and dispatch/result fields. Keep the existing unique prompt index and backfill existing rows as legacy text-only prompts with no selectable action.
2. Deploy the Server changes that understand both legacy prompt rows and the new action records. Existing Stop interactions continue through the unchanged behavior; old ambiguous prompts remain readable and are not retrofitted with unverifiable actions.
3. Deploy the terminal event, Server action, prompt-routing, and outbox changes. New failed results and new ambiguous messages can then emit signed controls. Existing adapter versions can carry the already-supported envelope and blocks, but the adapter package should be released with the corresponding contract tests.
4. Monitor rejected/stale action outcomes, operation rows stuck in dispatch-pending state, outbox uncertainty, and selection winner conflicts. Add cleanup for expired operation/prompt rows only after the Slack redelivery and delivery-reconciliation retention window.
...
- What exact failure categories are retryable? The implementation needs one authoritative classification rather than treating every `failed` status as retryable.
