...
`SlackInteractionRoutes` remains the authentication and delivery boundary. After `SlackAdapterLeaseService.ValidateRuntimeLeaseAsync` succeeds, it passes the validated operator, lease, and adapter identity as a `SlackLeaseContext`, including the lease-bound verified-Bot-token resolver, to the Server action handler. The route validates the normalized `block_actions` envelope, delegates the action-specific decision to the Server, and enqueues the returned text and blocks with a stable result dispatch reference. It will no longer derive a separate result reference from every raw action value when the service can return the operation/prompt reference. This prevents concurrent selections from creating unrelated updates to the same prompt.
...
On selection, the Server verifies the signature and prompt delivery context, loads the row, compares the signed candidate fingerprint, and rechecks workspace, prompt-owner Connection, signed direct-message mode, and expiry. It then calls `SlackConnectionAccessDecider.EvaluateAsync` for the prompt-owner Connection with the bound actor and the validated interaction `SlackLeaseContext`; this is the same current policy, allowlist, live-member, and channel-membership check used for a new invocation. It also rechecks selected Connection eligibility. A transaction conditionally records the first winner and stable selection operation before dispatch. A losing concurrent candidate observes the winner and returns `already_applied` or `stale` without dispatch.
...
Add Server tests for signing canonicalization, credential omission, expiry, bound-actor and current-Connection-access authorization, lease-backed live-member checks, terminal-state revalidation, retry operation concurrency/replay, fresh root Session and threaded follow-up identity creation, the threaded case with an unrelated queued follow-up, operation-targeted follow-up dispatch, single-winner prompt selection, candidate invalidation, original-context routing, and durable presentation updates. Add migration/store tests for prompt and operation uniqueness, the retryability category matrix, conditional claims, operation recovery after a write-before-dispatch failure, and selection recovery after the winner is committed but before dispatch. Inject failures at each boundary: after the operation commit, after launch/follow-up idempotent admission, before the operation outcome update, and before the interaction response. Verify that a process restart or interaction redelivery re-enters the pending operation and produces at most one attempt.
...
The interaction route follows this order: validate the adapter lease and normalized envelope, verify the signed action and bound actor/context, evaluate current Connection access through `SlackConnectionAccessDecider` using the validated `SlackLeaseContext`, atomically claim or load the action operation, commit its pending state before runtime dispatch, invoke the operation-targeted resume method, persist the outcome, and enqueue the stable result presentation. A redelivery that finds an existing receipt must still call resume when current access permits it: it returns the recorded outcome or advances a pending operation, and `SlackInteractionRoutes` always asks the outbox to enqueue the returned presentation reference. Recovery of a committed pending operation is independent of a later actor or lease change. Outbox uniqueness collapses duplicate updates. Original message ingress keeps its existing order of provider-inbox acceptance before launch/prompt work; button clicks use the action operation as their separate receipt and must never be stopped by the source-message inbox's duplicate result.
