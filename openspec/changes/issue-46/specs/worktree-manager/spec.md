...
WorktreeManager SHALL 为每个 Issue 创建独立的 git worktree 作为隔离工作区，基于该 issue 的项目 repository reference 解析出的当前 `baseBranch` 对应的远程分支创建。Repository path, remote, and base branch SHALL be resolved from current project repository configuration rather than from issue-owned repository snapshot fields.
...
#### Scenario: Issue repository reference resolves to updated project repository path
- **WHEN** the project repository path changes after the issue was created
- **THEN** WorktreeManager SHALL use the resolved current project repository path for that issue repository reference
- **AND** it SHALL NOT use a stale path copied onto the issue

#### Scenario: Missing issue repository reference blocks worktree creation clearly
- **WHEN** the issue repository reference cannot be resolved in the current project configuration
- **THEN** WorktreeManager SHALL return a repository configuration error for that issue
- **AND** it SHALL NOT silently use another repository or implicit base branch
