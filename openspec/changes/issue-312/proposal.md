...
- **Server 测试**（`packages/server/tests/Mohist.Server.Tests/Specs/Workflow/Grain/`）：现有 34 个 spec 文件作为行为守护，全部通过、不改；行为等价性由集成 spec 兜底。
