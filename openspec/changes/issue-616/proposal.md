...
- Preserve managed-Bot author identity metadata through the adapter and Manager transport so the Server can distinguish Mohist-managed Bots from unrelated third-party Bots. Normalize `event.app_id` with `bot_profile.app_id` fallback, retain Bot IDs and an optional Bot-user ID, reject conflicting identity fields for managed matching, and never treat the receiving Socket app identity as the author identity.
...
- Add regression coverage for adapter normalization and acknowledgement, Manager and Agent author-payload fixtures, author-identity propagation, the adapter-to-Server Manager envelope, both Server ingress paths, Agent App lifecycle/binding transitions, third-party Bot compatibility, and the no-side-effects guarantee.
