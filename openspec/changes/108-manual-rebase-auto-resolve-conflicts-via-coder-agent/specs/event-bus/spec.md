...
SSE 端点 SHALL 推送所有 agent 生命周期事件，包括 `agent_paused`。`ALL_EVENT_TYPES` 数组 SHALL 包含 `agent_started`、`agent_completed`、`agent_paused`、`agent_error`、`approval_requested`、`question_asked`、`question_answered`、`rebase_conflict`。`rebase_conflict` 事件 SHALL 支持 `status` 字段以区分冲突检测和解决失败两种语义。
...
#### Scenario: Rebase 冲突检测 — resolving 状态
- **WHEN** manual rebase 检测到冲突并启动 agent 解决
- **THEN** SSE 推送 `event: rebase_conflict`，payload 包含 `{ issueId, projectId, issueNumber, conflicts, status: "resolving" }`

#### Scenario: Rebase 冲突解决失败 — error 事件
- **WHEN** agent 冲突解决失败
- **THEN** SSE 推送 `event: rebase_conflict`，payload 包含 `{ issueId, projectId, issueNumber, status: "failed", error: "..." }`
