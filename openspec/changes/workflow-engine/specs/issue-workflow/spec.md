...
**Reason**: GitHub PR 流程替换为本地 git worktree + git merge 流程

**Migration**: 使用 `每个 Issue 对应一个工作分支`（ADDED）替代

### Requirement: PR 合并后标记 Issue 完成

**Reason**: PR 合并替换为 CLI 本地 git merge

**Migration**: 使用 `合并后标记 Issue 完成`（ADDED）替代
