...
- Update the Skill asset identity/hash and contract coverage so these rules are versioned, embedded, verifiable at dispatch time, and kept in one-to-one parity with every rule in the documented Slack collaboration-rules section.
...
- **Server Agent assets and contracts:** Update the embedded `mohist-slack-collaboration` Skill and its version/content-hash contract tests. The contract tests include an explicit rule-by-rule check against all six rules in `docs/slack.md`, including audience-appropriate mentions and Web-timeline-only fine-grained progress. The existing Server-owned `SlackExecutionContext` remains the source of the reply anchor and dispatch-scoped Slack facts.
