...
- API 错误响应增加 `code` 字段，区分 `LLM_NOT_CONFIGURED`、`LLM_CONFIG_INVALID`、`LLM_AUTH_FAILED`、`LLM_RATE_LIMITED`、`LLM_PROVIDER_ERROR` 等错误类型
- Explore 页面在 LLM 未配置时显示引导卡片（含配置文件路径、配置示例、刷新按钮），替代隐藏在 console 中的错误信息
...
- `packages/cli/web/src/components/ExplorePage.tsx` — LLM 状态检查 + 引导 UI（含刷新按钮）
- `packages/cli/web/src/hooks/useExploreStream.ts` — 解析 code 字段，展示分类错误提示
- `packages/cli/web/src/hooks/useQuery` 或类似 — 查询 status API
