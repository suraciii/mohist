...
WorktreeManager SHALL 为每个 Issue 创建独立的 git worktree 作为隔离工作区，基于项目的 `baseBranch` 对应的远程分支创建。在创建前 SHALL 清理 stale remote refs，创建后 SHALL 验证新分支与 base branch 有共同祖先。

#### Scenario: 创建 worktree

- **WHEN** 用户执行 `mo issue start <number>`
- **THEN** 系统执行智能 fetch 并 prune stale refs（`git fetch origin --prune`）
...
- **AND** 验证新分支与 `origin/<baseBranch>` 有共同祖先（`git merge-base` 成功）
- **AND** 返回 worktree 路径供 Agent 使用
- **AND** `{projectName}` 经过 slug 处理（小写、空格转 `-`、去除特殊字符）

#### Scenario: worktree 创建后无 merge base 时自动清理并报错

- **WHEN** 系统创建 worktree 后发现新分支与 `origin/<baseBranch>` 无共同祖先
- **THEN** 系统自动删除刚创建的 worktree 和分支
- **AND** 返回错误："Branch 'mo/issue-{N}' has no common ancestor with '{baseBranch}'. Check project base branch configuration."
