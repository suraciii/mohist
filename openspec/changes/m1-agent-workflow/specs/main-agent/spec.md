...
Main Agent SHALL 作为 LLM agent 编排者，读取 workflow.yaml 配置，按阶段调用 spawn_coder，传递上下文，推进工作流。
...
Main Agent SHALL 将 workflow.yaml 中的 `approval: true` 仅作为标记，执行完该阶段后添加 comment 并主动结束，不自动进入下一阶段。用户通过 `mo issue approve` 或重新 `mo issue start` 手动推进。

#### Scenario: approval stage 执行
- **WHEN** Main Agent 执行到 `approval: true` 的 plan 阶段
- **THEN** plan 完成后，Main Agent 调用 `add_comment("Plan 完成，等待审批")`，然后正常结束（不报错）

#### Scenario: 用户手动推进
- **WHEN** 用户查看 plan 结果后执行 `mo issue start 1` 或 `mo issue approve`
- **THEN** issue 进入 build 阶段，Main Agent 再次启动继续执行
