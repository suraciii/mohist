...
2. **Dark-theme breakage** — `BranchBar.tsx` and the per-task execution/progress log (`TaskLogPanel.tsx`) use literal light-theme-tinted palette utilities (`amber/blue/red/green/gray/purple/slate-*` fills) that ignore the dark theme. The original "task progress panel" was refactored into the stage-aware task list by issue-454; `StageBar.tsx`, `StepList`/`TaskItem.tsx`, and `CheckItem.tsx` are already tokenized, leaving `TaskLogPanel.tsx` as the remaining task surface with literal-palette chrome. (`WorkflowRunStatusPill.tsx` also carries literal palette but is not rendered on the page — only its definition, the barrel re-export, and its own test reference it — so it is out of scope.) The design system already defines semantic token families in `app/styles/index.css`: `success / warning / info / danger`, each with `-subtle / -border / -foreground`, plus `muted / muted-foreground / border / background`.
...
### D3 — Map literal palette to existing semantic token families (listed stateful blocks only)
No new tokens. In-scope blocks are the branch bar and the per-task execution/progress log chrome — the stateful container blocks named in the symptom. Per-state mapping for `BranchBar.tsx`:
...
- The `enabledClassName` / `reasonClassName` strings passed into `RebaseAction` become these token strings.

For `TaskLogPanel.tsx`, tokenize only the **light-surface chrome** that breaks in dark theme: the outer panel (`bg-white border-slate-200`), the search input (`bg-white border-slate-300 text-slate-900 focus:border-sky-500`), the download button (`bg-white/border-slate-300/text-slate-700`), the source-filter chips (`bg-white/bg-slate-100 border-slate-* text-slate-*`), and the amber truncation badge (`bg-amber-100 text-amber-800`) — map these to `bg-background`/`bg-muted`/`border-border`/`text-muted-foreground`/`text-foreground` and the amber badge to the `warning` family. **Explicitly exclude the deliberate dark log-console surface** (`bg-slate-900 text-slate-100` and its `text-sky-300`/`text-violet-200`/`text-red-300`/`text-slate-*` line content) — that palette is intentional for the terminal surface and tokenizing it would break legibility.

**Scope reconciliation with AC #2.** AC #2 reads "No literal palette utilities remain on the issue detail page; every listed block renders correctly in dark theme." The second clause scopes the first: the requirement applies to the *listed stateful blocks* (branch bar + task execution/progress log). Accordingly:
- `WorkflowRunStatusPill.tsx` is dropped as a target — it is not rendered on the page (no production usage).
- Other on-page literal palette is intentionally out of scope and tracked as follow-up, because it is either theme-legible (mid-saturation status-icon colors in `WorkflowSessionsPanel.tsx`) or belongs to adjacent surfaces (`PrDeliveryIndicator.tsx`, `ReviewSummary.tsx`, `ReviewReportModal.tsx`, `ArtifactContentViewer.tsx`, `ArtifactTextContent.tsx`, `CompositeParentOverview.tsx`, the composite-parent violet badge).
- Alternative considered: sweep every literal-palette utility on the page to satisfy a strict reading of AC #2. Rejected — it enlarges the change into approval/artifact/composite surfaces beyond the symptom and conflicts with the issue's "localized correction" Fix Shape.
...
- Sticky rail: in `IssueDetailPage.tsx:553`, on the reference-rail column add desktop-only sticky sizing: `lg:sticky lg:top-6 lg:self-start lg:max-h-[calc(100vh-3rem)] lg:overflow-y-auto`. The outer `overflow-y-auto` container (`:287`) is the scroll ancestor, so `lg:sticky` keeps the rail in view; `max-h` + internal overflow prevents an over-tall rail from being unreadable. (The rail cards are Details, Workflow Profile, Base Drift Detected, Convergence, Configuration, Start Prerequisites, and Readiness — there is no separate "Actions" card; the sticky rule applies to the whole rail column regardless of which cards are present.) Narrow viewport is untouched (it stacks/collapses as today).
...
- [Existing tests assert literal palette classes] → `BranchBar.test.tsx` asserts `border-amber-300 text-amber-800 hover:bg-amber-50`. These encode the old behavior and **must** be updated to the new token classes as part of the change — expected, not a regression.
- [TaskLogPanel dark console must not be touched] → The log line area (`bg-slate-900` + `text-slate-100/sky-300/violet-200/red-300`) is a deliberate terminal surface. Mitigate by tokenizing only the light-surface chrome and the amber badge, leaving the console interior unchanged; add/keep a test asserting the console surface is preserved.
- [AC #2 scope ambiguity] → The strict wording "no literal palette utilities remain on the issue detail page" is reconciled to the listed stateful blocks (see D3). Other on-page literal palette is documented out of scope; if a strict sweep is later required, it becomes a follow-up rather than expanding this issue.
