...
- **Compact / Reset 禁用原因悬浮说明**：当前 `SessionRecoveryActions` 禁用态只用 `title="Unavailable while session is active"` 作为原生浏览器 tooltip，鼠标停留短暂、信息粗略。改为：禁用态按钮暴露明确的 `data-disabled-reason` 标记，由父级 `Tooltip`-类组件渲染结构化悬浮说明（标题 + 详细原因），覆盖三类场景：session active（解释为什么现在不能用）/ 未到达能力前置条件（解释缺什么）/ 网络/状态未知（解释为何暂时禁用）。
...
- `session-action-weight`: header 内 Cancel 按钮的视觉与位置（次要 variant + 收进操作槽）+ `SessionRecoveryActions` 禁用态 Compact / Reset 的结构化悬浮说明（`data-disabled-reason` + 父级 tooltip 渲染）。覆盖"危险操作不再以最高视觉权重呈现"和"禁用操作有明确的禁用原因说明"两条 acceptance 条件。
...
- **Web（`packages/web/src/widgets/coder-session/ui/SessionRecoveryActions.tsx`）** —— 禁用态说明：
  - 暴露 `data-disabled-reason="active" | "prereq" | "unknown"` 标记替代（或补充）当前 `title` 属性；由 `SessionDetailShell` 父级包一层 Tooltip 渲染结构化说明。`active` 文案保留并细化（"Session is running — finish or cancel first"），新增 `prereq`（"Compact / Reset require a finished session"）、`unknown`（"Session status not confirmed yet — retry shortly"）场景。
...
- **Tests（`packages/web`）** —— 现有覆盖需迁移并扩展：
  ...
  - `SessionRecoveryActions.test.tsx`：禁用态暴露 `data-disabled-reason`；active / prereq / unknown 三种 reason 的渲染差异。
...
- **Risk (low)**：纯 presentation 层重排与文案调整，单页面范围；多数断言在 `SessionDetailShell` 的现有 render spec 上扩展；与 #427/#428/#429 范围正交，可并行启动；唯一的边界接触面是 `SessionFollowupComposer` 的 props 扩展（向后兼容）与 `SessionRecoveryActions` 的 `data-disabled-reason` 标记新增（向后兼容）。
