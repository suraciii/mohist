...
#### Scenario: Plan round complete event received
- **WHEN** a `plan_round_complete` event is received via SSE
- **THEN** the event is dispatched to the global event emitter for `useSessionTimeline` to update `planProgress` state
