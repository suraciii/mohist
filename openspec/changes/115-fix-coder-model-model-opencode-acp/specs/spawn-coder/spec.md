...
- **THEN** 系统将 template 替换为 "分析 issue: 用户登录"，启动 `opencode acp --cwd <worktree>` 子进程，通过 stdio JSON-RPC 发送 `initialize` → `session/new` → `session/setConfigOption(model)` → `session/prompt(替换后的task)`，等待响应返回结果文本

--- spawn_coder forwards model to opencode
- **WHEN** `spawn_coder` is called with `model: "anthropic/claude-3-5-sonnet-20241022"`
- **AND** `connection.newSession()` succeeds
- **THEN** the system calls `connection.setSessionConfigOption({ scope: 'agent', option: 'model', value: 'anthropic/claude-3-5-sonnet-20241022' })` before sending the prompt
- **AND** opencode uses the specified model for agent reasoning

--- model set before first prompt
- **WHEN** spawn_coder establishes an ACP connection with a `model` parameter
- **THEN** `setSessionConfigOption` is called after `newSession` and BEFORE the first `session/prompt`
- **AND** the model is applied to the newly created session
