...
- Refuse to delete a Profile still referenced by the Project default, an Issue, or an active WorkflowRun, identifying the blocking reference relationship. Profile deletion, Project-default writes, and WorkflowRun-binding writes are serialized per Project through a Profile reference coordinator; Issue explicit selection stays on the existing Issue creation/selection coordinator and is observed by the deletion check as a read projection, so a deletion cannot race any new reference into existence and the two coordinators never call each other.
...
- `workflow-profile-collection`: Project-scoped Profile list, view, create, edit, and delete behavior, including stable slash-capable IDs, built-in read-only Profiles in the same collection, raw Definition YAML reads, save-time Definition and Action-contract validation, and deletion protection with actionable reference reporting.
...
- **Docs** (`docs`, `design`): workflow Profile ownership and CLI reference move from templates to the Project-scoped collection model; `design/architecture.md` registers the new `WorkflowProfileReferenceCoordinator` as a second durable application process manager alongside `IssueRepositoryCoordinatorGrain`, with its ownership boundary (Project default / WorkflowRun binding / Profile deletion) and the rule that Issue selection stays on the existing Issue coordinator.
