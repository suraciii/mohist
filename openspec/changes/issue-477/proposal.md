...
- Refuse to delete a Profile still referenced by the Project default, any Issue including a terminal Issue, or an active WorkflowRun, identifying every blocking reference relationship. Profile deletion and every Profile reference write, including Issue explicit selection, are serialized per Project through one Profile reference coordinator. Issue creation still commits repository binding and Profile selection together in its single Issue transaction, so deletion cannot race any new reference into existence.
...
- **Docs** (`docs`, `design`): workflow Profile ownership and CLI reference move from templates to the Project-scoped collection model; `design/architecture.md` registers `WorkflowProfileReferenceCoordinator` as the durable application process manager for Profile deletion and all Profile reference writes, including Issue selection, while preserving Issue creation's single-transaction repository/Profile binding invariant.
