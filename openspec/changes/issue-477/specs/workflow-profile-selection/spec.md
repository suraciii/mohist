...
`mo issue create` and `mo issue edit` SHALL accept `--workflow-profile <profile>` to store an explicit Profile selection for the Issue. An explicit selection MUST refer to a Profile in the Issue's Project collection and SHALL take precedence over that Project's default when the Issue starts a WorkflowRun. `IssueRepositoryCoordinatorGrain` SHALL serialize this write with Issue creation and repository lifecycle commands; Issue creation SHALL preserve its existing invariant by committing repository binding and Profile selection in the same Issue transaction. A restrictive custom-Profile foreign key SHALL reject a deletion race as a retryable `workflow-profile-not-found` conflict; built-in Profiles are immutable and cannot be deleted.
