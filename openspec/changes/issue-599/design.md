...
The OpenCode `executePrompt` path and Pi adapter are changed so the coordinator can observe a request-attempt boundary instead of treating an SDK-owned retry sequence as one opaque request. Existing SDK retry events remain the source of native status details, but the adapter must either disable opaque automatic replay or expose each replay as a coordinator attempt. This is required to release permits during backoff and to enforce the independent rate-limit deadline. The normal OpenCode/Pi cleanup and generation-quarantine behavior remains in force for transport, deadline, and cancellation failures.
...
- Session boundary: runtime event types, transcript allowlist, typed Workflow observation, and reporter/outbox payload validation.
- Server: Runner report DTOs, `WorkflowItemTranslator`, `TaskReport`/`WorkResult`, Workflow run state transitions, status mapper/views, retry control, event catalog/serialization, and report/persistence tests.
...
- [A runtime SDK may hide automatic retry attempts and prevent exact permit release or Retry-After control] -> Keep the adapter boundary responsible for exposing one Provider attempt at a time; add a focused fake SDK contract test before enabling production retry behavior.
...
2. Integrate the controller into both runtime adapters and replace threshold-only 429 handling with the independent bounded retry coordinator. Add Pi/OpenCode adapter tests for Retry-After, fallback backoff, permit release, ordinary retry-budget separation, recovery, and expiry.
3. Extend the runtime event/outbox and Session-to-Workflow observation path with typed waiting facts. Add tests proving admission/backoff waiting is durable, nonterminal, does not advance the stage, and does not duplicate work.
4. Extend Runner reports and Workflow translation with `provider-rate-limited` and structured facts. Add the dedicated Workflow task/stage/run transition, persistence reload coverage, explicit retry action, and normal re-admission tests.
5. Update CLI/Web status models and renderers, then run the full contract and UI test suites. Verify existing success, genuine failure, cancellation, and unknown-result settlement behavior is unchanged.
6. Deploy Server, Runner, CLI, and Web as one compatible release. No database migration is required. Back up the Workflow state store before deployment and monitor Provider wait/expiry counts and retry outcomes.
