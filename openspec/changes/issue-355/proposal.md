...
- Preserve the existing fault-tolerance semantics: a malformed `config.jsonc` or watcher failure must not block server startup (mirrors today's `AddJsonStream` + try/catch behavior).
...
- **Risk**: The blast radius spans every configuration key (all flow through this source). File-watcher behavior varies across filesystems (network mounts, containers); the proposal requires reload failures to remain non-fatal, matching today's fault-tolerance. Tests must avoid real filesystem-watcher latency (per `design/testing.md`, use injectable providers/fake time, not wall-clock + sleep).
