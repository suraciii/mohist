...
The production adapter uses `spawnSync` with the existing external-process policy check, an abort signal, a 10-second timeout, UTF-8 decoding, and a 16 MiB buffer. Synchronous collection is intentional: it drains stdout before returning and avoids the previously observed race where process completion exposed only part of a roughly 49 KB response.

The test seam sits below this adapter, at a `SyncModelsProcessExecutor` dependency with the same command/arguments/options/result contract as `spawnSync`; it does not replace the entire command adapter. Production constructs the adapter with `spawnSync`. Tests construct the same adapter with a fake synchronous executor, call discovery through it, assert the exact `models --verbose` invocation plus `signal`, `timeout`, `encoding`, and `maxBuffer` options, and return a payload larger than 49 KB whose trailing model must reach the parser. Separate parser tests cover large preassembled text. This arrangement exercises command-result handling (`error`, non-zero `status`, complete `stdout`) without a real process and fails if the adapter is changed to an asynchronous exit-callback path.
...
Restore unit coverage for command precedence, every-call execution, complete large stdout, multiline JSON, variant-key extraction, malformed metadata recovery, empty/failure normalization, and order-insensitive equality. Command-boundary tests use the injected synchronous process executor beneath the real adapter; parser tests remain pure. Host specs use fake timers and fake discovery results to cover initial registration, runtime-independent periodic execution, changed/unchanged heartbeats, retained state on empty/failure, and timer cleanup.

Runtime tests remove catalog fixtures and assert that health success is sufficient for readiness, discovery failure cannot affect readiness, and rebuild recovery does not request a catalog. Shared OpenCode test support drops `RuntimeModelCatalog` and `catalogFactory` setup. Existing Issue, settings, create-dialog, and shared `ModelSelect` tests remain regression coverage. `AgentProfileEditor.test.tsx` must gain editor-surface coverage with a populated `modelVariants` response: render chips, persist a clicked model-plus-variant through create/update, reopen with the stored chip active, select the model body to clear only the variant, and clear the model to clear both fields. No Web production change is planned; if this test exposes a wiring defect, fixing that defect remains within the selector-regression acceptance boundary.
