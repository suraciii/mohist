...
An enabled-only `OtelDiagnosticsSampler` hosted service samples immediately and then every 10 seconds using injected `TimeProvider`. It runs one serial loop rather than timer callbacks, so samples cannot overlap; ticks that arrive while a sample runs coalesce into the next loop iteration. Shutdown prevents another sample and awaits the current fixed-operation synchronous probe rather than abandoning it. The guarantee is bounded operation count, not a wall-clock bound over host filesystem syscalls. It depends on two replaceable readers:

- `IProcessResourceReader` returns process total CPU time, working set and `GC.GetGCMemoryInfo().HeapSizeBytes`;
- `IOtelStorageProbe` performs a fixed-operation readiness probe and returns the combined size of `otel.db`, `otel.db-wal` and `otel.db-shm` plus the fixed 1-GiB budget.
...
3. Add the enabled-only continuous process/storage sampler and observable resource gauges by reusing the same production probe.
4. Add request scope/adapters and bounded route aggregation. Keep the response-local scope active for canonical agent reads even when OTel is off; suppress it on incoming Orleans turns.
5. Alter OTLP bind fallback to stop and dispose the failed host before rebuilding while preserving configured enabled intent and publishing `collector_bind_failed`; abort fallback on either shutdown failure.
6. Add agent candidate/processed/transcript accounting and the additive `amplification` response objects. Preserve current query behavior and make its amplification visible for #467/#468 to optimize later.
7. Run focused Server unit/spec tests and CLI specs, then the repository `npm test`. Verify metric contract, fake-time bucket rotation, transition logs, status no-scan behavior, self-feedback exclusion and `/api/health` independence.
