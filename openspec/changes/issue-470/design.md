...
- `PublishProcess(ProcessSampleResult result)` and `PublishStorage(StorageProbeResult result)`
- `RecordCollectorBind(CollectorBindResult result)` and `RecordProtection(ProtectionOutcome outcome)`
- `GetSnapshot()`

All mutation is thread-safe. Each public fact method owns only its named source: process publication alone changes `process_read` and process cache values, storage publication alone changes `storage_read` and storage cache values, `RecordIngest` alone changes `storage_write`, collector publication alone changes `collector`, and protection publication alone changes `ingest_protection`. The shared source-map mutation helper is private, so a caller cannot clear another producer's failure. `GetSnapshot` copies immutable values under the state lock and performs no database, filesystem, HTTP or grain call. The status route maps this internal snapshot to its wire DTO; the CLI knows only the wire DTO. Follow-up issues call the fact-oriented methods and do not mutate counters or DTO fields directly.

`IngestOutcome` contains non-negative `Received`, `Saved`, `Rejected` and `Dropped` Span-attempt counts, a typed storage-write result (`not_attempted`, `succeeded`, or `failed` with a bounded reason), and an optional bounded protection reason. It is produced once at the ingest boundary, including on a typed write failure, so callers cannot independently publish contradictory increments. `RecordIngest` activates only `storage_write` for a failed write and clears only that source for a later successful write; rejected or dropped observations are forwarded through the narrow protection publication path. `Received` counts parsed Span attempts; duplicate upserts count as saved; rejected means an admitted Span intentionally refused with a non-retryable OTLP partial-success response; dropped means malformed data or another loss for which the response will not request a retry. A rolled-back write that returns a retryable failure increments received and activates `storage_write`, but does not increment dropped, so the four counters intentionally need not satisfy a conservation equation. Issue #470 implements and tests `RecordIngest` as the production publication/read contract; it does not invent a rejection policy. #437/#471 are the first production publishers of non-zero `Rejected`, and their tests must drive that value through this same contract into status.
...
Add `RuntimeRequestMetricsMiddleware` after routing and before endpoint execution. It creates an internal `RequestWorkScope` in `AsyncLocal` when either (a) OTel is enabled and the matched endpoint is not under `/otel/v1` or `/otel/api`, or (b) the endpoint is a canonical or compatibility agent status/activity route. It records a monotonic start timestamp through `TimeProvider` and always clears the ambient slot in `finally`. The scope exposes an immutable `Snapshot()` during endpoint execution so an agent handler can build its response after all awaited work completes. At response completion the middleware calls `CloseAndSnapshot()`, which atomically marks the scope closed and copies its counters; every adapter increment checks the same closed state and becomes a no-op after that point. The middleware then resolves `RouteEndpoint.RoutePattern.RawText`, method and response status and calls `RuntimeObservability.CompleteRequest` exactly once only when OTel is enabled; off-state agent scopes are discarded.
...
The scope uses atomic integer increments so parallel fan-out inside one request is safe. `Snapshot()` copies the current counters without closing the scope; later awaited calls can still increment until the handler takes its final snapshot. `CloseAndSnapshot()` is the single terminal operation and has a deterministic race rule: an increment linearized before close is included, and one linearized after close is ignored. It carries no project or domain identity and cannot affect call results. `RequestWorkScope.SuppressFlow()` temporarily clears only this ambient slot while a fire-and-forget callback is queued, then restores the caller slot; the child execution context therefore captures no request scope. T-004 migrates the current detached launch sites in `SystemUpdateService` and `BackgroundHermesIssueNotificationDispatcher` to that API. Separate deterministic tests (a) retain a direct scope reference and prove increments after `CloseAndSnapshot()` do not change its terminal snapshot, (b) hold a child queued through `SuppressFlow()` behind an awaitable signal and prove it observes `RequestWorkScope.Current=null`, and (c) exercise each migrated launcher through its test seam and assert its callback starts without the ambient scope. "Downstream" is intentionally caller-side, not transitive: the incoming Orleans filter ensures grain-to-grain calls and grain-side database work are not attributed back to the originating HTTP request. Direct `otel.db` SQLite operations do not pass through the EF interceptor. OTel routes do not create a scope, so their query, ingest and status work is excluded. Storage sampling/maintenance runs outside HTTP scope and therefore cannot recursively contribute route work.
...
One `OtelDiagnosticsSampler` hosted service owns all diagnostic probing. Its `StartAsync` performs exactly one failure-contained process publication in every configured state: it calls only `IProcessResourceReader`, passes either a success sample or failure result to `PublishProcess`, and returns without invoking `IOtelStorageProbe`. Server availability therefore waits only for the published current process values or their explicit unavailable result; a storage open or metadata failure cannot delay or prevent that availability. The service's controlled async loop waits for the host `ApplicationStarted` lifecycle signal. Only after that signal does an enabled instance perform its sole immediate storage probe, then continue with injected 10-second ticks; a host that fails before `ApplicationStarted` never starts a storage probe. T-002 delivers the probe module but does not register a temporary initializer. The loop runs one serial iteration at a time, so storage samples cannot overlap; ticks that arrive while a storage sample runs coalesce into the next iteration. Shutdown prevents another sample and awaits the current fixed-operation synchronous storage probe rather than abandoning it. The guarantee is bounded operation count, not a wall-clock bound over host filesystem syscalls. It depends on two replaceable readers:

- `IProcessResourceReader` returns process total CPU time, working set and `GC.GetGCMemoryInfo().HeapSizeBytes`;
- `IOtelStorageProbe` performs a fixed-operation readiness probe and returns the combined size of `otel.db`, `otel.db-wal` and `otel.db-shm` plus the fixed 1-GiB budget. Its production implementation composes `IOtelReadinessConnectionFactory` (production calls the new `OtelDb.OpenReadinessConnection`; tests use named shared-memory SQLite or a throwing fake) and the extended fakeable `IFileSystem.GetFileLength`, which returns `null` only when a file is absent and propagates every other metadata failure.

CPU utilization is the delta in process CPU time divided by elapsed injected time and processor count, clamped to `[0,1]`. The immediate process sample makes working set and GC heap available in every state; CPU utilization is `null` until the second valid process sample establishes a delta. A process-reader failure invalidates all cached process values and activates `process_read`; a later successful read clears that source and starts CPU warm-up from a new baseline. Enabled status is degraded while that source is active. Configured off status remains off by state precedence but exposes `process_read_failed` and null process values. Storage growth is the byte delta between the oldest and newest enabled-state samples in a fixed 60-second, seven-slot ring divided by sample time. Until two valid samples exist, growth is `null`, not false zero. Observable gauges omit every value while off; status still reads the process cache directly. Storage fields are `null` while off because no storage metadata call runs.

T-002 adds `OtelDb.OpenReadinessConnection()` as the sole production readiness-open operation. It clones the read-write connection string with `Default Timeout=1` before constructing and opening the dedicated connection, then calls the existing `EnsureInitialized` schema path and returns the initialized connection. Every initialization command therefore inherits the one-second command timeout without relying on a `PRAGMA` issued after initialization or on an unobservable wrapper order. `IOtelReadinessConnectionFactory` is the replacement seam around that operation. A named shared-memory SQLite test asserts the returned connection's `DefaultTimeout` is one second and the fixed schema exists; no wall-clock lock-duration assertion is used. The production storage probe executes one `PRAGMA schema_version` metadata read on the returned connection. Unlike a constant expression, this touches the database header and read-lock path without scanning application rows. T-002 also extends every production/test `IFileSystem` implementation with `long? GetFileLength(string path)`: `null` means absent, a non-negative value is length, and metadata errors propagate. The probe calls it exactly once for each of database, WAL and SHM after initialization and sums absent sidecars as zero, so returned usage includes files created by initialization without a separate existence check. It never uses `Task.Run`, abandons a worker, counts rows, or enumerates Trace/Span history. The command timeout bounds SQLite lock wait; arbitrary filesystem syscalls are neither claimed cancellable nor assigned a wall-clock deadline. `PublishStorage` invalidates cached storage values and activates only `storage_read` for a read/probe failure, then clears only that source on a later successful probe. `RecordIngest` reports `storage_write` failure and clears only that source on a later successful write. #437 will replace/extend the same probe with its persisted watermarks and publish protection state through `RecordProtection`; it does not replace status assembly. Corrupt/locked/open failures use the connection-factory fake, and metadata missing/error/order/count behavior uses the repository `FakeFileSystem`, so no test touches host storage.
...
`RuntimeObservability` keeps a bounded map keyed by a fixed enum: `collector`, `process_read`, `storage_read`, `storage_write` and `ingest_protection`. An enabled instance starts with `collector_unverified` and `storage_unverified` active; successful Kestrel start clears the collector source, and the first successful storage probe clears the storage source. Process state starts without a failure source because no read has failed; the immediate sampler either publishes values or activates `process_read`. `collector_online` is projected from that same collector source, not maintained as a second flag. Status is derived as follows:
...
The OTLP-port fallback in `Program.cs` must stop rewriting `Mohist:Otel:Enabled=false`. After failed `StartAsync`, it calls `StopAsync` on the failed app and always awaits `DisposeAsync`. Alternate-host construction begins only when both operations succeed. A `StopAsync` or `DisposeAsync` failure is logged and propagated after disposal is attempted, because the process cannot prove the first Orleans silo terminated and must not risk starting a second. The alternate host omits only the OTLP Kestrel listener, keeps the user's configured enabled intent, records `collector_bind_failed`, and still maps the main-port status route. The failed primary may have run its one process publication, but it never receives `ApplicationStarted`; across both host graphs exactly one diagnostics loop enters post-start sampling and exactly one enabled immediate storage probe runs. A fallback integration test counts the primary and alternate service lifecycles, proves the primary never probes storage, proves one surviving silo and sampler with no OTLP listener, and verifies `degraded` status with `collector_online=false`.
...
Add one shared `AgentPathAmplificationDto` with camelCase wire fields `candidates`, `processed`, `transcriptRecords`, `databaseCalls` and `downstreamCalls`. `AgentStatusResponse` and `ActivityDto` each gain an `amplification` property; all existing fields, ordering and response limits remain unchanged. Keep `/api/projects/{projectRef}/agent/status|activity` canonical. Restore `GET /api/agent/status|activity` as thin compatibility aliases. A shared selector resolver trims both inputs, chooses the first nonblank `projectId` query value and otherwise the first nonblank `X-Mohist-Project` header value; blank or whitespace-only values are absent, and no nonblank selector returns 400 `No active project`. A nonblank query wins over a conflicting nonblank header. The aliases delegate to the same scoped handlers and never enumerate or aggregate all projects.
...
- `[AsyncLocal crosses an in-process Orleans boundary or flows into detached work] ->` Clear/restore it in an incoming grain-call filter, close the mutable scope atomically at response completion, and suppress this ambient slot while current fire-and-forget callbacks are queued. An in-process Orleans chain test proves only caller-side sends count; a signal-controlled detached-child test proves post-response work cannot mutate the observation.
...
- `[Sampler failures can leave stale resource values] ->` Invalidate the failed sample and activate `process_read` or `storage_read`; never present the prior value as current or substitute zero. Process recovery restarts CPU warm-up, and process failure while configured off retains `off` state while exposing the reason.
...
2. Add the fixed status API/CLI shape and the production fixed-operation storage-probe module over the publication core. Do not host or invoke the probe at startup yet; enabled status remains `storage_unverified` until step 3.
3. Install the sole continuous sampler: process sampling in every state and enabled-only immediate/repeated storage probing through the step-2 module. Never add a second initializer and never publish gauge measurements while off.
