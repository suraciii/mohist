...
Before a write transaction, `TraceIngester` makes one complete classification pass over the materialized request and creates an immutable `PreparedIngestBatch`. Every identifiable Span attempt is classified as parsed-for-write, malformed dropped, protection rejected, or another non-retryable drop. An `IngestOutcomeBuilder` with a private-result constructor combines that classification with exactly one write result: `not_attempted`, `committed`, or `rolled_back` with bounded reason text. It derives all four counters, so callers cannot provide contradictory aggregates. A committed duplicate upsert is saved; rollback sets saved to zero, preserves parsed attempts as received, activates `storage_write`, and does not convert retryable work into dropped work. Only `committed` from a transaction that actually entered the production write path clears `storage_write`; an empty, wholly rejected, wholly dropped, or parse-failed request is `not_attempted` and cannot establish write readiness. The same atomic `RecordIngest` call refreshes protection when rejected or dropped counts are non-zero.
...
Runner unit tests use fake factory/hosts/classifier and fake application-start signals to trigger generic failure, classified bind failure, stop failure, dispose failure, and dual failure without ports. They own all runtime assertions: stop/dispose ordering, no alternate on shutdown failure, at most one started host remaining, zero primary storage probes, one alternate immediate probe, and alternate degraded status with core health successful. These are lifecycle-boundary facts about the runner and fake host, not claims that a production silo was started.

A separate non-starting production composition test owns only static graph assertions. It compares primary and alternate `MohistHostPlan` values, verifies that epoch/enabled/application composition are identical while only OTLP listener intent and initial collector result differ, and inspects service descriptors to prove one sampler registration and the shared Orleans/API registration markers per plan. It does not call `Build`, `StartAsync`, resolve physical filesystem adapters, open listeners, or claim runtime probe/silo facts. Because the production adapter has one `Build(plan)` implementation and no alternate composition branch, the inspected plan and registrations are the exact inputs consumed by production construction.
