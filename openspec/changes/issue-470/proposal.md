...
- Maintain a bounded five-minute in-memory diagnostic window at one-second resolution, with at most 10 stable routes ranked by request amplification, latency, and route name; the summary resets on Server restart and does not become business data. "Anomalous" in the issue means this threshold-free top-ranked diagnostic view, not a learned baseline or alert threshold.
...
- Expose candidate, actually processed, transcript/database, and downstream-call counts on the canonical project-scoped agent status and activity read paths so polling and feed amplification can be diagnosed directly. Restore the issue's literal `/api/agent/status` and `/api/agent/activity` paths as compatibility aliases that require `projectId` query or `X-Mohist-Project`, resolve the same project, and delegate to the canonical handlers; they never aggregate globally.
...
- `runtime-observability-metrics`: Standard `Meter` signals and matching bounded local summaries cover request volume and latency, database and downstream amplification, process pressure, telemetry outcomes, and storage pressure using only stable low-cardinality labels, while excluding the built-in observability pipeline from feeding itself. The rejection signal is a publication contract consumed by the storage/admission protection issues; this change does not introduce a new rejection policy.
...
- **Agent read paths** (`packages/server/src/Mohist.Server/Api/AgentRoutes.cs` and their status/activity assemblers and queriers): add amplification counts to canonical project-scoped status/activity responses and the explicit unscoped compatibility aliases; instrument their database, transcript, and downstream work even while OTel collection is configured `off`, while Meter emission and route aggregation remain disabled in that state.
