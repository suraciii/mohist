...
- Expose candidate, actually processed, transcript/database, and downstream-call counts on the canonical project-scoped agent status and activity read paths so polling and feed amplification can be diagnosed directly. The issue's abbreviated `/api/agent/*` names refer to these current `/api/projects/{projectRef}/agent/*` surfaces; the removed unscoped routes are not restored.
...
- **Agent read paths** (`packages/server/src/Mohist.Server/Api/AgentRoutes.cs` and their status/activity assemblers and queriers): add amplification counts to the existing project-scoped status and activity responses and instrument their database, transcript, and downstream work even while OTel collection is configured `off`; Meter emission and route aggregation remain disabled in that state.
