...
Server SHALL 能够作为独立的后台进程运行，持续监听 HTTP 请求。Server 启动时 SHALL 初始化 ConfigLoader 加载 `~/.mohist/config.jsonc`，并将解析后的 provider 配置传递给 agent-runtime。

#### Scenario: 启动 server
- **WHEN** 用户执行 `mo server start`
- **THEN** server 在 localhost:3456 监听 HTTP 请求
- **AND** server 初始化 ConfigLoader，加载 config.jsonc
- **AND** server 初始化 Agent Runtime with resolved provider config
...
#### Scenario: config.jsonc 不存在时启动
- **WHEN** `~/.mohist/config.jsonc` 不存在
- **AND** 环境变量 `ANTHROPIC_API_KEY` 已设置
- **THEN** server SHALL 正常启动
- **AND** 使用环境变量中的 API key
- **AND** 使用默认模型 "anthropic/claude-sonnet-4-20250514"

#### Scenario: config.jsonc 解析失败时启动
- **WHEN** `~/.mohist/config.jsonc` 包含无效 JSON
- **THEN** server SHALL 打印明确的解析错误
- **AND** server SHALL 以非零退出码退出
