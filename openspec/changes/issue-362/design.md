...
`AgentLauncher` writes trigger event/subscription labels in the initial session open, before durable job submission. `AgentJobGrain` persists the input, lifecycle, candidate runner, and a stable work id derived from the grain key before acknowledging `SubmitAsync` or calling the Runner. Agent config is persisted as raw JSON text and reconstructed on demand, avoiding nullable `JsonElement` serializer drift across activation. A crash before Runner acceptance reactivates and resumes the same record. A crash after Runner acceptance replays the same `(ownerId, workId)`, which `RunnerGrain.AssignAgentJobAsync` absorbs idempotently.
