...
- **Small operator API/CLI surface; no Web or runner changes.** Dead-letter inspection and recovery are exposed as `GET /api/events/dead-letters`, `POST /api/events/dead-letters/{id}/redeliver`, and `mo event dead-letter list|redeliver`. Retry remains hand-rolled with a fixed per-handler attempt cap (see `design.md` D6 — Polly is *not* referenced in the codebase despite an earlier draft of this section claiming so).
