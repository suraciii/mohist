...
- **Small operator API/CLI surface; no Web or runner changes.** Direct-loopback-only, proxy-rejecting, stack-redacted dead-letter inspection and recovery are exposed as `GET /api/events/dead-letters`, `POST /api/events/dead-letters/{id}/redeliver`, and `mo event dead-letter list|redeliver`. Retry remains hand-rolled with a fixed per-handler attempt cap.
