...
- Server AgentSession runtime-event endpoints, `AgentSessionGrain`, and receipt/acceptance contracts change together with the Runner. A version-2 request carries `runtimeEventId` and complete delivery identity; the grain persists `Pending` and `Accepted` ledger states, idempotently flushes transcript effects, and retries Workflow binding/follow-up operations under that ID before acknowledging replayed or mixed batches across workflow, generic, Session, and cleanup routes. Existing transcript semantics and exact execution identities remain authoritative.
