...
- When an Agent result settlement becomes durably `Blocked`, release only the Runner control-plane active-work/capacity projection at that commit boundary, retain the settlement identity for a matching late result, and keep pre-deadline `Unknown` work represented. Non-authoritative `unknown` observations must not be converted into task failures when their observation is stale.
...
The report adapter must keep an inbound `unknown` result as an observation rather than using `InboundReport.Unknown.Fallback` as an inferred failure. After `ObserveAgentResultUnknownAsync` returns `Accepted`, it returns that acknowledgement without task settlement. After it returns `Stale` (including because the matching settlement is already `Blocked`), it returns `stale` without calling `ReceiveTaskReportAsync`; this prevents `FailTask` and leaves the blocked settlement and Runner projections unchanged. Only an explicitly authoritative non-`unknown` success or failure result may enter `ReceiveTaskReportAsync`, where the existing task/work/Runner identity checks still fence mismatched reports.
...
Manifest and executor tests will also verify that the engine-sourced expected branch is populated from `workspace.branch` and that existing workflow profiles do not need a second branch declaration. Server settlement tests will use a fake clock to keep an `Unknown` attempt active before its deadline, durably transition it to `Blocked`, observe one release from `activeWorks` and capacity, and accept only an identity-matching late authoritative report. They will also send a matching late `unknown` result through `WorkflowReportService` and verify a stale acknowledgement with no fallback task report, `TaskFailed` event, settlement mutation, or Runner projection change.
...
- [Risk] Filtering a durably blocked run from Runner projections could reject a legitimate late result or release the projection twice. -> Mitigation: filter only the indexed `AttentionStatus=blocked` projection after the settlement commit, keep the assignment and full task/work/Runner identity in the aggregate, accept matching late authoritative reports through the grain, reject mismatches as stale, and test repeated reconciliation/poll/status reads with fake time.
- [Risk] A matching late `unknown` observation could be translated into the existing failed fallback after the domain correctly rejects it as stale, creating `TaskFailed` after `Blocked`. -> Mitigation: make `WorkflowReportService` return `stale` immediately for a stale unknown observation and never call `ReceiveTaskReportAsync` with `InboundReport.Unknown.Fallback`; test the matching blocked case alongside authoritative and mismatched reports.
...
4. Align durable blocked-settlement persistence, Runner active-work/capacity queries, missing-redelivery reconciliation, and the report adapter's stale-unknown handling. Add fake-time tests for the exactly-once blocked boundary, pre-deadline `Unknown`, matching late authoritative reports, matching blocked `unknown` observations, and mismatched late reports.
