...
- **Workflow result handling:** Existing `WorkItemResult` failure reporting will carry the durable branch/workspace diagnostic. For `branch-invariant-violation`, the runner will report `failed` without `addTasks`; no recovery handler or retry task may be projected as successful completion, and no new AgentSession result-replay protocol is required. The report service will preserve the distinction between an authoritative task result and an `unknown` observation: a stale unknown observation returns `stale` and never forwards its failed fallback to `ReceiveTaskReportAsync`, so it cannot create `TaskFailed` after `Blocked`.
