...
The Web change treats `mohist/pi` as an Inline Agent task for Session/task-log milestones and extends the shared runtime-neutral usage type/rendering with nullable `cachedWriteTokens`. Server Session state, transition inputs, append-only Orleans serialization IDs, runtime-event parsing, `AgentUsageDto`, activity/read models, and all DTO mappers gain the matching field; existing persisted rows deserialize it as absent. Transcript, lineage, runtime, and tool DTOs otherwise remain unchanged. No Pi-specific Session component, model selector, or credential UI is added.
...
- Pi runtime unit/spec tests: readiness including empty catalog, trust/resource setup, credential confinement/redaction, create/open/cache, literal prompt, model/thinking changes, event dedup/reconciliation, provider configuration and policy, warning/deadline/cancellation/provider-abort confirmation, execution quarantine, missing file, and no replay.
...
- Server specs/unit tests: runtime-aware Workflow open/bind, dedicated sequenced Action route versus unchanged Follow-up/generic routes, existing OpenCode caller migration, expected-binding races, symmetric cross-runtime lineage, atomic stream-seal/rebind, workspace mismatch, Pi registration, Action sequence duplicate/gap/sealed/stale rejection, separate cached-read/cached-write persistence and API projection with append-only serializer IDs, Session facts not advancing Workflow, classification, and parent context.
- Coordinator tests: same-runtime and concurrent OpenCode/Pi tasks serialize by logical Session, project/WorkflowRun/name key dimensions remain isolated, the task lease spans completion plus cleanup against an intervening queued task, and a rebind fence remains held through drain and bind.
- Outbox tests run the production codec/state machine over in-memory byte storage: atomic manifest creation before Prompt, durable append before send, input acknowledgement before Prompt, ambiguous acknowledgement retry, ordered gap recovery, partial-snapshot replacement, per-stream corrupt-snapshot quarantine, global root/capability readiness failure, Runner restart drain/reconciliation, pre-rebind drain, append failure quarantine/repair, and no Prompt replay. Atomic-replace protocol tests over fake low-level operations cover write/sync/rename/directory-sync ordering and every failure point, including commit-uncertain. Default tests do not instantiate the production filesystem adapter.
- RunnerHost tests use fake timers to cover pre-registration runtime/storage readiness, empty catalog, retry/recovery, polling suppression, and continued result-ack drain. Web tests cover Pi inline-Agent classification, milestones, and every supplied usage dimension including distinct cache-read/cache-write values.
...
6. Add `piRuntime` to the current Runner host/executor context, combined runtime/storage pre-registration readiness gating, outbox-backed reporter, `mohist/pi` Action, promise projection, and cleanup reuse. Record the runtime handle/name-list additions as temporary #447-owned debt; leave AgentJob and Session handlers unchanged.
