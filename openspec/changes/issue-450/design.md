...
The Workflow host declares the fixed 60-minute duration through Runner-private context; Action input cannot override it. The runtime uses injected time, emits one wrap-up `steer`, fixes timeout/interruption before calling `abort`, and cannot let a late prompt completion change the result. It never replays a prompt with uncertain submission state. Extract the existing OpenCode provider-failure defaults into a runtime-neutral pure policy (`quota`/`credit`/`billing` patterns and threshold 5) and inject that policy into both runtimes; tests may inject a validated non-default policy, but this issue adds no user-facing setting or Action input. Provider exhaustion normalizes to `turn-failed`; the current binding remains unchanged.
...
2. validates optional `session` and the explicit `options.model` / `options.variant` object;
...
5. reports `session.input`, invokes `PiRuntime.runTurn`, flushes required runtime facts, and returns the final text only as private `turnFact`.
...
The Pi projector normalizes assistant text, reasoning, tool lifecycle/results, model, status, and usage. Usage includes input, output, cache read, cache write, thought when supplied, cost amount, and currency. Stable Pi message IDs and tool-call IDs make duplicate callbacks idempotent; after prompt completion, final messages reconcile any missing final assistant/tool/model/usage facts.

The Action sends normalized batches through `workflowAgentSessionRuntimeEvents` with the current physical `runtimeSessionId`. The Server's existing binding check rejects stale facts before state mutation. Add `cachedWriteTokens` through Session state/surrogates, runtime-event parsing, API/read models, and shared Web types; existing cost fields are reused.

The reporter may buffer only the current in-memory turn so callback delivery remains ordered. It must finish required input and final-fact writes before Action success. Any required Server write or final flush failure returns `session-reporting-failed`; completion, promise, and file checks do not run. There is no local durable outbox, background success, cursor, stream manifest, inventory, checkpoint, replay, or special transport ownership scheme. A later Workflow redelivery can repeat the turn, which is the accepted crash/reporting failure tradeoff.
...
Existing runtime-neutral Session transcript, tool, model, usage, and lineage components render Pi facts. Add fixtures for Pi and cache-write/cost usage; do not add a Pi-specific view, reporting-diagnostic field, model selector, provider form, or Session-command control.
...
- **Crash/reporting window:** without a new durable Runner protocol, process death or required reporting failure can lead Workflow redelivery to repeat a submitted turn. This is explicit and preferable to inventing ownership and recovery infrastructure in this issue.
...
3. Add runtime-aware Workflow Session binding and cache-write usage contracts with Server tests.
...
No database rewrite or Action-stream migration is required. Deploy Server and Runner together because the new Runner sends runtime-aware Workflow Session requests.
