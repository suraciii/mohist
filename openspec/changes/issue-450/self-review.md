...
No plan artifact assigns a Runner-scoped authoritative inventory route. After total local loss, startup can therefore report ready and poll before discovering admitted Pi/OpenCode turns, contradicting the pre-registration recovery and host test. Add a Server query scoped by the existing stable `runnerId` convention that enumerates current Workflow Action streams owned by that Runner with logical identity, physical binding, stream ID, cursor, projector checkpoint, and latest lifecycle. This finding does not require a new Runner enrollment, credential, or cross-channel authentication system. Assign the Server contract to T-003, inventory/local reconciliation to T-004, pre-registration composition to T-006, and tests for empty local storage with nonzero admitted Pi/OpenCode streams plus selective manifest loss.
