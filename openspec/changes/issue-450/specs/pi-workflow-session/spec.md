...
Runner provisioning SHALL issue one opaque bootstrap credential bound to its stable Runner identity. Before registration or polling, the Runner SHALL use that credential over the authenticated transport to retrieve an inventory of current Workflow Action streams it owns and reconcile every returned item against local storage. The Server SHALL derive the Runner identity from the credential rather than caller-supplied route/query/body fields. The inventory SHALL include logical source identity, current physical binding/work directory, stream ID/cursor, projector checkpoint, and latest turn lifecycle. It SHALL include no stream owned by another Runner and no sealed/history/generic/AgentJob binding. Successful registration SHALL rotate and return the bootstrap credential; failed recovery SHALL retain its prior credential for retry. Empty local storage or selective manifest loss SHALL therefore still recover every current owned prepared/admitted checkpoint before work claiming begins. Credential material MUST NOT enter Action stream state, outbox bytes, registration payloads, diagnostics, or logs.
