...
An operator-authorized enrollment flow SHALL issue one opaque stable Runner transport credential bound to its Runner ID, store only its hash on the Server, and supply its clear value through `MOHIST_RUNNER_CREDENTIAL`; explicit re-enrollment replaces that hash. A shared transport authenticator SHALL derive identity only from this credential for every Runner-originated HTTP route, including workflow-run status and generic AgentSession lookup/open/attach/runtime-events, SignalR hub handshake/reconnect, task-log upload, artifact upload, OpenSpec task-batch mutation, and workflow-variable mutation; caller-supplied Runner IDs MUST be rejected on mismatch and never establish identity. Task-log/artifact/OpenSpec task-batch/workflow-variable writes SHALL require the derived Runner to own their active work; OpenSpec task-batch and workflow-variable protocols SHALL include required workId and the Server SHALL validate it is active in the target run and assigned to that Runner before mutation. Generic AgentSession routes SHALL validate source/binding ownership before exposing or mutating a Session. Before registration or polling, the Runner SHALL use that credential over the authenticated transport to retrieve an inventory of current Workflow Action streams it owns and reconcile every returned item against local storage. The inventory SHALL include logical source identity, current physical binding/work directory, stream ID/cursor, projector checkpoint, and latest turn lifecycle. It SHALL include no stream owned by another Runner and no sealed/history/generic/AgentJob binding. Empty local storage or selective manifest loss SHALL therefore still recover every current owned prepared/admitted checkpoint before work claiming begins. Credential material MUST NOT enter Action stream state, outbox bytes, registration payloads, diagnostics, or logs.
