...
An operator-authorized enrollment flow SHALL issue one opaque stable Runner transport credential bound to its Runner ID, store only its hash on the Server, and supply its clear value through `MOHIST_RUNNER_CREDENTIAL`; explicit re-enrollment replaces that hash. Before registration or polling, the Runner SHALL use that credential over the authenticated transport to retrieve an inventory of current Workflow Action streams it owns and reconcile every returned item against local storage. The Server SHALL derive identity from the credential rather than caller-supplied route/query/body fields. Registration, poll, report, Workflow open/bind/rebind, inventory, and Action-event routes SHALL require the same transport identity and reject mismatched caller-supplied Runner IDs. The inventory SHALL include logical source identity, current physical binding/work directory, stream ID/cursor, projector checkpoint, and latest turn lifecycle. It SHALL include no stream owned by another Runner and no sealed/history/generic/AgentJob binding. Empty local storage or selective manifest loss SHALL therefore still recover every current owned prepared/admitted checkpoint before work claiming begins. Credential material MUST NOT enter Action stream state, outbox bytes, registration payloads, diagnostics, or logs.
