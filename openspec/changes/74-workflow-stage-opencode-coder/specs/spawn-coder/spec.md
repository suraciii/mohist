...
- **THEN** 系统将 template 替换为 "分析 issue: 用户登录"，启动 `opencode acp --cwd <worktree>` 子进程，通过 stdio JSON-RPC 发送 `initialize` → `session/new` → `session/setSessionConfigOption({ configId: "model", value: "..." })` → `session/prompt(替换后的task)`，等待响应返回结果文本
...
#### Scenario: spawn coder 使用 stage 相关模型
- **WHEN** Main Agent 调用 `spawn_coder({ ..., stage: "build" })`
- **AND** `~/.mohist/config.jsonc` 中配置了 `opencode.stageModels.build`
- **THEN** 系统在 `newSession` 后、`prompt` 前调用 `setSessionConfigOption({ configId: "model", value: "configured-model" })`
- **AND** workflow_log 记录 `model_selected` 事件

#### Scenario: spawn coder 配置模型验证失败
- **WHEN** `spawn_coder({ ..., stage: "build" })` 调用时 `opencode.stageModels.build` 配置了不存在的模型
- **THEN** 拒绝启动 session，返回错误包含可用模型列表
