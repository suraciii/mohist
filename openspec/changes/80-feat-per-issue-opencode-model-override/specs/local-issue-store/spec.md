...
#### Scenario: Issues table model column
- **WHEN** 数据库迁移到 schema version 15
- **THEN** issues 表新增 `model` 列（TEXT，nullable）
- **AND** 现有 issues 的 model 默认为 `NULL`
...
- **THEN** CLI 显示 Issue 详情（title, body, stage, status, labels, model）和所有 comments
