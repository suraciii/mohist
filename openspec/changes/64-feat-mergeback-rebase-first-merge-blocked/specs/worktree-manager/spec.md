...
### Requirement: WorktreeManager 支持 rebase issue 分支到最新 master

WorktreeManager SHALL 支持在 worktree 内将 issue 分支 rebase 到最新的 `origin/<baseBranch>`，并提供 abort 能力。

#### Scenario: rebase 成功

- **WHEN** MergeQueue 调用 `rebaseOntoMaster(worktreePath, baseBranch)`
- **AND** issue 分支可以干净地 rebase 到最新 baseBranch
- **THEN** 系统在 worktree 内执行 `git fetch origin <baseBranch>` + `git rebase origin/<baseBranch>`
- **AND** 返回 `{ success: true }`

#### Scenario: rebase 冲突时 abort

- **WHEN** MergeQueue 调用 `rebaseOntoMaster(worktreePath, baseBranch)`
- **AND** rebase 过程中出现冲突
- **THEN** 系统执行 `git rebase --abort` 清理 rebase 状态
- **AND** 返回 `{ success: false, message: "冲突描述", conflictingFiles: string[] }`

#### Scenario: rebase 前自动提交未提交更改

- **WHEN** MergeQueue 调用 `rebaseOntoMaster(worktreePath, baseBranch)`
- **AND** worktree 内有未提交的更改
- **THEN** 系统先 `git add` + `git commit` 提交所有更改
- **AND** 然后执行 rebase
