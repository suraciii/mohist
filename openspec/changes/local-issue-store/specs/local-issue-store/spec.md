...
#### Scenario: 创建 Issue
- **WHEN** 用户执行 `ph issue create "title" [-l label]...`
...
#### Scenario: 关闭 Issue
- **WHEN** 用户执行 `ph issue close <id>`
- **THEN** Server 设置 status 为 `blocked`（或后续定义的 closed 状态）
- **AND** Server 更新 `updated_at`

#### Scenario: 重新打开 Issue
- **WHEN** 用户执行 `ph issue reopen <id>`
...
#### Scenario: 添加评论
- **WHEN** 用户执行 `ph issue comment <id> "comment text"`
