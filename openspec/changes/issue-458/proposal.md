...
- When a later turn resumes the same Workflow AgentSession, the server establishes a deterministic persistence boundary for the prior closed turn before accepting the new activity, so back-to-back turns cannot merge or overwrite an input while waiting for deferred persistence.
...
- **Server/Session persistence** (`packages/server/src/Mohist.Server/Sessions/` and focused server specs): the existing AgentSession event path flushes a pending closed turn before accepting activity that resumes it. No endpoint, request shape, database schema, or persisted model changes.
