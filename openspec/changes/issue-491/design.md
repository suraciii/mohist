...
Build the CloudEvent with `agentid` (required) and issue/epic/workflow-run lineage when the job's launch context carries it (sourced from `State.Input` / `State.RoutedPlan`); omit them for contextless jobs. Add the `com.mohist.agent.job.failed` constant to `EventCatalog.ReverseDns` and the `All` list. Register a new `AgentJob` family in `ProducerConformance` with `agentid` required and issue/epic/workflowrunid optional. Carry `failureReason`/`failureCategory` in the payload so the notification renderer can surface why it failed (`HermesIssueNotificationPayload` already has a `FailureReason` field).
