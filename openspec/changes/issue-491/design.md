...
The reminder's self-clean condition (`ReceiveReminder :1090`) widens from "no pending session close" to "no pending session close **and** failure event emitted (or not a failure)" — this matters for agentless jobs (raw/workflow dispatch), which have no `PendingSessionClose` and would otherwise unregister the reminder before the event append succeeds.

- *Alternative: emit from the AgentSession terminal-close path.* Rejected: agentless jobs have no session (`:1002`), so failures would be silently missed; and session transcript facts are not dispatched CloudEvents.
...
Build the CloudEvent with `agentid` (required) and issue/epic/workflow-run lineage when the job's launch context carries it (sourced from `State.Input` / `State.RoutedPlan`); omit them for contextless jobs. A raw prompt job with no resolved Agent uses the distinct `com.mohist.agent.raw-job.failed` contract instead of fabricating an Agent identity from the job key. Add both failure-event constants to `EventCatalog.ReverseDns` and the `All` list. Register `AgentJob` in `ProducerConformance` with `agentid` required and issue/epic/workflowrunid optional, plus a `RawAgentJob` family that explicitly forbids `agentid`. The raw contract is durable and auditable but does not enter the Agent-response notification or routing protocol because it has no Agent identity. Carry `failureReason`/`failureCategory` in the payload so the notification renderer can surface why it failed (`HermesIssueNotificationPayload` already has a `FailureReason` field).
