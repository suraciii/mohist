...
The Issue projection already stores `ParentIssueNumber`, while the parent title and body remain authoritative in the parent Issue state. `WorkflowItemTranslator` renders a `WorkItem` into `WorkDispatch`; the runner poll route then maps that internal value to the HTTP `WorkDispatchResponse`, and the runner passes the rendered task prompt to `mohist/opencode` unchanged. No response field carries related Issue context.
...
- Carry that data as an optional, typed HTTP dispatch-response field and present it as clearly labelled read-only background before the existing task prompt.
...
Add a narrow `IssueQuerier` operation that accepts the current `(projectId, issueNumber)`, reads the child's projected `ParentIssueNumber`, and, when present, deserializes the referenced parent Issue state to return only `Title` and `Body`. The result is a small Issue-owned read value; it has no comments, attachments, artifacts, status, or child collection by construction.

The runner poll API calls this read operation only while assembling an eligible HTTP response. `WorkflowItemTranslator`, `WorkDispatch`, the Workflow domain, and `WorkflowRun.Metadata` receive no parent field. API is the existing composition boundary that already converts Runner output into the wire DTO, and it is outside the domain dependency matrix enforced by `ArchitectureRules`; this avoids introducing the forbidden `Runner -> Issue` dependency. Resolution occurs for each poll response, so a retry or later Plan task sees the parent's current title and body, and a child detached before a later response is treated as an ordinary issue.
...
- Calling `IssueQuerier` from `WorkflowItemTranslator` was rejected because the architecture allowlist permits `Runner -> Workflow` and `Runner -> Sessions`, but rejects `Runner -> Issue`.
...
### D2. Assemble a purpose-built optional field at the HTTP poll boundary

Keep the Orleans-serialized `WorkDispatch` unchanged. Extend only the plain HTTP `WorkDispatchResponse` with optional `ParentIssueContextResponse { title, body }`. The poll route maps the Issue-owned read value into that API-owned DTO, and the runner mirrors the field in `WorkDispatchResponse`, `RenderedWorkItem`, and `ActionContext`, mapping it through `ServerConnection.poll` and `WorkExecutor` without placing it under `with`, `variables`, or `prompts`.

The poll response mapper resolves and populates the field only when all of these are true:
...
Checks, non-Plan tasks, AgentJob dispatches, other actions, and ordinary issues retain `null`/absent context. This API-boundary gate is the single authority for applicability; the runner consumes the optional field without independently re-deriving Issue or stage rules. If no parent context resolves, the field is absent and existing dispatch behavior is preserved; corruption policy is outside this change.
...
- Placing the cross-domain assembler in AgentOps was rejected because AgentOps owns operator-facing read reports, not execution dispatch; the existing HTTP response mapper is the narrower composition boundary.
...
Server specs cover the narrow parent lookup and poll-response gating for child Plan `mohist/opencode`, ordinary Plan, child non-Plan, checks, and unrelated actions. They also assert that the HTTP context shape cannot carry comments or artifacts and that `WorkDispatch` remains unchanged. Runner tests cover poll mapping, `ActionContext` propagation, exact preservation when context is absent, and submitted prompt content/labels when context is present.
...
- `[Runner-side transport adds several pass-through fields] ->` Use one purpose-built value object and keep all applicability logic in the API response mapper, avoiding duplicated policy in each transport layer.
