...
- **Tests**: Add or update unit/integration tests for stage transitions, Check readiness dry runs, deterministic spec sync validation, archive ordering, no-agent-fix Integrate failures, merge/final health blocking, API state reporting, UI rendering of Integrate/Done evidence, and final T-010 regression coverage for the complete Check -> Integrate -> Done contract.
