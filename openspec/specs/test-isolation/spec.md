...
- StateManager constructor SHALL call `initializeDatabase(db)` internally to ensure schema is ready
...
- [ ] `new StateManager(db)` initializes database schema automatically
...
- Tests SHALL create their own `DatabaseManager` instances via `new DatabaseManager({ inMemory: true })`
- Tests using StateManager SHALL create `new StateManager(db)` with their own db instance
- Tests using repos directly SHALL call `initializeDatabase(db)` themselves
- Tests SHALL NOT share database connections with other tests
- Tests SHALL clean up via `db.close()` in `afterEach` blocks
...
- [ ] All 6 affected test files updated to use DI pattern
- [ ] 18 unaffected test files pass without any modifications
