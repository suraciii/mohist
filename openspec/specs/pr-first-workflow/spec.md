...
- **THEN** `mohist/rebase` SHALL return `output.errorCode: conflict` and SHALL leave the rebase in progress
- **AND** the profile SHALL execute `recover:resolve-rebase-conflicts` declared under `recover:rebase` recovery
