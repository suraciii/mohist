...
public class InboxQuerierTests
{
    [Fact]
    public async Task ListAsync_ReturnsNonArchivedItemsMostRecentFirst()
    {
        ...
        await SeedAsync(store, "proj_a", "evt-1", DateTimeOffset.UnixEpoch.AddMinutes(-30), 1, "Issue 1");
        await SeedAsync(store, "proj_a", "evt-2", DateTimeOffset.UnixEpoch.AddMinutes(-10), 2, "Issue 2");
        await SeedAsync(store, "proj_a", "evt-3", DateTimeOffset.UnixEpoch.AddMinutes(-20), 3, "Issue 3");
    ...
    public async Task ListAsync_ExcludesArchivedItems()
    {
        await using var database = CreateDatabase();
        var store = new InboxStore(database.Factory);
        var keep = await SeedAsync(store, "proj_a", "evt-keep", DateTimeOffset.UnixEpoch, 1, "Keep me");
        var drop = await SeedAsync(store, "proj_a", "evt-drop", DateTimeOffset.UnixEpoch, 2, "Drop me");
    ...
    public async Task ListAsync_OnlyReturnsItemsInRequestedProject()
    {
        await using var database = CreateDatabase();
        var store = new InboxStore(database.Factory);
        await SeedAsync(store, "proj_a", "evt-a-1", DateTimeOffset.UnixEpoch, 1, "A1");
        await SeedAsync(store, "proj_a", "evt-a-2", DateTimeOffset.UnixEpoch, 2, "A2");
        await SeedAsync(store, "proj_b", "evt-b-1", DateTimeOffset.UnixEpoch, 1, "B1");
        var querier = new InboxQuerier(database.Factory);
    ...
    public async Task ListAsync_ReturnsEmptyForUnknownProject()
    {
        await using var database = CreateDatabase();
        var store = new InboxStore(database.Factory);
        await SeedAsync(store, "proj_a", "evt-1", DateTimeOffset.UnixEpoch, 1, "A1");
        var querier = new InboxQuerier(database.Factory);
    ...
    public async Task ListAsync_ReflectsReadStateFromStore()
    {
        await using var database = CreateDatabase();
        var store = new InboxStore(database.Factory);
        var read = await SeedAsync(store, "proj_a", "evt-read", DateTimeOffset.UnixEpoch, 1, "Read");
        await store.MarkReadAsync("proj_a", read.Id);
        await SeedAsync(store, "proj_a", "evt-unread", DateTimeOffset.UnixEpoch.AddMinutes(1), 2, "Unread");
        var querier = new InboxQuerier(database.Factory);
    ...
    public async Task ListAsync_TieBreaksItemsWithIdenticalCreatedAtById()
    {
        ...
        var same = DateTimeOffset.UnixEpoch;
        await using var database = CreateDatabase();
