...
/// <summary>
/// Unit tests for <see cref="AgentUsageReporter"/>. API-level coverage lives
/// in <c>AgentCostRollupApiSpecs</c> and <c>AgentUsageTimeseriesApiSpecs</c>.
/// </summary>
public class AgentUsageReporterTests : IAsyncLifetime
{
    private readonly FakeTimeProvider _time = new(new DateTimeOffset(2026, 6, 30, 0, 0, 0, TimeSpan.Zero));
    private SqliteConnection _keeper = null!;
    private IDbContextFactory<MohistDbContext> _dbFactory = null!;
    private AgentUsageReporter _reporter = null!;

    public Task InitializeAsync()
    {
        var connectionString = $"Data Source=agent-usage-{Guid.NewGuid():N};Mode=Memory;Cache=Shared";
        ...
        var options = new DbContextOptionsBuilder<MohistDbContext>()
            ...
            .Options;
        _dbFactory = new DbFactory(options);
        var sessionQuery = new AgentSessionQuery(_dbFactory, _time);
        _reporter = new AgentUsageReporter(_dbFactory, sessionQuery, _time);
    ...
    private AgentUsageReporter ResolveReporter() => _reporter;
    ...
    private Task<ProjectDto> CreateProjectAsync() => Task.FromResult(
        new ProjectDto($"project-{Guid.NewGuid():N}", "", "", ""));
    ...
    private sealed class DbFactory(DbContextOptions<MohistDbContext> options) : IDbContextFactory<MohistDbContext>
    {
