...
public class ActionContractValidatorTests
{
    ...
    // =======================================================================
    // mohist/agent virtual Action (issue-131 T-001)
    // =======================================================================
    ...
    {
        // The runner never publishes mohist/agent, but a defensive catalog that
        // already includes it must be replaced by the server-owned manifest so
        // closed-key and template-on-name rules still hold.
        var catalog = new ActionCatalog(
    ...
    public void Validate_MohistAgentTask_MissingNameAndPrompt_ReportedTogether()
    {
        ...
        Assert.Contains(errors, e =>
            e.Path == "stages[0].tasks[0].with.name"
            && e.Message.Contains("missing required"));
        Assert.Contains(errors, e =>
    ...
    public void Validate_MohistAgentTask_TemplatePrompt_Accepted()
    {
        var task = new TaskDefinition("review",
            ...
            With: With(
                ("name", JsonString("reviewer")),
                ("prompt", JsonString("${{ prompts.review }}"))));
        var stage = new StageDefinition("build", [task], []);
        var definition = new WorkflowDefinition([stage]);
    ...
    {
        var check = new CheckDefinition("audit",
            Uses: "mohist/agent",
            With: With(("name", JsonString("reviewer")), ("prompt", JsonString("Audit this change."))));
        var stage = new StageDefinition("build", [], [check]);
        var definition = new WorkflowDefinition([stage]);
    ...
    {
        // Even with malformed inputs, the check rejection must surface first so
        // authors do not misread the failure as a typo in their with block.
        var check = new CheckDefinition("audit",
            Uses: "mohist/agent",
    ...
    {
        var innerTask = new TaskDefinition("recover-review",
            Uses: "mohist/agent",
    ...
    {
        var feedbackTask = new TaskDefinition("revise",
            Uses: "mohist/agent",
    ...
    {
        var agentTask = new TaskDefinition("review",
            Uses: "mohist/agent",
            With: With(("name", JsonString("reviewer")), ("prompt", JsonString("Audit this change."))));
        var inlineTask = new TaskDefinition("compile",
            Uses: "mohist/opencode",
