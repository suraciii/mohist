...
public class StageLockTests : WorkflowGrainTests
{
    public StageLockTests(WorkflowGrainFixture fixture) : base(fixture) { }
    ...
    public async Task SameProjectIntegrateStages_RunSequentiallyAcrossWorkflows()
    {
        ...
        var secondIntegrate = await Mohist.Server.UnitTests.Support.TestWait.ForAsync(
    ...
    private WorkflowStartInput ProjectInput(string projectId)
    {
    ...
    private async Task DispatchStageTerminalAsync(string workflowRunId, string type, string stage)
    {
        ...
        var stored = await Mohist.Server.UnitTests.Support.TestWait.ForAsync(
            async () =>
            {
