...
public class DatabaseInitializationTests
{
    ...
    public async Task Migrate_WhenEmptyDatabase_CreatesAllTables()
    {
        ...
        Assert.True(await TableExistsAsync(connection, "Attachments"));
        ...
        Assert.True(await IndexExistsAsync(connection, "IX_Attachments_ExpiresAt"));
        Assert.True(await IndexExistsAsync(connection, "IX_Attachments_ProjectId_Owner"));
        Assert.True(await TableExistsAsync(connection, "__EFMigrationsHistory"));
    }

    [Fact]
    public async Task Migrate_WhenAttachmentMigrationWasMissed_CreatesAttachments()
    {
        await using var connection = new SqliteConnection("Data Source=:memory:");
        await connection.OpenAsync();
        ...
        await using (var initialDb = new MohistDbContext(options))
        {
            await initialDb.GetService<IMigrator>()
                .MigrateAsync("20260618100150_AddAgentsTable");
        }

        await using var db = new MohistDbContext(options);
        Assert.Contains(
            "20260618112000_AddAttachments",
