...
namespace Mohist.Server.UnitTests.Agent.Grain;

[Collection("AgentJobGrain")]
public class AgentJobGrainTests
{
    private readonly AgentJobGrainFixture _fixture;

    public AgentJobGrainTests(AgentJobGrainFixture fixture)
    {
        _fixture = fixture;
    }
    ...
    public async Task SubmitAsync_GenericSession_NoEligibleRunner_ClosesSessionAsFailed()
    {
        ...
        var closedPayload = await WaitForAsync(
            async () =>
            {
                await using var db = TestDatabaseSchema.CreateDbContext(_fixture.ConnectionString);
...
public class AgentJobOptionsBindingTests
{
    private readonly WorkflowGrainFixture _fixture;

    public AgentJobOptionsBindingTests(WorkflowGrainFixture fixture)
    {
        _fixture = fixture;
    }
