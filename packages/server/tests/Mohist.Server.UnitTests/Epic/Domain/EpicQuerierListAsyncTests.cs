...
public class EpicQuerierListAsyncTests
{
    [Fact]
    ...
    public async Task ListAsync_AllArchivedLinkedIssues_YieldsZeroCounts()
    {
        await using var database = CreateDatabase();
        await SeedEpicAsync(database, "proj_1", "epic_1", 1);
        await SeedIssueAsync(database, "proj_1", "issue_1", 1, IssueStatus.Done, archivedAt: DateTime.UnixEpoch);
        await SeedLinkAsync(database, "epic_1", "issue_1", 1, "proj_1");
    ...
    {
        var now = updatedAt ?? DateTimeOffset.UnixEpoch;
