...
public class EventBridgeTests
{
    ...
    [Fact]
    public async Task EventBridge_ProjectScopedInboxHint_DeliversOnlyOwningAndUnscopedConnections()
    {
        const string inboxHint = "com.mohist.inbox.item-persisted";
        var registry = new ConnectionSubscriptionRegistry();
        foreach (var connectionId in new[] { "conn-A", "conn-B", "conn-cross" })
        {
            registry.RegisterConnection(connectionId);
            registry.Subscribe(connectionId, inboxHint);
        }
        ...
        await bridge.HandleAsync(
            BuildEvent(inboxHint, "/mohist/inbox", new Dictionary<string, string>
            {
                ["projectid"] = "proj-A",
            }),
        ...
        Assert.Equal(2, hub.Messages.Count);
        Assert.Contains(hub.Messages, message => message.ConnectionId == "conn-A");
        Assert.Contains(hub.Messages, message => message.ConnectionId == "conn-cross");
        Assert.DoesNotContain(hub.Messages, message => message.ConnectionId == "conn-B");
        var ownerMessage = Assert.Single(hub.Messages, message => message.ConnectionId == "conn-A");
        var envelope = Assert.IsType<CloudEventEnvelope>(ownerMessage.Data);
        Assert.Equal(inboxHint, envelope.Type);
