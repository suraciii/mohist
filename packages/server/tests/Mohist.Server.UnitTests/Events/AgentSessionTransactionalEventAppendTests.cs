...
public class AgentSessionTransactionalEventAppendTests : IAsyncLifetime
{
    ...
    public async Task SaveAsync_StampsSourceAndSubjectOnEveryLifecycleEnvelope()
    {
        ...
        await store.SaveAsync(session.Id, session, [
            ...
            new AgentSessionContextCompacted(null, null, null, "summary", "summary text", DateTime.UnixEpoch),
            new AgentSessionContextExhausted("context_exhaustion", 96d, 960, 1000, DateTime.UnixEpoch),
            new AgentSessionContextHealthUpdated("yellow", 65d, 650, 1000, DateTime.UnixEpoch),
        ]);
