...
public class JSONTests
{
    ...
    [Fact]
    public void ServerSource_NoLocalJsonSerializerOptionsConstructors()
    {
        // Source-grep guard from the json-serialization spec: business code SHALL
        // not construct its own JsonSerializerOptions. All options must come
        // from the JSON facade.
        var serverSrc = LocateServerSourceRoot();
        Assert.NotNull(serverSrc);

        var offenders = new List<string>();
        foreach (var path in Directory.EnumerateFiles(serverSrc!, "*.cs", SearchOption.AllDirectories))
        {
            var relative = Path.GetRelativePath(serverSrc!, path).Replace('\\', '/');
            if (relative.StartsWith("Infrastructure/JSON.cs", StringComparison.Ordinal))
            {
                continue;
            }
        ...
        Assert.True(
            offenders.Count == 0,
            "Found local JsonSerializerOptions construction outside the JSON facade: " + string.Join(", ", offenders));
    }

    [Fact]
    public void ServerSource_NoLocalStaticJsonSerializerOptionsFields()
    {
        // The audit table from design.md requires that local `static readonly
        // JsonSerializerOptions` fields are either gone or delegate to JSON.Options.
        // This guard ensures new fields are not silently re-introduced. Fields
        // that delegate to JSON.Options (a "thin delegator") are allowed.
        var serverSrc = LocateServerSourceRoot();
        Assert.NotNull(serverSrc);
        ...
        foreach (var path in Directory.EnumerateFiles(serverSrc!, "*.cs", SearchOption.AllDirectories))
        {
            ...
            foreach (var line in content.Split('\n'))
            {
                ...
                if (line.Contains("JSON.Options", StringComparison.Ordinal)
        ...
        Assert.True(
            offenders.Count == 0,
            "Found local static JsonSerializerOptions fields outside the JSON facade: " + string.Join(", ", offenders));
    }

    private static string? LocateServerSourceRoot()
    {
        // Walk up from the test assembly to find packages/server/src/Mohist.Server.
        var dir = new DirectoryInfo(AppContext.BaseDirectory);
        while (dir is not null)
        {
            var candidate = Path.Combine(dir.FullName, "packages", "server", "src", "Mohist.Server");
