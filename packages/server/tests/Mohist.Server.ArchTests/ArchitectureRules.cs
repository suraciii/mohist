...
public class ArchitectureRules
{
    ...
    public void DomainSubscriptionHandlers_ResideWithAssignedFeatureModule()
    {
        ...
        var assigned = new Dictionary<string, string[]>(StringComparer.Ordinal)
        {
            ...
            ["GitHub"] = ["GitHubIssueCommandHandler.cs", "GitHubIssueCloseHandler.cs", "GitHubIssueContentSyncHandler.cs", "GitHubIssueMirrorHandler.cs", "GitHubIssueReopenHandler.cs", "GitHubPullRequestReviewHandler.cs", "GitHubWriteBackHandler.cs"],
        };
    ...
    [
        ...
        ("Project", "Workflow"),
        // Sessions→Agent is the Session's own lifecycle coupling to its
        // launch and Job owners (AgentSessionGrain marks an unknown Job,
        // AgentSessionRetryService resolves the launch it retries). It is
        // not a capacity dependency: the permit gate that once forced this
        // edge is deleted, and admission is the derived capacity claim.
        ("Sessions", "Agent"),
    ];
