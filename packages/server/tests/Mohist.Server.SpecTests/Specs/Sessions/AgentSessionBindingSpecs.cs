...
public class AgentSessionBindingSpecs : AgentSessionTestSupport
{
    ...
    public async Task RunnerAttach_DifferentPhysicalSession_ReturnsConflictAndPreservesBinding()
    {
        ...
        await _client.PostOkAsync($"/api/runner/{_runnerId}/agent-sessions/{session.ProjectId}/{session.Id}/attach", new { runtimeSessionId = "runtime-1", runtime = "opencode", expectedRuntime = "opencode", expectedRuntimeSessionId = (string?)null, workDir = "/work", processPid = 1234 });

        using var response = await _client.PostAsJsonAsync($"/api/runner/{_runnerId}/agent-sessions/{session.ProjectId}/{session.Id}/attach", new { runtimeSessionId = "runtime-2", runtime = "opencode", expectedRuntime = "opencode", expectedRuntimeSessionId = "runtime-1", workDir = "/work", processPid = 1234 });
