...
public sealed class PhysicalSecretKeyFileTests
{
    ...
    [Fact]
    public async Task EnsureKeyAsync_AdoptsKeyCreatedConcurrently_WhenLosingCreateRace()
    {
        // Simulates two hosts initializing a missing key file at the same
        // time. This host checks (file missing), then another host persists
        // its key before this host's create-if-absent step runs. The loser
        // must adopt the winner's key rather than return its own freshly
        // generated bytes, so every host shares one master key.
        var ops = new FakeOperations { LoseExclusiveCreate = true };
        var winnerKey = NewKey(0xA4);
        ops.Seed(Path, winnerKey);
        var store = NewStore(ops);

        var key = await store.EnsureKeyAsync(Path);

        Assert.Equal(winnerKey, key);
        Assert.Empty(ops.Writes); // must not overwrite the winner's file
    }
    ...
    private sealed class FakeOperations : ISecretKeyFileOperations
    {
        ...
        /// <summary>
        /// When set, this host observes the key file as missing at the
        /// existence check but loses the create-if-absent race (another
        /// host persisted its key in the window). Used to exercise the
        /// concurrent-initialization convergence path.
        /// </summary>
        public bool LoseExclusiveCreate { get; set; }
        ...
        public bool FileExists(string path) => Exists(path) && !LoseExclusiveCreate;

        public bool TryCreateExclusive(string path, byte[] bytes, UnixFileMode ownerOnlyMode)
        {
            lock (_gate)
            {
                if (LoseExclusiveCreate || _files.ContainsKey(path))
                    return false;
                _files[path] = bytes;
