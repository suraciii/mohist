...
public class AgentSessionBindingSpecs : AgentSessionTestSupport, IClassFixture<DefaultMohistIntegrationFixture>
{
    ...
    public async Task AgentSessionOpen_ClosedRuntimeObservation_DoesNotRebindSession()
    {
        ...
        var reopened = await grain.OpenAsync(new OpenAgentSessionCommand(
            retryRunnerId,
            "opencode",
            Metadata: GenericAgentSessionMetadata.Metadata(new GenericAgentSessionContext(project.Id, "agent-1", "Agent One", session.IssueNumber))));
    ...
    public async Task OpenAgentSession_ExistingBoundSessionKeepsRuntimeBinding()
    {
        ...
        var opened = await _fixture.Grains.GetGrain<IAgentSessionGrain>(session.Id)
            .OpenAsync(new OpenAgentSessionCommand(
                _runnerId,
                "opencode",
                Metadata: GenericAgentSessionMetadata.Metadata(new GenericAgentSessionContext(project.Id, "agent-1", "Agent One", work.Issue!.IssueNumber))));
    ...
    public async Task OpenAgentSession_ClosedObservationKeepsRuntimeBinding()
    {
        ...
        var opened = await _fixture.Grains.GetGrain<IAgentSessionGrain>(session.Id)
            .OpenAsync(new OpenAgentSessionCommand(
                _runnerId,
                "opencode",
                Metadata: GenericAgentSessionMetadata.Metadata(new GenericAgentSessionContext(project.Id, "agent-1", "Agent One", issue.Number, Title: "Fix review findings"))));
