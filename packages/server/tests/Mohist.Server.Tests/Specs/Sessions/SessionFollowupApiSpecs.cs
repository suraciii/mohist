...
public class SessionFollowupApiSpecs
{
    ...
    private async Task<string> ResolveSessionIdAsync(string workflowRunId, string sessionName)
    {
        ...
        return await db.AgentSessionLabels
            .Where(label => label.Key == Mohist.Server.Workflow.Services.Sessions.AgentSessionQueryMetadataKeys.WorkflowRunId && label.Value == workflowRunId)
            .Join(db.AgentSessionLabels.Where(label => label.Key == Mohist.Server.Workflow.Services.Sessions.AgentSessionQueryMetadataKeys.SessionName && label.Value == sessionName),
                left => left.SessionId,
