...
[Trait("level", "L1")]
public class EpicApiSpecs : EpicApiTestSupport, IClassFixture<DefaultMohistIntegrationFixture>
{
    public EpicApiSpecs(DefaultMohistIntegrationFixture fixture) : base(fixture)
    {
    }
    ...
    public async Task EpicDetailRoute_ResolvesNumericReference()
    {
        var project = await _client.CreateProjectWithDefaultRepositoryAsync<ProjectDto>(
            "/api/projects",
            $"epic-numresolve-{Guid.NewGuid():N}",
            repoName: "main",
            gitUrl: $"file://{Guid.NewGuid():N}");
    ...
    public async Task EpicLookup_ByNumberRoute_UnknownNumberReturnsNotFoundEnvelope()
    {
        var project = await _client.CreateProjectWithDefaultRepositoryAsync<ProjectDto>(
            "/api/projects",
            $"epic-bynum-missing-{Guid.NewGuid():N}",
            repoName: "main",
            gitUrl: $"file://{Guid.NewGuid():N}");
    ...
    public async Task EpicPatch_UpdatesTitle()
    {
        var project = await _client.CreateProjectWithDefaultRepositoryAsync<ProjectDto>(
            "/api/projects",
            $"epic-patch-title-{Guid.NewGuid():N}",
            repoName: "main",
            gitUrl: $"file://{Guid.NewGuid():N}");
    ...
    public async Task EpicPatch_UpdatesDescription()
    {
        var project = await _client.CreateProjectWithDefaultRepositoryAsync<ProjectDto>(
            "/api/projects",
            $"epic-patch-desc-{Guid.NewGuid():N}",
            repoName: "main",
            gitUrl: $"file://{Guid.NewGuid():N}");
    ...
    public async Task EpicPatch_AdvancesUpdatedAt()
    {
        var project = await _client.CreateProjectWithDefaultRepositoryAsync<ProjectDto>(
            "/api/projects",
            $"epic-patch-updated-{Guid.NewGuid():N}",
            repoName: "main",
            gitUrl: $"file://{Guid.NewGuid():N}");
        var created = await _client.PostDataAsync<EpicDto>($"/api/projects/{project.Id}/epics", new { title = "Titled", description = "body", priority = "p2", projectId = project.Id });
    ...
    public async Task EpicPatch_PreservesStatus()
    {
        var project = await _client.CreateProjectWithDefaultRepositoryAsync<ProjectDto>(
            "/api/projects",
            $"epic-patch-status-{Guid.NewGuid():N}",
            repoName: "main",
            gitUrl: $"file://{Guid.NewGuid():N}");
        var created = await _client.PostDataAsync<EpicDto>($"/api/projects/{project.Id}/epics", new { title = "Titled", description = "body", priority = "p2", projectId = project.Id });
    ...
    public async Task EpicPatch_PartialUpdate_LeavesUnspecifiedFieldsUnchanged()
    {
        var project = await _client.CreateProjectWithDefaultRepositoryAsync<ProjectDto>(
            "/api/projects",
            $"epic-patch-partial-{Guid.NewGuid():N}",
            repoName: "main",
            gitUrl: $"file://{Guid.NewGuid():N}");
