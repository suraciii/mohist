...
public class AgentJobTerminalDeliverySpecs : AgentJobGrainTestSupport
{
    ...
    private async Task<global::Mohist.Server.Infrastructure.Data.Sessions.AgentSessionTranscriptPartRow> GetSingleSessionClosedPartAsync(string sessionId)
    {
        var parts = (await ListSessionClosedPartsAsync(sessionId))
            .Where(part => part.Type == TranscriptPartTypes.SessionActivity)
            .ToList();
    ...
    private async Task<List<global::Mohist.Server.Infrastructure.Data.Sessions.AgentSessionTranscriptPartRow>> ListSessionClosedPartsAsync(string sessionId)
    {
