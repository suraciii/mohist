...
namespace Mohist.Server.ComponentSpecs.Specs.Workflow.Grain;
...
public class StageLockSpecs : WorkflowGrainSpecs
{
    ...
    public async Task SameProjectIntegrateStages_RunSequentiallyAcrossWorkflows()
    {
        ...
        var secondIntegrate = await Mohist.Server.ComponentSpecs.Support.TestWait.ForAsync(
    ...
    private async Task DispatchStageTerminalAsync(string workflowRunId, string type, string stage)
    {
        ...
        var stored = await Mohist.Server.ComponentSpecs.Support.TestWait.ForAsync(
            async () =>
            {
