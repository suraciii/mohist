...
[Collection("MohistDb")]
public class IssueApprovalWaitQuerierSpecs
{
    private readonly MohistDbFixture _fixture;

    public IssueApprovalWaitQuerierSpecs(MohistDbFixture fixture)
    {
        _fixture = fixture;
    }
    ...
    [Fact]
    public async Task GetApprovalWaitAsync_UsesRequestedWindowLength()
    {
        using var scope = _fixture.Services.CreateScope();
        var db = scope.ServiceProvider.GetRequiredService<MohistDbContext>();
        var project = new ProjectInfo { Id = $"proj-approval-range-{Guid.NewGuid():N}", Name = "Approval Range" };
        var now = new DateTimeOffset(2026, 6, 19, 12, 0, 0, TimeSpan.Zero);
        SeedIssue(db, project, $"{project.Id}-issue", workflowRunId: "wr-approval-range");
        await db.SaveChangesAsync();
        await SeedWorkflowRunAsync(
            db,
            "wr-approval-range",
            ApprovalRunState("wr-approval-range", now.AddDays(-14), TimeSpan.FromHours(2)));

        var service = scope.ServiceProvider.GetRequiredService<IssueMetricsQuerier>();
        var defaultWindow = await service.GetApprovalWaitAsync(project.Id, now);
        var thirtyDayWindow = await service.GetApprovalWaitAsync(project.Id, now, windowDays: 30);

        Assert.Equal(0, defaultWindow.SampleCount);
        Assert.Equal(1, thirtyDayWindow.SampleCount);
        Assert.Equal(TimeSpan.FromHours(2).TotalSeconds, thirtyDayWindow.AverageSeconds);
    }
