...
namespace Mohist.Server.ComponentSpecs.Specs.Runner.SignalR;
...
public class RunnerHubSpecs
{
    ...
    private sealed class ThrowingGrainFactory : IGrainFactory
    {
        ...
        Orleans.Runtime.IAddressable IGrainFactory.GetGrain(GrainId grainId, GrainInterfaceType interfaceType)
            => throw new NotSupportedException();
        Orleans.Runtime.IAddressable IGrainFactory.GetGrain(Type grainInterfaceType, IdSpan grainKey, string? grainClassNamePrefix)
            => throw new NotSupportedException();
        Orleans.Runtime.IAddressable IGrainFactory.GetGrain(Type grainInterfaceType, IdSpan grainKey)
            => throw new NotSupportedException();
