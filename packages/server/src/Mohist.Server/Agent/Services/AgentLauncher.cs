...
public sealed class AgentLauncher : IAgentLauncher, IScopedService
{
    ...
    {
        // Definition-first launch resolves from the Agent definition alone —
        // the same two-term rule Readiness applies — so an Agent Readiness
        // reports launchable dispatches with the configuration Readiness
        // resolved. Resolution happens once, here, and the result is stamped
        // onto the durable launch snapshot: later Agent edits cannot change
        // an in-flight launch.
