...
public sealed record WorkflowWork
{
    ...
    public static WorkflowWork Task(string stage, string id, string title, string? uses, Dictionary<string, JsonElement?>? with, TaskArtifactCapture? artifacts = null, List<TaskOutputDefinition>? outputs = null, Dictionary<string, string>? setVars = null) => new(stage, "task", new TaskData(id, title, uses, with, artifacts, outputs, setVars));
    ...
    public sealed record TaskData(string Id, string Title, string? Uses, Dictionary<string, JsonElement?>? With, TaskArtifactCapture? Artifacts = null, List<TaskOutputDefinition>? Outputs = null, Dictionary<string, string>? SetVars = null);
    ...
    extension(WorkflowRun run)
    {
        public WorkflowWork? NextWork()
        {
            ...
            if (pendingTask is not null)
                return WorkflowWork.Task(current.Id, pendingTask.Id, pendingTask.Title, pendingTask.Uses, pendingTask.WithInput, pendingTask.Artifacts, pendingTask.Outputs, pendingTask.SetVars);
