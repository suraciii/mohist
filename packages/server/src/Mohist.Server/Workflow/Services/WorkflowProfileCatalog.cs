...
public static class WorkflowProfileCatalog
{
    ...
    private const string LocalDescription = "Default general-purpose Mohist pipeline: plan (Workspace plan artifacts) → build (task list, verification) → check (review evidence, merge readiness) → integrate (rebase, squash, push).\nRequires approval at the plan and check stages, then squashes and pushes the working branch directly into the repository base branch.\nTypical duration: 20-60 minutes for a focused change.\nNot suited for: trivial one-line fixes, throwaway spikes, or quick experiments — these don't warrant a full plan-check-integrate cycle.";
    private const string GithubPrDescription = "Default general-purpose Mohist pipeline that delivers through a GitHub PR: plan (Workspace plan artifacts, draft PR) → build (task list, verification) → check (review evidence, publish, PR checks) → integrate (GitHub auto-merge).\nRequires approval at the plan and check stages. The workflow opens a draft PR after planning, marks it ready after check approval, and enables squash auto-merge during integration.\nTypical duration: 20-60 minutes for a focused change.\nChoose this over mohist/local when you want each issue to ship as a reviewable, traceable GitHub PR.\nNot suited for: trivial one-line fixes, throwaway spikes, or quick experiments — these don't warrant a full plan-check-integrate cycle.\nRequires the `gh` CLI on the runner host, authentication, and repository auto-merge support.";
