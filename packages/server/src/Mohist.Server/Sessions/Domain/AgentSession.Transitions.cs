    ...
    extension(AgentSession session)
    {
            ...
            DateTime now)
        {
            ...
            for (var index = 0; index < turns.Count; index++)
            {
                ...
                // The original turn remains interrupted history. Recovery
                // transitions belong to the replacement turn once it exists.
                if (transition.State == AgentWorkInterruptionStates.Recovering && isOriginal)
                    continue;
