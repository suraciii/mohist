...
class FakeSession implements PiSdkSession {
  ...
  prompt(text: string, options?: PiPromptOptions): Promise<void> {
    ...
    return new Promise<void>((resolve) => {
      this.completions.push({ resolve, text })
    })
  }
  ...
  abort(): Promise<void> {
    ...
    this.emit({ type: 'agent_settled', id: `settled-${this.abortCalls}` })
    return Promise.resolve()
  }
  ...
  complete(content = 'final answer'): void {
    ...
    if (head) {
      this.isStreaming = this.completions.length > 0
      head.resolve()
    }
  }
...
describe('PiRuntime', () => {
  ...
  it('creates a physical binding and runs a literal prompt with per-turn selection', async () => {
    ...
    const created = await runtime.createSession({
      target: { runtime: 'pi', runtimeSessionId: null, workDir: '/workspace' },
    })
    ...
    const resultPromise = runtime.runTurn(
      {
        ...
        options: { model: 'provider/family/model', variant: 'high' },
      },
      new AbortController().signal,
    )
    await Promise.resolve()
    ...
    await expect(resultPromise).resolves.toMatchObject({
      ok: true,
      value: { facts: { finalAssistantText: 'answer', runtimeSessionId: '/virtual/sessions/one.jsonl' } },
    })
  })
  ...
  it('restores the exact bound path and never replays after a late completion', async () => {
    ...
    services.create = async (options) => {
      ...
      const open = value.openSession
      value.openSession = async (path, cwd) => {
        opened = path
        return open(path, cwd)
      }
      return value
    }
    ...
    const resultPromise = timedRuntime.runTurn(
      {
        target: { runtime: 'pi', runtimeSessionId: session.sessionFile, workDir: '/workspace' },
        prompt: 'once',
        durationMs: 100,
  ...
  it('steers once at minute 55 and warns immediately for a five minute turn', async () => {
    ...
    const short = shortRuntime.runTurn(
      {
        target: { runtime: 'pi', runtimeSessionId: shortSession.sessionFile, workDir: '/workspace' },
        prompt: 'short',
        durationMs: 5 * 60_000,
      },
      new AbortController().signal,
  ...
  it('does not replace a missing binding and aborts provider exhaustion without leaking credentials', async () => {
    ...
    const turn = runtime.runTurn(
      { target: { runtime: 'pi', runtimeSessionId: session.sessionFile, workDir: '/workspace' }, prompt: 'provider' },
      controller.signal,
      {
        onEvent: (event) => {
          events.push(event)
  ...
  it('followup projects turn events through the observer until prompt resolves', async () => {
    ...
    expect(
      events.some(
        (event) =>
          (event as { type?: unknown }).type === 'tool_call.completed' &&
          (event as { payload?: { toolCallId?: string } }).payload?.toolCallId === 'tool-2',
      ),
    ).toBe(false)
  })
...
describe('Pi policy and projection', () => {
  it('validates startup policy and appends patterns', () => {
    const parsed = parseProviderErrorPolicy({
      MOHIST_PROVIDER_ERROR_PATTERNS: '["sentinel"]',
      MOHIST_PROVIDER_RETRY_THRESHOLD: '2',
    })
    expect(parsed.ok).toBe(true)
    if (parsed.ok) {
      expect(parsed.value.consecutiveRetryThreshold).toBe(2)
      expect(parsed.value.nonRecoverablePatterns).toHaveLength(14)
    }
  ...
  it('normalizes stable facts, deduplicates callbacks, reconciles final messages, and diagnoses unknown events', () => {
    ...
    expect(projector.project({ type: 'compaction_start', id: 'compact-1' })[0]).toMatchObject({
      type: 'compaction',
