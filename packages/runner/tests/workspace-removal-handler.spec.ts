...
const removalTestRuntime = vi.hoisted(() => {
  type State = { readonly mocks: { deleteDirectory: ReturnType<typeof vi.fn>; validateWorkspaceIdentity: ReturnType<typeof vi.fn> } }
...
describe("RemoveWorkspace removal fence", () => {
  ...
  ] as const)("returns cleanup failure and does not enter the callback when the fence is %s", async (kind, message) => await withRemovalMocks(async () => {
