...
describe('mohist/openspec-tasks', () => {
  ...
  it('OpenSpecTaskWithoutExplicitPrompt_PromptLoaderSpecResolvesThroughRegisteredLoader', async () => {
    ...
    const renderedWith = renderTemplate(loadedWith, {
      prompts: { build: '<base>build instructions</base>' },
        ...
        tasks: [
          {
            ...
            description:
              "Prepend a YAML block. bodies must remain byte-identical so the runner's ${{ prompts.xxx }} resolution is unaffected.",
