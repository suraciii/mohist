...
describe('ServerConnection.report', () => {
  ...
  // issue-559 T-003: an AgentJob work report — including the typed
  // workflow completion evaluation embedded in the agent output — never
  // rides the Workflow task-report channel. The agent-job report carries
  // the owner kind and job identity without a workflowRunId, so the
  // Workflow task-report payload never encodes agent execution facts.
  it('reportsAgentJobWorkWithoutTheWorkflowTaskReportChannel', async () => {
    fetchMock.mockResolvedValueOnce(mockResponse({ status: 200, body: '{}' }))
    const connection = new ServerConnection(options())
    const work = {
      workflowRunId: 'wf-agent-handoff',
      workId: 'agent-work-1',
      workType: 'agent-job',
      ownerKind: 'agent-job',
      agentJobId: 'job-1',
    ...
    expect(body.agentJobId).toBe('job-1')
    expect(body).not.toHaveProperty('workflowRunId')
    // The typed evaluation rides the agent result on the agent-job report
    // channel; the Workflow task-report payload is not involved.
    expect(body.output.expectation.satisfied).toBe(true)
    expect(body.output.expectation.matched).toBe('<promise>done</promise>')
  })
})
