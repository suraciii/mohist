...
describe('ServerConnection.workflowRunsStatus', () => {
  ...
  it('Throws_OnNonOkResponse', async () => {
    ...
    await expect(makeConnection().workflowRunsStatus(['wr-1'], new AbortController().signal)).rejects.toMatchObject({
      operation: 'workflowRunsStatus',
      kind: 'http',
      httpStatus: 500,
  ...
  it('ThrowsTypedProtocolError_OnMalformedSuccess', async () => {
    ...
    await expect(makeConnection().workflowRunsStatus(['wr-1'], new AbortController().signal)).rejects.toMatchObject({
      operation: 'workflowRunsStatus',
      kind: 'protocol',
      safeMessage: 'workflowRunsStatus returned malformed JSON',
    } satisfies Partial<RunnerTransportError>)
  })
