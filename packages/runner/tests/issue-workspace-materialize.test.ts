...
import { currentRunnerFileSystem, type RunnerResourceContext } from '../src/system/filesystem.js'
...
describe('materializeIssueWorkspace', () => {
    ...
    vitestIt(name, async () => {
      ...
      await withTestRunnerResources(async (fileSystem) => {
        const state = {
          ...
          commandBehavior: async (): Promise<FakeCommandResult> => ({
            success: true,
            stdout: '',
            stderr: '',
            exitCode: 0,
  ...
  function installFakeGit() {
    currentState().commandBehavior = async (command, args, cwd) => {
      ...
      if (command === 'git' && gitArgs[0] === 'clone') {
        const destination = gitArgs.at(-1)
        if (destination) await currentRunnerFileSystem().ensureDir(join(destination, '.git'))
      }
      ...
      if (command === 'git' && gitArgs[0] === '-C' && gitArgs[2] === 'rev-parse' && gitArgs[3] === '--verify') {
        ...
        return {
          ...
          exitCode: 128,
          combinedOutput: 'fatal: needed a single revision',
        }
      }

      if (command === 'git' && gitArgs[0] === '-C' && gitArgs[2] === 'rev-parse' && gitArgs[3] === '--abbrev-ref') {
        return {
          success: true,
          stdout: `${gitArgs[1]?.includes('issue-99') ? 'mohist/ws-issue-99' : RUN_BRANCH}\n`,
          stderr: '',
          exitCode: 0,
          combinedOutput: '',
        }
  ...
  it('clones the repository when directory does not exist', async () => {
    ...
    expect(marker!.repositories).toEqual([{ name: REPOSITORY_NAME, gitUrl: GIT_URL }])
    await expect(currentRunnerFileSystem().lstat(join(result.path, 'REPOS', REPOSITORY_NAME))).resolves.toBeDefined()
    await expect(currentRunnerFileSystem().lstat(join(result.path, 'PLANS'))).resolves.toBeDefined()
    await expect(currentRunnerFileSystem().lstat(join(result.path, 'RESEARCH'))).resolves.toBeDefined()
    await expect(currentRunnerFileSystem().lstat(join(result.path, '.scratch'))).resolves.toBeDefined()
  ...
  it('fails closed when an existing checkout has residual Git state', async () => {
    ...
    ).rejects.toThrow(/residual Git operation state/)
  })
    ...
    currentState().commandBehavior = async (_command, args) => {
      ...
      if (gitArgs[0] === 'clone') {
        return {
          ...
          exitCode: 128,
          combinedOutput: 'connection refused',
        }
      }
  ...
  it('parallel issues have disjoint directories with independent clones', async () => {
    ...
    currentState().commandBehavior = async (command, args, cwd) => {
      ...
      if (command === 'git' && gitArgs[0] === '-C' && gitArgs[2] === 'rev-parse' && gitArgs[3] === '--abbrev-ref') {
        const branch = gitArgs[1]?.includes('issue-99') ? 'mohist/ws-issue-99' : 'mohist/ws-issue-42'
    ...
    expect(cloneCalls[0]!.args.at(-1)).toContain('issue-42')
    expect(cloneCalls[1]!.args.at(-1)).toContain('issue-99')
    expect(cloneCalls[0]!.args.at(-1)).not.toBe(cloneCalls[1]!.args.at(-1))
