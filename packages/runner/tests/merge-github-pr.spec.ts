...
describe('mohist/merge-github-pr action', () => {
  ...
  it('uses the explicitly declared repository despite different Variables', async (resources) => {
    ...
    const result = await callAction(
      mergeGitHubPrAction,
      context(
        { repositoryUrl: 'https://github.com/acme/repo.git', prNumber: 42, method: 'squash', subject: 'Issue title' },
  ...
  it('rejects an invalid explicit repository URL', async (resources) => {
    const result = await callAction(
      mergeGitHubPrAction,
      context({ repositoryUrl: 'not a Git URL', prNumber: 42, method: 'squash', subject: 'Issue title' }),
    )
  ...
  it('forwards gh command output to the task log sink', async (resources) => {
    ...
    const result = await callAction(
      mergeGitHubPrAction,
      withLog(context({ prNumber: 42, method: 'squash', subjectFrom: 'issue.title' }), writes),
    )

    expect(result.error).toBeUndefined()
    expect(
      writes.some((write) => write.source === 'action:merge-github-pr' && write.text.includes('gh pr merge 42')),
    ).toBe(true)
  })
  ...
  it('fails with errorCode pr-checks-failed when a check is FAIL/CANCELLED/ACTION_REQUIRED and skips the merge call', async (resources) => {
    ...
    const failureCases: string[] = ['FAILURE', 'CANCELLED', 'ACTION_REQUIRED']
