...
describe('Pi runtime projector protocol', () => {
  ...
  it('maps the Pi tool execution lifecycle to started, updated, and completed events', () => {
    ...
    expect(started[0]).toMatchObject({
      ...
      payload: { toolCallId: 'call-1', status: 'running', rawInput: { path: 'README.md' } },
    })
    expect(updated[0]).toMatchObject({
      type: 'tool_call.updated',
      payload: { toolCallId: 'call-1', status: 'running', rawOutput: 'partial' },
    })
    expect(completed[0]).toMatchObject({
      type: 'tool_call.completed',
      payload: { toolCallId: 'call-1', status: 'completed', rawOutput: 'contents' },
    })
  ...
  it('reconciles a final assistant snapshot without duplicating streamed text', () => {
    ...
    const facts = projector.reconcile([
      { role: 'assistant', content: [{ type: 'text', text: 'hello world' }], usage: { input: 1 } },
    ])
    ...
    expect(facts).toContainEqual(
      expect.objectContaining({
        type: 'message.delta',
        payload: { text: ' world', partId: 'assistant-1:message.delta:0', messageId: 'assistant-1' },
      }),
    )
  ...
  it('preserves unknown cost and reports an explicit zero cost', () => {
    ...
    const zeroFacts = zero.reconcile([
      { role: 'assistant', content: [], usage: { cost: { total: 0, currency: 'USD' } } },
    ])
    expect(zeroFacts).toContainEqual(
      expect.objectContaining({
        type: 'usage.updated',
