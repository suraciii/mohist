...
describe('WorkExecutor artifact capture', () => {
  ...
  it('completesTaskAndIncludesUploadIdsWhenAllDeclaredArtifactsExist', async (workDir) => {
    ...
    const result = await executor.execute(
      buildWork(workDir, { files: [{ path: 'review.md' }, { path: 'design.md' }] }),
      new AbortController().signal,
    )
  ...
  it('declaredArtifactMissingIsNonFatalWarning', async (workDir) => {
    ...
    const result = await executor.execute(
      buildWork(workDir, { files: [{ path: 'missing.md' }] }),
      new AbortController().signal,
    )
  ...
  it('declaredArtifactUploadFailureIsNonFatalWarning', async (workDir) => {
    ...
    const result = await executor.execute(
      buildWork(workDir, { files: [{ path: 'review.md' }] }),
      new AbortController().signal,
    )
  ...
  it('declaredDirectoryExceedsLimitsIsNonFatalWarning', async (workDir) => {
    ...
    const result = await executor.execute(
      buildWork(workDir, { files: [{ path: 'specs' }] }),
      new AbortController().signal,
    )
  ...
  vitestIt(
    'keeps explicit repository working-directory execution on the held directory after replacement',
    async () => {
      const fileSystem = new MemoryDirectoryHandleFileSystem()
      const workDir = '/virtual/executor-repository-swap'
      const repository = join(workDir, 'REPOS', 'main')
      const heldRepository = join(workDir, 'REPOS', 'main-held')
      const outside = '/virtual/outside-repository'
      await fileSystem.ensureDir(repository)
      await fileSystem.ensureDir(outside)
      const work = buildWork(workDir, null)
      work.with = { 'working-directory': 'REPOS/main' }
      let actionWorkDir = ''
      const executor = new WorkExecutor(
        makeRegistry(async (_inputs, host) => {
          actionWorkDir = host.workDir
          await fileSystem.rename(repository, heldRepository)
      ...
      const result = await withTestRunnerResources(
        async () => await executor.execute(work, new AbortController().signal),
        { fileSystem, gitRunner: nonGitRunner },
      )

      expect(result.status).toBe('completed')
      expect(actionWorkDir).toMatch(/\/memory-handle-\d+$/)
      expect(fileSystem.exists(join(heldRepository, 'written.txt'))).toBe(true)
      expect(fileSystem.exists(join(outside, 'written.txt'))).toBe(false)
    },
  )
