...
describe('captureOne', () => {
  ...
  it('refusesPathsEscapingWorkspace', async () => {
    await expect(captureOne(paths().workDir, { path: '../escape.md', source: 'declared' })).rejects.toThrow(
      /escapes the workspace/,
    )
    await expect(captureOne(paths().workDir, { path: '/etc/passwd', source: 'declared' })).rejects.toThrow(
      /escapes the workspace/,
    )
  })
  ...
  it('enforcesFileCountLimitForDirectory', async () => {
    ...
    await expect(captureOne(paths().workDir, { path: 'specs', source: 'declared' }, limits)).rejects.toThrow(
      /file limit/,
    )
  })
  ...
  it('enforcesSingleFileSizeLimit', async () => {
    ...
    await expect(captureOne(paths().workDir, { path: 'big.bin', source: 'declared' }, limits)).rejects.toThrow(
      /single-file limit/,
    )
  })
...
describe('captureArtifacts', () => {
  ...
  it('missingDeclaredArtifact_ProducesFailureWithDeclaredSource', async () => {
    ...
    expect(outcome.failures).toEqual([expect.objectContaining({ path: 'review.md', source: 'declared' })])
  })
})
...
describe('uploadCapturedArtifacts', () => {
  it('uploadsEachCaptureAndReturnsUploadIds', async () => {
    ...
    const result = await uploadCapturedArtifacts(
      connection,
      'wf-1',
      'work-1',
      outcome.captures,
