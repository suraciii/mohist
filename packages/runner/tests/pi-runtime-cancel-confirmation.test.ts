...
class FakeSession implements PiSdkSession {
  ...
  private readonly subscription = new Promise<void>((resolve) => {
    this.resolveSubscription = resolve
  })

  subscribe(listener: (event: unknown) => void): () => void {
    this.listeners.add(listener)
    this.resolveSubscription?.()
    this.resolveSubscription = null
...
describe('PiRuntime cancel confirmation', () => {
  ...
  it('does not confirm an already-idle Pi session as a stopped turn', async () => {
    const session = new FakeSession()
    session.isStreaming = false
    ...
    await expect(
      runtime.cancel({ target: { runtime: 'pi', runtimeSessionId: session.sessionFile, workDir: '/workspace' } }),
    ).resolves.toMatchObject({ ok: true, value: { cancelled: true, stopConfirmed: false } })
  })
