...
describe('provisionNamedWorkspaceHome', () => {
  ...
  it('keeps an existing directory and reports created=false on re-provisioning', async () => {
    ...
    const second = await provisionNamedWorkspaceHome({
      runnerRoot: root,
      projectId: 'mohist',
      workspaceName: 'pay',
      registry,
  ...
  it('rejects a symlinked directory through the managed-path walk', async () => {
    ...
    await expect(
      provisionNamedWorkspaceHome({ runnerRoot: root, projectId: 'mohist', workspaceName: 'pay', registry }),
  ...
  it('writes the marker inside .mohist/workspace.json', async () => {
    ...
    const raw = JSON.parse(
      (await readFile(namedWorkspaceMarkerPath(namedWorkspacePath(root, 'mohist', 'pay')), 'utf8')) as string,
    )
