...
describe('CleanupLoop', () => {
  ...
  describe('resolution pass', () => {
    ...
    it('resolves a marker-mismatch eligible entry to stuck and leaves the directory intact', async () => {
      ...
      const result = await fixture.expectWarnings(
        [
          `workspace cleanup: refused to remove ${path} — workspace identity (wr-other-run) does not match registry (ws:project-1:wr-mismatch)`,
        ],
        () => fixture.loop.runOnce(policy, new AbortController().signal),
      )
  ...
  describe('safeRemove', () => {
      ...
      expect(capturedLogs()).toEqual(
        expect.arrayContaining([
          expect.objectContaining({
            level: 'ERROR',
            message: 'workspace cleanup failed to remove path',
            fields: expect.objectContaining({
              path,
              exception: expect.objectContaining({ name: 'Error', message: `stub delete failed: ${path}` }),
            }),
          }),
        ]),
      )
    ...
    it('marker mismatch aborts before deletion', async () => {
      ...
      const removed = await fixture.expectWarnings(
        [
          `workspace cleanup: refused to remove ${path} — workspace identity (wr-other) does not match registry (ws:project-1:wr-1)`,
        ],
        () => fixture.loop.safeRemove(entry),
      )
