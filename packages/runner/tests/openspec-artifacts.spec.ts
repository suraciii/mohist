...
describe("mohist/openspec-artifacts", () => {
  ...
  it("returns success and lists zero missing artifacts when all four plan artifacts exist", async () => {
    ...
    await currentRunnerFileSystem().ensureDir(join(changeDir, "specs", "pr-first-workflow"))
    await currentRunnerFileSystem().writeText(join(changeDir, "proposal.md"), "proposal\n")
    await currentRunnerFileSystem().writeText(join(changeDir, "specs", "pr-first-workflow", "spec.md"), "spec\n")
    await currentRunnerFileSystem().writeText(join(changeDir, "design.md"), "design\n")
    await currentRunnerFileSystem().writeText(join(changeDir, "tasks.json"), JSON.stringify({ tasks: [] }))
  ...
  it("returns failure listing only the specs directory when specs/ is missing", async () => {
    const workDir = await createTestTempDir("mohist-openspec-artifacts-")
    const changeDir = join(workDir, "openspec", "changes", "issue-270")
    await currentRunnerFileSystem().ensureDir(changeDir)
    await currentRunnerFileSystem().writeText(join(changeDir, "proposal.md"), "proposal\n")
    await currentRunnerFileSystem().writeText(join(changeDir, "design.md"), "design\n")
    await currentRunnerFileSystem().writeText(join(changeDir, "tasks.json"), JSON.stringify({ tasks: [] }))
  ...
  it("returns failure when a required directory is present as a file", async () => {
    ...
    await currentRunnerFileSystem().ensureDir(changeDir)
    await currentRunnerFileSystem().writeText(join(changeDir, "proposal.md"), "proposal\n")
    await currentRunnerFileSystem().writeText(join(changeDir, "specs"), "not a directory\n")
    await currentRunnerFileSystem().writeText(join(changeDir, "design.md"), "design\n")
    await currentRunnerFileSystem().writeText(join(changeDir, "tasks.json"), JSON.stringify({ tasks: [] }))
  ...
  it("returns failure when a required file is present as a directory", async () => {
    const workDir = await createTestTempDir("mohist-openspec-artifacts-")
    const changeDir = join(workDir, "openspec", "changes", "issue-270")
    await currentRunnerFileSystem().ensureDir(join(changeDir, "specs"))
    await currentRunnerFileSystem().ensureDir(join(changeDir, "proposal.md"))
    await currentRunnerFileSystem().writeText(join(changeDir, "design.md"), "design\n")
    await currentRunnerFileSystem().writeText(join(changeDir, "tasks.json"), JSON.stringify({ tasks: [] }))
