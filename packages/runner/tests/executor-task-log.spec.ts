...
const withTaskLogResources = <T>(
  ...
  return withTestRunnerResources(async () => await body('/virtual/task-log-executor', processSpawner), {
    gitRunner,
...
describe('WorkExecutor forwards action output to the task log', () => {
  const it = (name: string, body: (workDir: string, processSpawner: FakeProcessSpawner) => Promise<void>) =>
    vitestIt(name, () => withTaskLogResources(body))
  ...
  it('ProjectsCoreProcessOutputThroughSetVars', async (workDir, processSpawner) => {
    ...
    const executor = new WorkExecutor(
      ...
      {
        patchRunVars: async (_workflowRunId: string, vars: Record<string, unknown>) => {
          patches.push(vars)
        },
      } as never,
      workDir,
  ...
  it('DeliberatelyFailingOpsCommandOutputReachesCollectorBuffer', async (workDir) => {
    ...
    const lines = collector
      .flush()
      .entries.filter((e) => e.source === 'action:rebase')
      .map((e) => e.text)
  ...
  it('RegistersRuntimeConfiguredSecretsBeforeBuffering', async (workDir) => {
    ...
    const { collector } = await withTaskLogResources(async () => await runWith(registry, workDir), nonGitRunner, {
      [secretName]: secret,
    })
