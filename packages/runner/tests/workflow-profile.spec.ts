...
import { withTestRunnerResources, currentTestResourceState } from './support/test-resources.js'
...
for (const [profileId, path] of Object.entries(profileFiles)) {
  describe(`${profileId} workflow profile`, () => {
    ...
    it('build stage lanes carry the exact command lines with the .NET runtime prelude', async () => {
      ...
      expect(dotnet.indexOf('export DOTNET_ROOT=/home/user/.dotnet')).toBeGreaterThanOrEqual(0)
      expect(dotnet.indexOf('export DOTNET_ROOT=/home/user/.dotnet')).toBeLessThan(
  ...
  it('the .NET lane shell sees DOTNET_ROOT before dotnet invokes', async () => {
    ...
    await withTestRunnerResources(
      async () => {
        ...
        expect(lines[0]).toBe('export DOTNET_ROOT=/home/user/.dotnet')
        ...
        expect(script.indexOf('export DOTNET_ROOT=/home/user/.dotnet')).toBeLessThan(script.indexOf('dotnet test'))
      },
      {
        commandRunner: {
