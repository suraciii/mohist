...
type WorkOverrides = Partial<
  Pick<DispatchWorkItem, 'workflowRunId' | 'workId' | 'uses' | 'ownerKind' | 'agentJobId'> & { actionWorkId: string }
>
...
export function taskLogWork(overrides: WorkOverrides = {}): DispatchWorkItem {
  ...
  return {
    ...
    ...(overrides.ownerKind === 'agent-job' ? { agentJobId: overrides.agentJobId ?? 'aj-336' } : {}),
    projectId: 'project-1',
    ...
    variables: {
      ...
      repository: {
        ...
        baseBranch: 'main',
        name: 'master',
        remoteFingerprint: 'fake-fingerprint',
