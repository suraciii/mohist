...
import {
  createOpenCodeFetch,
  createSpawnedOpencodeServer,
  terminateOpencodeTree,
} from '../src/runtime/opencode/server-process.js'
...
describe('createOpenCodeFetch', () => {
  it('bounds a hanging dispatcher close and destroys it after the deadline', async () => {
    ...
    try {
      ...
      const pending = terminateOpencodeTree({ close: serverClose }, { close, destroy } as unknown as Dispatcher, 25)
