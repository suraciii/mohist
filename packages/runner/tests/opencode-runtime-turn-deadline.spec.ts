...
import { OpenCodeRuntime, type RuntimeProviderErrorPolicy } from '../src/runtime/opencode/index.js'
import { CLEANUP_OPERATION_TIMEOUT_MS, DEADLINE_WARNING_TEXT, WARNING_WINDOW_MS } from '../src/runtime/opencode/turn.js'
...
function buildRuntime(args: BuildArgs = {}): BuildResult {
  ...
  const sessionCreate = vi.fn(async (params: { directory?: string; model?: unknown }) => {
    const id = args.createId
      ? args.createId(params)
      : `ses_${(params.directory ?? 'default').replace(/[^a-z0-9]+/gi, '_')}`
    return { data: { id } }
  })
...
describe('OpenCodeRuntime.runTurn — warning injection failure is non-fatal', () => {
  it('Swallows a rejected promptAsync and emits exactly one info-level diagnostic, leaving the turn to complete', async () => {
    ...
    try {
      ...
      const sameCodeCount = result.value.diagnostics.filter(
        (d) => d.code === 'deadline-warning-injection-failed',
      ).length
