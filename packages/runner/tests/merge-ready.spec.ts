...
function installGit(
  resources: MergeReadyTestResources,
  respond: (
    call: WorkspaceCall,
    history: WorkspaceCall[],
  ) =>
...
describe('mohist/merge-ready (ref-safe, is-ancestor)', () => {
  ...
  it('ExplicitRemoteOverridesDefault', async (resources) => {
    ...
    const result = await callAction(
      mergeReadyAction,
      context({ remote: 'upstream' }, { repository: { gitUrl: 'https://example.com/repo.git', baseBranch: 'main' } }),
    )
    const output = result.output as Record<string, unknown>
  ...
  it('MissingSource_DoesNotUseWorkspaceVariableOrRunGit', async (resources) => {
    ...
    const result = await callAction(
      ...
      context(
        { source: null },
        {
          workspace: { name: 'issue-9', branch: 'different-branch', changeDir: null },
