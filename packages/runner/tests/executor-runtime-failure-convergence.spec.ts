...
function createExecutor(
  runtime: unknown,
  connection: unknown,
  outbox = makeRecordingOutbox(),
  executionWorkDir = workDir,
  piRuntime: unknown = null,
) {
...
describe('WorkExecutor runtime failure convergence', () => {
      ...
      async runTurn() {
        return {
          ...
          error: {
            ...
            diagnostics: [
              { severity: 'error' as const, code: 'abort-unconfirmed', message: 'Pi did not confirm stop' },
            ],
          },
  ...
  it('creates a fresh physical session before retrying a failed binding and satisfies the task artifact', async () => {
    ...
    const resetWorkflowAgentSession = vi.fn(
      async (
        _projectId: string,
        ...
        _body: unknown,
        _signal: AbortSignal,
      ) => ({ runtimeSessionId: 'runtime-new', workDir: retryWorkDir }),
    )
    const runTurn = vi.fn(async (request: { target: { runtimeSessionId: string | null; workDir: string } }) => {
      ...
      return {
        ...
        value: {
          facts: {
            finalAssistantText: 'proposal written',
