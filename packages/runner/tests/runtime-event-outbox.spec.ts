...
describe('AgentSessionRuntimeEventOutbox — durable storage', () => {
  ...
  it('persists one-event entries with a local record ID, binding-free target, runtime session id, payload, acknowledgement policy, and sequence position', async () => {
    ...
    await outbox.enqueueProducedFact(
      inputRecord({ id: 'evt_2', event: { type: 'message.delta', payload: { text: 'x' } } }),
    )
...
describe('AgentSessionRuntimeEventOutbox — acknowledgement policies', () => {
      ...
      deliver: {
        async send(record) {
          return [
            {
              type: record.event.type,
              inputDeliveryId: 'other-delivery',
              agentTurnId: 'turn-1',
              agentSessionId: 'agent-session-1',
            },
          ]
  ...
  it('confirms a matching input as already consumed only after two consecutive empty responses', async () => {
    ...
    const persisted = JSON.parse(
      (outbox as unknown as { fileSystem: { body(path: string): string | null } }).fileSystem.body(
        RUNTIME_EVENT_OUTBOX_FILE,
      ) ?? '{}',
    ) as { entries: unknown[] }
  ...
  it('keeps the empty confirmation and waiter pending when terminal settlement persistence fails', async () => {
    ...
    expect(JSON.parse(fileSystem.body(RUNTIME_EVENT_OUTBOX_FILE) ?? '{}')).toMatchObject({
      entries: [{ id: 'persist-failure-empty' }],
    })
  })
  ...
  it('matching-receipt retains the head on timeout, transport failure, non-2xx, malformed, empty, or receipt without the submitted type', async () => {
    const cases: Array<{
      name: string
      response?: () => AgentSessionRuntimeEventReceipt[] | Promise<AgentSessionRuntimeEventReceipt[]>
      error?: unknown
    }> = [
  ...
  it('successful-response settles any 2xx receipt array including []', async () => {
    const cases: AgentSessionRuntimeEventReceipt[][] = [
      [],
  ...
  it('successful-response retains the head on timeout, transport failure, non-2xx, or malformed response', async () => {
    const cases: Array<{
      ...
      {
        name: 'timeout',
        response: () => Promise.reject(Object.assign(new Error('runtime-event delivery timeout'), {})),
      },
...
describe('AgentSessionRuntimeEventOutbox — managed-sequence FIFO', () => {
  it('drains one head per managed producer sequence key', async () => {
    ...
    await outbox.kick()
    await outbox.enqueueBeforeExecution(
      followupTerminal({ id: 'ff-2', event: { type: 'session.followup_failed', payload: { status: 'failed' } } }),
    )
    await outbox.kick()
