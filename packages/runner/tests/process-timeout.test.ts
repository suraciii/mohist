...
describe('runCommand timeout', () => {
  const it = (
    name: string,
    body: (spawner: FakeProcessSpawner, kills: Array<{ pid: number; signal: string | number | undefined }>) => unknown,
  ) =>
    vitestIt(name, () => {
  ...
  it('ParentAbortRejectsInsteadOfProducingTimeout', async (spawner, kills) => {
    vi.useFakeTimers({ toFake: ['setTimeout'] })
