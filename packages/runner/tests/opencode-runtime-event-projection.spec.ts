...
describe('OpenCode runtime event projection', () => {
  ...
  it('keeps tolerant fallbacks for legacy and malformed session error payloads', () => {
    ...
    const stringPayload = projector.project(
  ...
  it('uses one live source per text part when snapshots arrive before deltas', () => {
    ...
    const mirroredDelta = projector.project(
      event('session.next.text.delta', {
        textID: 'part_1',
        assistantMessageID: 'msg_1',
        delta: 'Hello',
    ...
    const nextSnapshot = projector.project(
      event('message.part.updated', {
        part: { id: 'part_1', messageID: 'msg_1', sessionID: 'ses_1', type: 'text', text: 'Hello world' },
      }),
    )

    expect([...snapshot, ...mirroredDelta, ...nextSnapshot].map((item) => item.payload.text)).toEqual([
      'Hello',
      ' world',
    ])
  })
  ...
  it('projects OpenCode tool state into stable tool-call events', () => {
    ...
    expect(started[0]).toMatchObject({
      ...
      payload: { toolCallId: 'call_1', toolName: 'read', status: 'running' },
    })
    expect(completed[0]).toMatchObject({
      type: 'tool_call.completed',
      payload: { toolCallId: 'call_1', status: 'completed', rawOutput: 'ok' },
    })
  })
