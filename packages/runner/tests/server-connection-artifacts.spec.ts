...
describe('ServerConnection.uploadArtifact', () => {
  ...
  it('throwsStructuredErrorOnConflict', async () => {
    const responseBody = JSON.stringify({
      ...
      details: {
        existingUploadId: 'artup_first',
        existingContentHash: 'sha256:aaa',
        incomingContentHash: 'sha256:bbb',
      },
    })
    ...
    await expect(
      connection.uploadArtifact(
        'wf-1',
        'work-1',
        { path: 'review.md', contentHash: 'sha256:bbb', size: 1, content: new Uint8Array([0x01]) },
        new AbortController().signal,
      ),
  ...
  it('usesAgentJobArtifactEndpointWhenOwnerKindIsAgentJob', async () => {
    fetchMock.mockResolvedValueOnce(
      mockResponse({
        ...
        body: JSON.stringify({
          data: {
            uploadId: 'artup_agent',
            workflowRunId: 'agent-job-1',
            workId: 'agent-work-1',
            path: 'review.md',
            size: 5,
          },
