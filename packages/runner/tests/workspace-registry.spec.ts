...
describe('WorkspaceRegistry', () => {
      ...
      await withTestRunnerResources(async (fileSystem) => {
        ...
        await workspaceRegistryTestStorage.run(state, async () => {
          try {
            ...
            if (fileSystem.exists(state.root))
              throw new Error(`workspace registry test root was not cleaned: ${state.root}`)
          }
        })
      })
    })
  ...
  it('Load_LegacyFile_RejectsEntries', async () => {
    ...
    await testFileSystem().ensureDir(join(testRoot(), '.mohist'))
    await testFileSystem().writeText(
      filePath,
      JSON.stringify(
        {
          version: 1,
  ...
  it('Persist_UsesAtomicRename_NoTempFilesRemain', async () => {
    ...
    const dirEntries = await testFileSystem().readdir(join(testRoot(), '.mohist'))
