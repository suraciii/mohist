...
import { opencodeAction, parseOpencodeInput, DEFAULT_TURN_DEADLINE_MS } from '../src/actions/opencode.js'
...
describe('opencodeAction — happy path + turn fact', () => {
  ...
  it('Persists a new physical Session before the first prompt and reuses it across tasks', async () => {
    ...
    const openWorkflowAgentSession = vi.fn(
      async (_projectId: string, _workflowRunId: string, _sessionName: string, _body: unknown) => ({
        runtimeSessionId,
        workDir: '/tmp/work',
      }),
    )
    const attachWorkflowAgentSession = vi.fn(
    ...
    expect(attachWorkflowAgentSession.mock.invocationCallOrder[0]).toBeLessThan(
      client.sessionPrompt.mock.invocationCallOrder[0] ?? Number.MAX_SAFE_INTEGER,
    )
    expect(client.sessionPrompt.mock.calls.map((call) => (call[0] as { sessionID: string }).sessionID)).toEqual([
      'ses_default',
      'ses_default',
    ])
  ...
  it('Action forwards the frozen reasoning effort into the runtime turn options', async () => {
    ...
    const request = runTurn.mock.calls[0]?.[0] as {
      options?: { variant?: string | null; reasoningEffort?: string | null }
    }
  ...
  it('Rejects unknown option keys with a diagnostic in the runtime, not a failure', async () => {
    ...
    const context = baseContext({
      ...
      with: {
        prompt: 'do',
        options: { model: 'openai/gpt-5', type: 'opencode', livenessQuietThresholdMs: 5000 },
      } as never,
    })
    const result = await callAction(opencodeAction, context)
    expect(result.error).toBeUndefined()
