...
describe('Action result boundary', () => {
  const it = Object.assign(
    (name: string, body: (workDir: string) => Promise<void> | void) =>
  ...
  it('preserves successful output', async (workDir) => {
    await expect(execute(succeed({ promise: 'PASS' }), workDir)).resolves.toMatchObject({
      status: 'completed',
      output: { promise: 'PASS' },
    })
  })
  ...
  it.each([
    ...
    await withExecutorResources(
      async (workDir) =>
        await expect(execute({ output } as unknown as ActionResult, workDir)).resolves.toMatchObject({
          status: 'failed',
  ...
  it('rejects non-JSON object values', () => {
    expect(validateActionOutputShape({ completedAt: new Date('2026-01-01T00:00:00Z') })).toContain(
      'successful Action output must be a JSON object or null',
    )
  })
