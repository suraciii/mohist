...
describe("branch-integrity task boundaries", () => {
  ...
  function withBoundaryResources<T>(fake: StatefulFakeWorktree, body: () => Promise<T>): Promise<T> {
    return withTestRunnerResources(
      async () => await body(),
      { gitRunner: fake.gitRunner, workspacePrepareExistsChecker: fake.existsChecker },
    )
  }
      ...
      boundaryRegistry(
        async () => ({
          error: {
            ...
            message: "workspace health failure: operation=verify expectedBranch=" + EXPECTED_BRANCH + " observedBranch=(detached)",
          },
        }),
  ...
  it("keeps ordinary conflict failures eligible for their configured recovery path", async () => {
    ...
    const executor = boundaryExecutor(
      boundaryRegistry(
        async () => ({ error: { code: "conflict", message: "rebase conflict" } }),
        ["conflict"],
      ),
      EXPECTED_BRANCH,
    )
