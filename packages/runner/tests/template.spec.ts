...
import {
  ...
  renderTemplate,
  renderWithSkippedFields,
  unresolvedReferences,
  wholeStringUnresolvedReferences,
...
describe('renderTemplate', () => {
  it('VariableValueContainsTemplate_RendersNestedVariables', () => {
    const rendered = renderTemplate(
      ...
      {
        prompts: {
          proposal: 'Write to artifacts/changes/issue-${{ issue.number }}/proposal.md for ${{ issue.title }}',
        },
        issue: {
          number: 2,
    ...
    expect(rendered?.prompt).toBe(
      'Write to artifacts/changes/issue-2/proposal.md for Document update smoke validation note after update',
    )
  })
  ...
  it('UnresolvedReference_Throws', () => {
    expect(() => renderTemplate({ prompt: '${{ unknown.var }}' }, {})).toThrow(
      "Template variable 'unknown.var' was not found",
    )
  })
  ...
  it('UnresolvedEmbeddedReference_MixedWithResolved_Throws', () => {
    expect(() => renderTemplate({ prompt: 'Start ${{ known }} then ${{ unknown }} end' }, { known: 'X' })).toThrow(
      'unknown',
    )
  })
  ...
  it('LiteralFieldPath_SkipsRendering', () => {
    ...
    const rendered = renderTemplate(
      {
        expect: {
          markers: [{ path: '${{ file }}/x.md', contains: 'see ${{ docs }} for details' }],
        },
      },
      {
        file: '/tmp',
  ...
  it('LiteralFieldPath_StandaloneExpectRoot_ProtectsMarkersContainsAndOneOfEntries', () => {
    ...
    expect(rendered.markers[0].path).toBe('artifacts/changes/issue-408/review.md')
    ...
    expect(rendered.markers[0].oneOf).toEqual(['<promise>${{ verdict }}</promise>', '<promise>FAIL</promise>'])
  })
})
...
describe('renderWithSkippedFields', () => {
  it('renders string values as strings (not character-indexed objects)', () => {
    ...
    expect(rendered?.path).toBe('artifacts/changes/issue-7/tasks.json')
  ...
  it('passes through skipped fields without rendering', () => {
    ...
    const rendered = renderWithSkippedFields(
      { path: 'artifacts/changes/issue-${{ issue.number }}/tasks.json', task },
      { issue: { number: 7 } },
      new Set(['task']),
    )
  ...
  it('rejects object and array values in string interpolation', () => {
    expect(() =>
      renderWithSkippedFields(
        { text: 'model=${{ vars.agent }}' },
        { vars: { agent: { model: 'opencode' } } },
        new Set(),
      ),
    ).toThrow(/object or array/)
    expect(() =>
      renderWithSkippedFields({ text: 'items=${{ vars.items }}' }, { vars: { items: ['a'] } }, new Set()),
    ).toThrow(/object or array/)
  })

  it('expands nested references and rejects cycles and excessive depth', () => {
    expect(
      renderTemplate({ value: '${{ vars.alias }}' }, { vars: { alias: '${{ vars.real }}', real: 'done' } })?.value,
    ).toBe('done')
    expect(() =>
      renderTemplate({ value: 'prefix ${{ vars.a }}' }, { vars: { a: '${{ vars.b }}', b: '${{ vars.a }}' } }),
    ).toThrow(/cycle/)

    const vars: Record<string, string> = { value: 'done' }
    for (let index = 0; index < 6; index += 1)
      vars[`v${index}`] = `\${{ vars.${index === 5 ? 'value' : `v${index + 1}`} }}`
...
describe('unresolvedReferences', () => {
  ...
  it('IgnoresLiteralFieldPaths', () => {
    const unresolved = unresolvedReferences({ expect: { markers: [{ contains: '${{ would.fail }}' }] } }, {})
    expect(unresolved).toEqual([])
  })

  it('ReturnsEmptyWhenAllResolve', () => {
    const unresolved = unresolvedReferences({ prompt: '${{ a.b }}' }, { a: { b: 'value' } })
    expect(unresolved).toEqual([])
  })
})
    ...
    const unresolved = wholeStringUnresolvedReferences(
      {
        description:
          ...
          'Write to artifacts/changes/issue-${{ issue.number }}.',
      },
      { issue: { number: 49 } },
    )
    expect(unresolved).toEqual([])
...
describe('task output variables', () => {
  it('ResolvesTaskOutputReferenceInWithValue', () => {
    const rendered = renderTemplate(
      { path: '${{ tasks.proposal.outputs.changeName }}/specs' },
      { tasks: { proposal: { outputs: { changeName: 'issue-97' } } } },
    ) as { path: string }
  ...
  it('ResolvesTaskOutputReferenceInArtifactPath', () => {
    const rendered = renderTemplate(
      ...
      { tasks: { proposal: { outputs: { changeDir: 'artifacts/changes/issue-97' } } } },
    ) as { files: Array<{ path: string }> }

    expect(rendered.files[0].path).toBe('artifacts/changes/issue-97/review.md')
  })

  it('MissingTaskOutputReferenceFailsWhenEmbedded', () => {
    expect(() =>
      renderTemplate({ path: '${{ tasks.proposal.outputs.missing }}/specs' }, { tasks: { proposal: { outputs: {} } } }),
    ).toThrow('tasks.proposal.outputs.missing')
  })

  it('MissingTaskOutputReferenceFailsAsWholeString', () => {
    expect(() => renderTemplate({ path: '${{ tasks.proposal.outputs.missing }}' }, {})).toThrow(
      'tasks.proposal.outputs.missing',
    )
  })

  it('MissingTaskOutputReferenceIsReportedAsUnresolved', () => {
    expect(unresolvedReferences({ path: '${{ tasks.proposal.outputs.missing }}/specs' }, {})).toEqual([
      'tasks.proposal.outputs.missing',
    ])
  })

  it('NonTaskUnresolvedReferenceStillFailsAsBefore', () => {
    expect(() => renderTemplate({ path: '${{ unknown.var }}' }, {})).toThrow(
      "Template variable 'unknown.var' was not found",
    )
  })
  ...
  it('ExistingTemplateBehaviorWithTaskOutputsUnchanged', () => {
    const rendered = renderTemplate(
      { prompt: 'Write to artifacts/changes/issue-${{ issue.number }}/proposal.md for ${{ issue.title }}' },
      {
    ...
    expect(rendered.prompt).toBe('Write to artifacts/changes/issue-97/proposal.md for Document update')
  })
})
