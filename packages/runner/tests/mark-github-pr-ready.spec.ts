...
import { markGitHubPrReadyAction } from '../src/actions/github-pr.js'
...
function context(withOverrides: JsonObject = {}, variables: JsonObject = {}): ActionContext {
  return {
    ...
    variables: {
      ...
      workspace: { name: 'issue-9', branch: 'mohist/run-wr-ready-1' },
...
describe('mohist/mark-github-pr-ready registry', () => {
  it('registers mark-github-pr-ready under its new id', () => {
    ...
    expect(resolved.kind === 'definition' ? resolved.definition.manifest.name : null).toBe(
...
describe('mohist/mark-github-pr-ready action', () => {
  ...
  it('forwards gh command output to the task log sink', async (resources) => {
    ...
    resources.githubPrGhRunner = async (cmd, args, _cwd, _signal, _env, options) => {
      ...
      if (full.startsWith('gh pr view 42 --json state,isDraft,url'))
        return ghOk(JSON.stringify({ state: 'OPEN', isDraft: true, url: 'https://github.com/acme/repo/pull/42' }))
    ...
    expect(result.error).toBeUndefined()
    expect(
      writes.some((write) => write.source === 'action:mark-github-pr-ready' && write.text.includes('gh pr ready 42')),
    ).toBe(true)
  })
  ...
  it('uses the explicitly declared repository despite different Variables', async (resources) => {
    ...
    const result = await callAction(
      ...
      context(
        { repositoryUrl: 'https://github.com/acme/repo.git', prNumber: 42 },
        { repository: { gitUrl: 'https://example.com/other.git' } },
      ),
    )
  ...
  it('returns errorCode pr-state-conflict if the PR is closed', async (resources) => {
    installGh(resources, (cmd, args) => {
      ...
      switch (full) {
        ...
        case 'gh pr view 42 --json state,isDraft,url':
          return ghOk(
            JSON.stringify({ state: 'CLOSED', isDraft: true, url: 'https://github.com/example/repo/pull/42' }),
          )
        default:
          return ghFail(`unexpected gh call: ${full}`)
      }
  ...
  it('NetworkGhCalls_AllReceiveTimeoutMs', async (resources) => {
    ...
    for (const command of [
      'gh --version',
      'gh auth status',
      'gh pr view 42 --json state,isDraft,url',
      'gh pr ready 42',
    ]) {
