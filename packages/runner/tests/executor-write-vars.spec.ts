...
describe('WorkExecutor result variable effects', () => {
  const it = (name: string, body: (workDir: string) => Promise<void>) =>
    vitestIt(name, () => withExecutorResources(body))
  ...
  it('merges result effects and setVars into one patch with setVars precedence', async (workDir) => {
    ...
    const registry = defineTestActions({
      'test/write-vars': {
        ...
        run: async () => {
          ...
          return {
            output: { checkpoint: 'from-output' },
            effects: { writeVars: { checkpoint: 'from-effect', extra: true } },
          }
        },
      },
    })
    ...
    expect(patchCalls).toEqual([
      { workflowRunId: 'wf-write-vars', vars: { checkpoint: 'from-output', extra: true }, signal },
    ])
  })
})
