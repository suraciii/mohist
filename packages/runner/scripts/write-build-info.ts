...
function main() {
  ...
  const manifest = identity
    ? buildManifest(() => identity.buildGitHash ?? identity.gitHash ?? identity.sourceRevision ?? null, Date.now, {
  ...
  process.stdout.write(`wrote ${manifestPath} (buildGitHash=${manifest.buildGitHash ?? 'null'})\n`)
}
