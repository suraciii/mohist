...
export class SkillResolver {
  ...
  private roots(workDir: string): readonly string[] {
    ...
    return [
      ...
      ...additions,
      join(this.homeDir, '.mohist', 'cli', 'skill-data'),
    ]
  }
  ...
  private async load(
  ...
  ): Promise<{ readonly ok: true; readonly skill: ResolvedSkill } | SkillResolution> {
    for (const root of roots) {
      ...
      if (rel.startsWith('..') || isAbsolute(rel))
        return this.failure(name, 'SKILL.md resolves outside its configured root')
      let body: string
      try {
