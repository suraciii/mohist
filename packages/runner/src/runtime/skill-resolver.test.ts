...
describe('SkillResolver', () => {
  ...
  it('does not resolve an empty list and serializes skill data', async () => {
    const empty = await new SkillResolver({ environment: { MOHIST_SKILL_ROOTS: '/does/not/exist' } }).resolve(
      [],
      '/does/not/exist',
    )
    expect(empty).toEqual({ ok: true, skills: [] })
    const envelope = buildExecutionEnvelope('goal', 'agent instructions', [
      { name: 'demo', instructions: 'body with ${not-a-template}' },
    ])
    expect(envelope).toContain(
      JSON.stringify({
        instructions: 'agent instructions',
        skills: [{ name: 'demo', instructions: 'body with ${not-a-template}' }],
      }),
    )
  ...
  it('preserves a normal dispatch exactly and serializes managed Slack facts separately', () => {
    ...
    const envelope = buildExecutionEnvelope('goal', null, [inlineSlackCollaborationSkill(context)], context)
