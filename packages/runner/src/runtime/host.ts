...
import {
  ...
  POLL_TIMEOUT_MS,
  raceInterval,
  READINESS_DIAGNOSTIC_RELOG_INTERVAL_MS,
  startTaskLogFlushTrigger,
  TASK_LOG_FLUSH_INTERVAL_MS,
  TASK_LOG_FLUSH_LINE_THRESHOLD,
  TASK_LOG_INCREMENTAL_UPLOAD_TIMEOUT_MS,
  TASK_LOG_UPLOAD_TIMEOUT_MS,
  terminalDeliveryFailure,
} from './host-support.js'
export { startTaskLogFlushTrigger }
...
function usesOpenCode(work: DispatchWorkItem): boolean {
  if (work.ownerKind === 'agent-job') {
    const runtime = typeof work.with?.runtime === 'string' ? work.with.runtime : work.agentDefinition?.runtime
