...
const DEFAULT_PATTERNS = [
  /quota/i, /credit/i, /billing/i, /usage[ -]?limit/i, /payment[ -]?required/i,
  /insufficient[ _-]?balance/i, /额度/i, /余额/i, /计费/i, /欠费/i, /使用上限/i,
  /已达到[^。]*(?:限额|上限)/i, /限额[^。]*(?:重置|恢复)/i,
] as const
...
export function parseProviderErrorPolicy(env: Record<string, string | undefined>): PiResultPolicy {
  ...
  if (rawPatterns !== undefined && rawPatterns.trim() !== "") {
    let values: unknown
    try { values = JSON.parse(rawPatterns) } catch {
  ...
  return { ok: true, value: Object.freeze({ nonRecoverablePatterns: Object.freeze(patterns), consecutiveRetryThreshold: threshold }), diagnostics }
}
