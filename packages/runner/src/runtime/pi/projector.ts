...
export function createPiProjector(
    ...
    content.forEach((part, index) => {
      ...
      if (partType === 'thinking')
        facts.push(
          ...reconcileText(
            'reasoning.delta',
            `${messageId}:reasoning.delta:${index}`,
            messageId,
  ...
  const projectTool = (
    ...
    return [
      emit(
        phase === 'started' ? 'tool_call.started' : phase === 'updated' ? 'tool_call.updated' : 'tool_call.completed',
        payload,
      ),
    ]
  }
  ...
  return {
    project(source) {
      ...
      switch (eventType) {
        ...
        case 'tool_execution_start':
          return projectTool(event, 'started')
        case 'tool_execution_update':
          return projectTool(event, 'updated')
        case 'tool_execution_end':
          return projectTool(event, 'completed')
        case 'compaction_start':
          return emitOnce('compaction', eventId(event, 'compaction-start'), { phase: 'started' })
        case 'compaction_end':
          return emitOnce('compaction', eventId(event, 'compaction-end'), {
            phase: 'completed',
            error: event.errorMessage,
          })
        case 'auto_retry_start':
          return emitOnce('provider.retry', eventId(event, 'retry-start'), {
            phase: 'started',
            attempt: event.attempt,
            maxAttempts: event.maxAttempts,
            delayMs: event.delayMs,
            message: stringValue(event.errorMessage),
          })
        case 'auto_retry_end':
          return emitOnce('provider.retry', eventId(event, 'retry-end'), {
            phase: 'ended',
            attempt: event.attempt,
        ...
        case 'model_change':
          return emitResolvedModel(eventId(event, 'model'), event, emitOnce, appliedEffort)
    ...
    reconcile(messages) {
      ...
      messages.forEach((message, index) => {
        ...
        if (!value) return
        if (message.role === 'assistant')
          facts.push(...projectAssistantMessage(value, activeAssistantId ?? `assistant-${index}`))
...
function normalizeUsage(usage: Record<string, unknown>): Record<string, unknown> {
  ...
  return {
    ...
    ...(numberValue(cost?.total) !== undefined
      ? { costAmount: numberValue(cost?.total) }
      : numberValue(cost?.amount) !== undefined
        ? { costAmount: numberValue(cost?.amount) }
        : {}),
...
function maskPayload(payload: Record<string, unknown>, masker: CredentialMasker): Record<string, unknown> {
  ...
  for (const [key, value] of Object.entries(payload))
    result[key] = typeof value === 'string' ? masker.mask(value) : value
  return result
}
