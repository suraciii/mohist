...
interface DirectoryEntry {
  ...
  readonly unknownHandle: boolean
  admitted: number
  used: boolean
  disposing: boolean
  removing: boolean
...
export class OpenCodeDirectoryInstances {
  ...
  constructor(
    ...
    private readonly ownerForDirectory: (directory: string) => string | null = () => null,
  ) {}
  ...
  async reclaimWhere(
    ...
    const trackedDirectories = [...this.entries.entries()].filter(([, entry]) => entry.used)
    const candidates = trackedDirectories.filter(([directory, entry]) =>
      predicate(directory, entry.workspaceOwner, entry.unknownHandle),
    )
  ...
  private async dispose(directory: string, entry: DirectoryEntry): Promise<DirectoryReleaseResult> {
    ...
    if (!client) {
      const result = this.failure(
        ...
        'OpenCode client is unavailable while releasing a directory Instance',
      )
    ...
    try {
      ...
      if (snapshot.kind === 'busy') {
        ...
        if (disposed?.data !== true) {
          result = this.failure(
            directory,
            'opencode-instance-dispose-unconfirmed',
            'OpenCode Instance disposal was not confirmed',
          )
        } else {
      ...
      result = this.failure(
        directory,
        'opencode-instance-release-failed',
        errorMessage(cause, 'OpenCode directory Instance release failed'),
      )
    }
    this.completeDispose(directory, entry, result)
    return result
  ...
  private async prepareRemoval(directory: string, entry: DirectoryEntry): Promise<DirectoryReleaseResult> {
    ...
    if (!client)
      return this.failure(
        directory,
        'opencode-runtime-unavailable',
        'OpenCode client is unavailable while removing a workspace',
      )
    try {
      const statuses = await client.session.status({ directory }, { throwOnError: true })
      const snapshot = readStatusSnapshot(statuses?.data)
  ...
  private newEntry(directory = ''): DirectoryEntry {
    const workspaceOwner = directory ? this.ownerForDirectory(directory) : null
    return {
      generation: this.generation,
      workspaceOwner,
      unknownHandle: directory.startsWith('/proc/') && workspaceOwner === null,
      admitted: 0,
      used: false,
      disposing: false,
      removing: false,
...
function readStatusSnapshot(data: unknown): StatusSnapshot {
  ...
  for (const value of Object.values(data)) {
    if (
      ...
      return {
        kind: 'invalid',
        diagnostic: statusDiagnostic('OpenCode session.status returned a malformed status entry'),
      }
    }
    ...
    if (type === 'busy' || type === 'retry') {
      return {
        ...
        diagnostic: {
          severity: 'info',
          code: 'opencode-instance-busy',
          message: `OpenCode session.status returned ${type}`,
        },
      }
    }
