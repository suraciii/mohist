...
// task-list prompt loader), so the renderer never sees them.
//
...
function renderValue(value: JsonValue, variables: JsonObject, currentPath: string): JsonValue {
  ...
  if (Array.isArray(value))
    return value.map((item, index) => renderValue(item, variables, appendPath(currentPath, String(index))))
  if (typeof value === 'object' && value !== null) {
    return Object.fromEntries(
      Object.entries(value).map(([key, child]) => [key, renderValue(child, variables, appendPath(currentPath, key))]),
    )
  }
  return value
}
...
function walkForWholeStringUnresolved(
  ...
  if (Array.isArray(value)) {
    value.forEach((item, index) =>
      walkForWholeStringUnresolved(item, appendPath(currentPath, String(index)), variables, unresolved),
    )
    return
  }
  if (typeof value === 'object' && value !== null) {
