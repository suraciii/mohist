...
describe("ServerConnection agent-input attachments", () => {
  it("fetches bytes only through the owning input scoped route", async () => {
    fetchSpy.mockResolvedValue(new Response("content", {
      status: 200,
      headers: {
...
describe("ServerConnection named workspace materialization report", () => {
  it("posts the materialized path and parses the recorded home", async () => {
    fetchSpy.mockResolvedValue(
      new Response(JSON.stringify({ runnerId: "runner-1", path: "/virtual/ws/pay" }), { status: 200, headers: { "content-type": "application/json" } }),
    )

    const report = await new ServerConnection(options).reportWorkspaceMaterialized("project-1", "pay", "/virtual/ws/pay", signal)

    expect(report).toEqual({ runnerId: "runner-1", path: "/virtual/ws/pay" })
    expect(fetchSpy.mock.calls[0]?.[0]).toBe("https://runner.test/api/runner/runner-1/workspaces/project-1/pay/materialized")
...
describe("ServerConnection workspace reclaimability", () => {
  itEach([
    ...
    fetchSpy.mockResolvedValue(new Response(JSON.stringify(payload), { status: 200, headers: { "content-type": "application/json" } }))

    const info = await new ServerConnection(options).getWorkspaceReclaimability("project-1", "pay", signal)

    expect(info).toEqual(expected)
    expect(fetchSpy.mock.calls[0]?.[0]).toBe("https://runner.test/api/runner/runner-1/workspaces/project-1/pay/reclaimable")
    const init = fetchSpy.mock.calls[0]?.[1] as RequestInit | undefined
