...
describe('ServerConnection machine credential', () => {
  it('attaches the credential as a Bearer header on every request', async () => {
    ...
    await connection.poll(signal, { processGeneration: 'test-generation', inFlight: [], awaitingAck: [], admissionReady: false })
  ...
  it('leaves requests headerless without a credential', async () => {
    ...
    await new ServerConnection(options).poll(signal, { processGeneration: 'test-generation', inFlight: [], awaitingAck: [], admissionReady: false })
