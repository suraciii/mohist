...
import { processAction, scriptAction, artifactExistsAction, markerAction } from './built-in-core.js'
...
export const builtInActions: ReadonlyArray<ActionDefinition> = [
  ...
  defineAction({
    manifest: {
      ...
      outputs: [
        ...
        { name: 'stdout', description: 'Retained stdout prefix' },
        { name: 'stderr', description: 'Retained stderr prefix' },
        { name: 'stdoutTruncated', description: 'Whether the result boundary shortened stdout' },
        { name: 'stderrTruncated', description: 'Whether the result boundary shortened stderr' },
      ],
  ...
  defineAction({
    manifest: {
      ...
      outputs: [{ name: 'promise', description: 'Completion promise projected by the task executor' }],
      errors: [
        ...
        { code: 'session-reporting-failed', description: 'Workflow AgentSession receipt reporting failed' },
    ...
    manifest: {
      ...
      inputs: {
        ...
        expect: {
          types: ['string'],
          default: 'open,ready',
          description: 'Comma-separated expected states (open/ready/merged)',
        },
      },
      outputs: [
        { name: 'kind', description: 'Output kind discriminator' },
