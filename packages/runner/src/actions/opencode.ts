...
export async function opencodeAction(inputs: JsonObject, host: ActionHost): Promise<ActionResult> {
  ...
  try {
    prompt = await resolvePrompt(inputs.prompt, {
      with: {},
...
export function composeOpencodePrompt(
  prompt: string,
  parentIssueContext?: { title: string; body: string | null } | null,
): string {
...
type OptionsParse = { kind: 'ok'; options: OpencodeOptions | undefined } | { kind: 'failure'; result: ActionResult }
...
type ParsedOptions = { kind: 'ok'; options: OpencodeOptions } | { kind: 'failure'; result: ActionResult }
