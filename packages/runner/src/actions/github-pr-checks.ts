...
type PrStatusCheckRollupParseResult = { kind: 'ok'; checks: PrCheckEntry[] } | { kind: 'invalid'; message: string }
...
type PrChecksClassification = { kind: 'pending' } | { kind: 'passed' } | { kind: 'failed'; message: string }
...
export function classifyPrChecks(entries: PrCheckEntry[]): PrChecksClassification {
  ...
  for (const entry of entries) {
    ...
    if (bucket === 'pending' || bucket === '') pending = true
    if (bucket === 'fail') failed.push(formatFailedCheck(entry))
  }
  if (failed.length > 0) return { kind: 'failed', message: failed.join('; ') }
  return pending ? { kind: 'pending' } : { kind: 'passed' }
}
