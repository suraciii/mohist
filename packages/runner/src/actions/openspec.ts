...
export async function archiveChangeAction(inputs: JsonObject, host: ActionHost): Promise<ActionResult> {
  ...
  const changeDir = resolveChangeDir(host.workDir, inputs)
  if (!changeDir)
    return archiveFailure('config-error', "Archive change requires 'changeDir'", { kind: 'archive-change' })
  ...
  if (sourcePresent && destinationPresent && hintedDestination) {
    return archiveFailure(
      'partial-archive',
      `Both source and archive exist; refusing to proceed: source=${changeDir} archive=${hintedDestination}`,
      { kind: 'archive-change' },
    )
  }
  if (!sourcePresent) {
  ...
  if (!destinationValidation)
    return archiveFailure('config-error', `Archive destination escapes archive root: ${destinationRel}`, {
      kind: 'archive-change',
    })
  ...
  try {
    ...
    return archiveFailure(
      'retry-safe',
      `Failed to move change directory: ${err instanceof Error ? err.message : String(err)}`,
      { kind: 'archive-change' },
    )
  }
  ...
  if (!addResult.success) {
    ...
    return archiveFailure(
      'retry-safe',
      `git add archive change failed: ${addResult.combinedOutput || addResult.stderr || `exit ${addResult.exitCode}`}`,
      {
        kind: 'archive-change',
        source: changeDir,
        destination,
  ...
  const rmResult = await openSpecGitRunner(
    host.workDir,
  ...
  if (!rmResult.success) {
    ...
    return archiveFailure(
      'retry-safe',
      `git rm --cached archive change failed: ${rmResult.combinedOutput || rmResult.stderr || `exit ${rmResult.exitCode}`}`,
      {
        kind: 'archive-change',
        source: changeDir,
        destination,
  ...
  const diffResult = await openSpecGitRunner(
    host.workDir,
  ...
  if (!diffResult.success) {
    ...
    return archiveFailure(
      'retry-safe',
      `git diff archive change failed: ${diffResult.combinedOutput || diffResult.stderr || `exit ${diffResult.exitCode}`}`,
      {
        kind: 'archive-change',
        source: changeDir,
        destination,
  ...
  const commitResult = await openSpecGitRunner(
    host.workDir,
    ['commit', '-m', commitMessage, '--', sourceRel, destinationRel],
    host.signal,
    opts,
  )
  if (!commitResult.success) {
    ...
    return archiveFailure(
      'retry-safe',
      `git commit archive change failed: ${commitResult.combinedOutput || commitResult.stderr || `exit ${commitResult.exitCode}`}`,
      {
        kind: 'archive-change',
        source: changeDir,
        destination,
...
function archiveSuccessNoChange(
  changeDir: string,
  destination: string,
  ...
  return writeVar ? { output, effects: { writeVars: { archive: destinationRel } } } : { output }
}
...
function validateHintDestination(workDir: string, archiveDir: string, destinationRel: string): string | null {
  ...
  if (
    !destinationRel ||
    isAbsolute(destinationRel) ||
    destinationRel.includes('\0') ||
    inside === '' ||
    inside.startsWith('..') ||
    isAbsolute(inside)
  ) {
...
function validateWorkspaceRelativePath(value: string): string | null {
  if (!value || isAbsolute(value) || value.startsWith('../') || value.includes('\0'))
    return `Archive source path escapes workspace: ${value}`
  return null
}
