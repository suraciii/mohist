...
export async function resolveCreatePrText(
  inputs: JsonObject,
  host: ActionHost,
): Promise<{ kind: 'ok'; title: string; body: string } | { kind: 'failure'; message: string }> {
  ...
  const sourceError =
    validateIssueFieldSource('titleFrom', titleSource) ?? validateIssueFieldSource('bodyFrom', bodySource)
