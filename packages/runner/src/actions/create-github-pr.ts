...
import {
  ...
  type GitHubPrErrorCode,
  type GitHubPrStep,
  type GitHubPrStepMetadata,
...
export async function createGitHubPrAction(inputs: JsonObject, host: ActionHost): Promise<ActionResult> {
  ...
  if (!repositoryUrl || !source || !target)
  ...
  record(
    ...
    ghPrecheck.ok ? 0 : ghPrecheck.exitCode,
    ghPrecheck.output,
    ghPrecheck.ok ? undefined : timeoutStepMetadata(ghPrecheck),
  )
  if (!ghPrecheck.ok) {
...
> {
  const listResult = await gh(
    ...
    withGitHubRepository(
      ['pr', 'list', '--head', head, '--base', base, '--state', 'open', '--json', 'number,url,isDraft'],
      githubRepository,
    ),
    workDir,
    signal,
  ...
  record(
    ...
    listResult.exitCode,
    listOutput,
    timeoutStepMetadata(listResult),
  )
  ...
  if (existing.length > 0) {
    ...
    const editResult = await gh(
      'gh',
      withGitHubRepository(editArgs, githubRepository),
      workDir,
      signal,
      undefined,
      options,
    ...
    record(
      'gh-pr-edit',
      `pr edit ${pr.number} --title "${title}" --body "${body}"`,
      editResult.exitCode,
      editOutput,
      timeoutStepMetadata(editResult),
    )
    ...
    return {
      ...
      prUrl: pr.url,
      operation: 'reused',
      output: editOutput || `Reused PR #${pr.number}`,
    }
  }
  ...
  const createResult = await gh(
    'gh',
    withGitHubRepository(createArgs, githubRepository),
    workDir,
    signal,
    undefined,
    options,
  ...
  record(
    'gh-pr-create',
    `pr create --head ${head} --base ${base} --title "${title}"${draft ? ' --draft' : ''}`,
    createResult.exitCode,
    createOutput,
    timeoutStepMetadata(createResult),
  )
