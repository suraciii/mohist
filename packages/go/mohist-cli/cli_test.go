...
func TestResolveConfigPrecedenceAndSafeFailures(t *testing.T) {
	...
	if err != nil || cfg.ServerURL != "http://server/" || cfg.OperatorToken != "direct" || cfg.OperatorID != "operator" {
		t.Fatalf("config = %+v, err = %v", cfg, err)
	}
	deps, _, _ = testDeps(nil, map[string]string{"MOHIST_OPERATOR_TOKEN_PATH": "/token"})
	cfg, err = ResolveConfig(deps)
	if err != nil || cfg.OperatorToken != "file-token" {
	...
	if err == nil || strings.Contains(err.Error(), "secret") || strings.Contains(err.Error(), "file-token") {
...
func TestRunCommandShapeAndFieldDiscoveryDoNotCallHTTP(t *testing.T) {
	...
	if code := Run(context.Background(), []string{"wat"}, deps); code != ExitUsage || errOut.Len() == 0 {
		t.Fatalf("unknown command code=%d stderr=%q", code, errOut.String())
	}
	*out, *errOut = strings.Builder{}, strings.Builder{}
	if code := Run(context.Background(), []string{"run", "why", "--json"}, deps); code != ExitOK || !strings.Contains(out.String(), "workflowRunId") || calls != 0 {
		t.Fatalf("field discovery code=%d output=%q calls=%d", code, out.String(), calls)
	}
	*out, *errOut = strings.Builder{}, strings.Builder{}
	if code := Run(context.Background(), []string{"doctor", "extra"}, deps); code != ExitUsage {
		t.Fatalf("incomplete doctor code=%d stderr=%q", code, errOut.String())
	}
}
...
func TestRunSendsAuthenticatedRequestAndProjectsJSON(t *testing.T) {
	...
	deps, out, errOut := testDeps(roundTripFunc(func(r *http.Request) (*http.Response, error) {
		got = r
		return response(200, `{"success":true,"data":{"workflowRunId":"wr-1","status":"Failed","failure":null,"tasks":[],"dispatch":{"status":"missing"},"events":[]}}`), nil
	}), map[string]string{"MOHIST_SERVER_URL": "http://server", "MOHIST_OPERATOR_TOKEN": "token", "MOHIST_OPERATOR_ID": "cli-test"})
	if code := Run(context.Background(), []string{"run", "why", "wr/1", "--json", "workflowRunId,status"}, deps); code != ExitOK {
		t.Fatalf("code=%d stderr=%q", code, errOut.String())
	}
	if got.URL.EscapedPath() != "/api/runs/wr%2F1/diagnosis" || got.Header.Get("Authorization") != "Bearer token" || got.Header.Get(operatorIDHeader) != "cli-test" || got.Header.Get("Accept") != "application/json" {
		t.Fatalf("request = %v headers=%v", got.URL, got.Header)
	}
...
func TestRunMapsTransportAndCancellation(t *testing.T) {
	...
	if code := Run(context.Background(), []string{"doctor"}, deps); code != ExitOperation || strings.Contains(errOut.String(), "network secret") {
		t.Fatalf("transport code=%d stderr=%q", code, errOut.String())
	}
...
func TestDoctorFailureReturnsOperationExit(t *testing.T) {
	...
	if code := Run(context.Background(), []string{"doctor"}, deps); code != ExitOperation || !strings.Contains(out.String(), "migrations") || errOut.Len() != 0 {
...
func TestClientRejectsMalformedAndFailedResponses(t *testing.T) {
	for _, body := range []string{"not-json", `{"success":false,"error":"nope","code":"forbidden"}`} {
		...
		if code := Run(context.Background(), []string{"doctor"}, deps); code != ExitOperation || errOut.Len() == 0 {
			t.Fatalf("body=%q code=%d stderr=%q", body, code, errOut.String())
		}
	}
}
