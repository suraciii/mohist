...
func organizationDeps(transport http.RoundTripper) (Dependencies, *strings.Builder, *strings.Builder) {
	...
	return Dependencies{
		...
		Lookup: func(name string) (string, bool) {
			if name == "MOHIST_TOKEN" { return "token", true }
			if name == "MOHIST_SERVER_URL" { return "http://server", true }
			return "", false
		},
		ReadFile: func(path string) (string, error) {
			if path == "/work/tree/.mohist/cli-state.json" { return `{"activeProjectId":"proj"}`, nil }
			return "body", nil
		},
...
func TestIssueInvalidFlagsDoNotCallHTTP(t *testing.T) {
	...
	if code := Run(context.Background(), []string{"issue", "list", "--all", "--archived", "--project", "proj"}, deps); code != ExitUsage { t.Fatalf("code=%d", code) }
	if calls != 0 { t.Fatalf("calls=%d", calls) }
}
...
func TestLabelMalformedResponseIsRejected(t *testing.T) {
	...
	if code := Run(context.Background(), []string{"label", "list", "--project", "proj", "--json", "key"}, deps); code != ExitOperation || !strings.Contains(errOut.String(), "invalid shape") { t.Fatalf("code=%d err=%q", code, errOut.String()) }
}
...
func TestIssueCancellationReturns130(t *testing.T) {
	...
	if code := Run(ctx, []string{"issue", "view", "42", "--project", "proj"}, deps); code != ExitCanceled { t.Fatalf("code=%d", code) }
}
