...
describe('Recovery Verb Regression Suite', () => {
  ...
  describe('POST /api/issues/:number/restart — deprecated', () => {
    ...
    it('returns deprecation error without mutating status', async () => {
      const issue = issueService.create({ projectId, title: 'Another Blocked Issue' });
      issueRepo.blockIssue(issue.id, 'Another failure');
      ...
      const updated = issueRepo.findById(issue.id);
      expect(updated?.status).toBe(IssueStatus.Blocked);
      expect(updated?.blockedReason).toBe('Another failure');
    });
    ...
    it('does not reset stage to backlog', async () => {
      const issue = issueService.create({ projectId, title: 'Stage Issue' });
      issueRepo.updateStage(issue.id, Stage.Check);
      issueRepo.blockIssue(issue.id, 'Check failed');
