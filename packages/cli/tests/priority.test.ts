...
describe('Priority - Migration v14', () => {
  ...
  it('should add priority column with default p2', () => {
  ...
  it('should convert priority:high label to p1 and remove label', () => {
    ...
    db.run(
      `INSERT INTO issues (id, number, project_id, title, stage, status, labels, priority, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      ['id-1', 1, project.id, 'Labeled Issue', 'draft', 'active', JSON.stringify(['bug', 'priority:high']), 'p2', '2024-01-01', '2024-01-01']
    );

    initializeDatabase(db);

    const row = db.get<{ priority: string; labels: string }>(
      'SELECT priority, labels FROM issues WHERE id = ?',
      ['id-1']
    );
  ...
  it('should convert priority:critical label to p0', () => {
    db = new DatabaseManager({ inMemory: true });
    initializeDatabase(db);
    ...
    db.run(
      `INSERT INTO issues (id, number, project_id, title, stage, status, labels, priority, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      ['id-1', 1, project.id, 'Critical Issue', 'draft', 'active', JSON.stringify(['priority:critical']), 'p2', '2024-01-01', '2024-01-01']
    );
  ...
  it('should convert all priority label variants correctly', () => {
    db = new DatabaseManager({ inMemory: true });
    initializeDatabase(db);
    ...
    const testCases = [
      { label: 'priority:critical', expected: 'p0' },
      { label: 'priority:p0', expected: 'p0' },
      { label: 'priority:high', expected: 'p1' },
      { label: 'priority:p1', expected: 'p1' },
      { label: 'priority:medium', expected: 'p2' },
      { label: 'priority:p2', expected: 'p2' },
      { label: 'priority:low', expected: 'p3' },
      { label: 'priority:p3', expected: 'p3' },
      { label: 'priority:backlog', expected: 'p4' },
      { label: 'priority:p4', expected: 'p4' },
    ];
    ...
    for (let i = 0; i < testCases.length; i++) {
      db.run(
        `INSERT INTO issues (id, number, project_id, title, stage, status, labels, priority, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [`id-${i}`, i + 1, project.id, `Issue ${i}`, 'draft', 'active', JSON.stringify([testCases[i].label]), 'p2', '2024-01-01', '2024-01-01']
      );
    }
    ...
    for (let i = 0; i < testCases.length; i++) {
      const row = db.get<{ priority: string }>(
        'SELECT priority FROM issues WHERE id = ?',
        [`id-${i}`]
      );
      expect(row?.priority).toBe(testCases[i].expected);
    }
  });

  it('should leave issues without priority labels at default p2', () => {
    db = new DatabaseManager({ inMemory: true });
    initializeDatabase(db);
    ...
    db.run(
      `INSERT INTO issues (id, number, project_id, title, stage, status, labels, priority, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      ['id-1', 1, project.id, 'No Priority Label', 'draft', 'active', JSON.stringify(['bug', 'feature']), 'p2', '2024-01-01', '2024-01-01']
    );
  ...
  it('should create index idx_issues_project_priority', () => {
    db = new DatabaseManager({ inMemory: true });
    initializeDatabase(db);

    const indexes = db.all<{ name: string }>(
      "SELECT name FROM sqlite_master WHERE type='index' AND name='idx_issues_project_priority'"
    );
