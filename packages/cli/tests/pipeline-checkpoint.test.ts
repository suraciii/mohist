...
vi.mock('child_process', () => {
  ...
  return {
    execFile: Object.assign(execFileMock, {
      [Symbol.for('nodejs.util.promisify.custom')]: vi.fn(async (...args: unknown[]) => {
...
describe('PlanStageRunner runPlanStage checkpoint resume', () => {
  ...
  it('commits plan artifacts when the final aggregate Plan task completes', async () => {
    const existingArtifacts = new Set<string>([
    ...
    vi.mocked(execFile).mockImplementation((...args: unknown[]) => {
      ...
      if (commandArgs[0] === 'status') {
        callback(null, '?? openspec/changes/1-test-issue/self-review.md\n', '');
    ...
    checkpointRepo.upsert(1, 'plan', ['proposal', 'specs', 'design', 'tasks'], 'self-review');

    const runner = new PlanStageRunner();
    const worktreePath = path.dirname(path.dirname(path.dirname(CHANGE_DIR)));
    const result = await runAggregatePlanTask(
      runner,
      createMockIssue(),
      createMockArtifactManager(CHANGE_DIR),
      'self-review',
      { completeTask: vi.fn() },
    );
    ...
    expect(execFile).toHaveBeenCalledWith(
      'git',
      ['add', '--', path.relative(worktreePath, CHANGE_DIR)],
      { cwd: worktreePath },
      expect.any(Function),
    );
    expect(execFile).toHaveBeenCalledWith(
      'git',
      ['commit', '-m', 'plan(issue-1): Test Issue', '--no-verify'],
      { cwd: worktreePath },
      expect.any(Function),
    );
