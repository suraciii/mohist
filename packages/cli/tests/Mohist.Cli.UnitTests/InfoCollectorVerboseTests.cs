...
public class InfoCollectorVerboseTests
{
    [Fact]
    public async Task Collect_VerboseTrue_PopulatesVerboseSection()
    {
        ...
        Assert.True(isRepo, $"Expected GitRemote.IsGitRepo=true; runner.Source.Path={result.Runner.Source?.Path ?? "null"}; server.Source.Path={result.Server.Source?.Path ?? "null"}; repo-exists={fs.DirectoryExists(repoDir)}; git-invocations=[{paths}]");
