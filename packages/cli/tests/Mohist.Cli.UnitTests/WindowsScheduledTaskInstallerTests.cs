...
public class WindowsScheduledTaskInstallerTests
{
    private const string UserProfile = @"C:\Users\mohist-test";
    ...
    public async Task LogsServer_Follow_StreamsNewLinesUntilCancelled()
    {
        ...
        var output = new SignalingStringWriter("new line 1");
        var watcher = new FakeLogChangeWatcher();
        ...
        await followStarted.Task;

        files.WriteAllText(ServerLog, "initial\nnew line 1\nnew line 2\n");
        watcher.RaiseChanged();
        await output.LineWritten;
    ...
    private sealed class SignalingStringWriter(string line) : StringWriter
    {
        private readonly TaskCompletionSource _lineWritten = new(TaskCreationOptions.RunContinuationsAsynchronously);

        public Task LineWritten => _lineWritten.Task;

        public override void WriteLine(string? value)
        {
            base.WriteLine(value);
            if (string.Equals(line, value, StringComparison.Ordinal))
                _lineWritten.TrySetResult();
        }
    }

    private sealed class FakeLogChangeWatcher : ILogChangeWatcher
    {
