...
public sealed class SkillsInstallSpecs
{
    ...
    {
        var targetRoot = Path.Combine("/test", $"skills-install-path-{Guid.NewGuid():N}");
        _files.AddDirectory(targetRoot);
        _files.SetCurrentDirectory(Path.Combine("/test", $"skills-install-cwd-{Guid.NewGuid():N}"));
    ...
    {
        _files.SetCurrentDirectory(Path.Combine("/test", $"skills-install-claude-{Guid.NewGuid():N}"));
    ...
    {
        _files.SetCurrentDirectory(Path.Combine("/test", $"skills-install-overwrite-{Guid.NewGuid():N}"));
    ...
    {
        _files.SetCurrentDirectory(Path.Combine("/test", $"skills-install-untouched-{Guid.NewGuid():N}"));
    ...
    {
        var hermesHome = Path.Combine("/test", $"hermes-home-{Guid.NewGuid():N}");
    ...
    {
        var hermesHome = Path.Combine("/test", $"custom-hermes-home-{Guid.NewGuid():N}");
        _environment[SkillInstallService.HermesHomeEnvironmentVariable] = hermesHome;
    ...
    {
        var hermesHome = Path.Combine("/test", $"hermes-home-{Guid.NewGuid():N}");
        var repoPath = Path.Combine("/test", $"hermes-repo-{Guid.NewGuid():N}");
    ...
    {
        var hermesHome = Path.Combine("/test", $"hermes-config-{Guid.NewGuid():N}");
    ...
    {
        _files.SetCurrentDirectory(Path.Combine("/test", $"skills-install-runtime-{Guid.NewGuid():N}"));
    ...
    {
        var targetRoot = Path.Combine("/test", $"skills-install-assets-{Guid.NewGuid():N}");
        _files.AddDirectory(targetRoot);
        ...
        SkillAssetService? assets = null)
    {
        ...
        services.AddSingleton(new MohistCliApi(CreateRejectingHttpClient(), output, error, _files, commands));
        ...
        services.AddSingleton(new RuntimeConsistencyValidator(CreateRejectingHttpClient(), commands, _files, _environment, output));
        services.AddSingleton(new ServiceReadinessProbe(CreateRejectingHttpClient(), output));
        services.AddSingleton(new RunnerRefreshVerifier(CreateRejectingHttpClient(), commands, _files));
        services.AddSingleton(new UpdateOutcomeReporter(CreateRejectingHttpClient(), output));
