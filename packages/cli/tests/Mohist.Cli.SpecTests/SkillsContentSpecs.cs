...
public sealed class SkillsContentSpecs
{
    ...
    {
        var agentsStubDir = Path.Combine("/test", $"agents-stubs-{Guid.NewGuid():N}", ".agents", "skills", "mohist");
    ...
    {
        var userHome = Path.Combine("/test", $"mohist-content-home-{Guid.NewGuid():N}");
    ...
    public async Task Path_FallsBackToSiblingRoot_WhenManagedCacheIsAbsent()
    {
        ...
        var resolver = new SkillAssetRootResolver(
            ...
            getOverrideAssetRoot: () => null,
            getManagedAssetRoot: null,
            getUserHome: () => Path.Combine("/test", $"empty-home-{Guid.NewGuid():N}"));
        var assets = new SkillAssetService(_files, _environment, resolver);
    ...
    {
        var userHome = Path.Combine("/test", $"user-home-{Guid.NewGuid():N}");
    ...
    {
        var overrideRoot = Path.Combine("/test", $"override-assets-{Guid.NewGuid():N}");
    ...
    {
        var mohistSkillsDir = Path.Combine("/test", $"mohist-skills-content-runtime-{Guid.NewGuid():N}", ".mohist", "skills");
    ...
    {
        var targetRoot = Path.Combine("/test", $"mohist-content-defaults-{Guid.NewGuid():N}", "skill-data");
    ...
    {
        var managedRoot = Path.Combine("/test", $"mohist-skills-content-managed-{Guid.NewGuid():N}", ".mohist", "cli", "skill-data");
        _files.AddDirectory(managedRoot);
        WriteSkill(managedRoot, "mohist", "managed mohist body");
        WriteSkill(managedRoot, "mohist-explore", "managed explore body");
        return managedRoot;
