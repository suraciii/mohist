...
export class PlanStageRunner extends BaseStageRunner {
  ...
  private async executeSingleArtifactTask(
    ...
    if (completedSteps.includes(task.type) && task.verifyArtifact()) {
      if (isLastTask) await this.finishAggregatePlanArtifacts(ctx, changeDir);
  ...
  private async finishAggregatePlanArtifacts(ctx: StageContext, changeDir: string): Promise<void> {
    try {
