...
function migrateToVersion25(db: DatabaseManager): void {
  db.transaction(() => {
    const tableInfo = db.all<{ name: string }>("PRAGMA table_info(issues)");
    const hasStageModels = tableInfo.some(col => col.name === 'stage_models');

    if (!hasStageModels) {
      db.exec('ALTER TABLE issues ADD COLUMN stage_models TEXT DEFAULT NULL');
    }
...
CREATE TABLE IF NOT EXISTS workflow_stage_runs (
  ...
  started_at          TEXT,
  completed_at        TEXT,
  work_source_state   TEXT,
  build_work_source_state TEXT,
  created_at          TEXT NOT NULL,
  updated_at          TEXT NOT NULL
);
`;
...
function migrateToVersion29(db: DatabaseManager): void {
  db.transaction(() => {
    const tableInfo = db.all<{ name: string }>('PRAGMA table_info(workflow_stage_runs)');
    const hasBuildWorkSourceState = tableInfo.some(column => column.name === 'build_work_source_state');
    if (!hasBuildWorkSourceState) {
...
function migrateToVersion36(db: DatabaseManager): void {
  db.transaction(() => {
    const tableInfo = db.all<{ name: string }>('PRAGMA table_info(workflow_runs)');
    const hasWorkflowDefinition = tableInfo.some(column => column.name === 'workflow_definition');
    if (!hasWorkflowDefinition) {
      db.exec('ALTER TABLE workflow_runs ADD COLUMN workflow_definition TEXT');
    }
    setSchemaVersion(db, 36);
  });
}

function migrateToVersion37(db: DatabaseManager): void {
  db.transaction(() => {
    const tableInfo = db.all<{ name: string }>('PRAGMA table_info(workflow_stage_runs)');
    const hasWorkSourceState = tableInfo.some(column => column.name === 'work_source_state');
    if (!hasWorkSourceState) {
