...
export function createIssueRoutes(
  ...
  app.post('/:number/retry', async (c) => {
    try {
      ...
      if (project && worktreeManager && checkpointRepo) {
        const worktreePath = worktreeManager.getPath(project.name, issue.number);
        if (worktreePath) {
          const changeDir = findChangeDir(worktreePath, issue);
          if (changeDir) {
            const tasksPath = path.join(changeDir, 'tasks.json');
            if (fs.existsSync(tasksPath)) {
              hasCheckpoint = true;
              ...
              return c.json({
                success: true,
                data: { issue: updatedIssue, message: `Issue #${number} retrying from checkpoint, resuming pipeline` },
              } satisfies ApiResponse);
            }
          }
        }
      ...
      return c.json({
        ...
        data: {
          issue: updatedIssue,
          message: `Issue #${number} no checkpoint found, reset to draft. Use start to begin again.`,
        },
      } satisfies ApiResponse);
    } catch (error) {
      return c.json({ success: false, error: error instanceof Error ? error.message : 'Unknown error' } satisfies ApiResponse, 500);
  ...
  app.post('/:number/restart', async (c) => {
    try {
      ...
      if (checkpointRepo) {
        checkpointRepo.deleteAll(issue.number);
      }
      ...
      return c.json({
        ...
        data: {
          issue: updatedIssue,
          message: `Issue #${number} reset to draft. Use start to begin again.`,
        },
      } satisfies ApiResponse);
    } catch (error) {
      return c.json({ success: false, error: error instanceof Error ? error.message : 'Unknown error' } satisfies ApiResponse, 500);
