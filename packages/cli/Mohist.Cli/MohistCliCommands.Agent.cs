...
internal static class AgentCommands
{
    ...
    public static Command Build(MohistCliApi api)
    {
        ...
        agent.Subcommands.Add(BuildSubscription(api));
    ...
    private static Command BuildSubscription(MohistCliApi api)
    {
        var subscription = new Command(
            "subscription",
            "Manage CloudEvent subscriptions attached to an Agent (issue-391). Subcommands: create <agent>, list <agent>, delete <agent> <subscription-id>.");

        subscription.Subcommands.Add(BuildSubscriptionCreate(api));
        subscription.Subcommands.Add(BuildSubscriptionList(api));
        subscription.Subcommands.Add(BuildSubscriptionDelete(api));
    ...
    private static Command BuildSubscriptionCreate(MohistCliApi api)
    {
        var cmd = new Command(
            "create",
            "Create a CloudEvent subscription on an Agent. Sends POST /api/projects/{projectRef}/agents/{agentRef}/subscriptions and prints the created subscription identity.");
        var agentRefArg = new Argument<string>("agent") { Description = "Agent name or id (resolves project-scoped)" };
        var nameOpt = new Option<string>("--name") { Description = "Subscription name (unique within the Agent)" };
        var filterTypeOpt = new Option<string>("--filter-type") { Description = "Event-type matcher (e.g. com.mohist.workflow.stage.*, com.mohist.workflow.stage.approval-requested|check-requested)" };
        var filterSourceOpt = new Option<string?>("--filter-source") { Description = "Optional exact CloudEvent source constraint" };
        var filterSubjectOpt = new Option<string?>("--filter-subject") { Description = "Optional exact CloudEvent subject constraint" };
        var responsePromptOpt = new Option<string?>("--response-prompt") { Description = "Inline response prompt text (mutually exclusive with --response-prompt-file and --response-prompt-stdin)" };
        var responsePromptFileOpt = new Option<string?>("--response-prompt-file") { Description = "Read response prompt from a UTF-8 file (recommended for long prompts)" };
        var responsePromptStdinOpt = new Option<bool>("--response-prompt-stdin") { Description = "Read response prompt from stdin" };
        var priorityOpt = new Option<int?>("--priority") { Description = "Optional priority override (default 0 at dispatch time)" };
        var (projectOpt, projectIdOpt) = MohistCliCommands.ProjectRefOption();
        var outputOpt = MohistCliCommands.OutputOption("table");

        cmd.Arguments.Add(agentRefArg);
        cmd.Options.Add(nameOpt);
        cmd.Options.Add(filterTypeOpt);
        cmd.Options.Add(filterSourceOpt);
        cmd.Options.Add(filterSubjectOpt);
        cmd.Options.Add(responsePromptOpt);
        cmd.Options.Add(responsePromptFileOpt);
        cmd.Options.Add(responsePromptStdinOpt);
        ...
        cmd.SetAction(ctx =>
        {
            ...
            var name = ctx.GetValue(nameOpt);
            var filterType = ctx.GetValue(filterTypeOpt);
            var filterSource = ctx.GetValue(filterSourceOpt);
            var filterSubject = ctx.GetValue(filterSubjectOpt);
            var responsePrompt = ctx.GetValue(responsePromptOpt);
            var responsePromptFile = ctx.GetValue(responsePromptFileOpt);
            var responsePromptStdin = ctx.GetValue(responsePromptStdinOpt);
            ...
            async Task<int> CreateAsync()
            {
                ...
                {
                    api.Error.WriteLine("--filter-type is required");
                    return 1;
                }
                ...
                var resolvedPrompt = await BodyInputResolver.ResolveAsync(
                    responsePrompt, responsePromptFile, responsePromptStdin,
                    new BodyInputResolver.SourceFlags("--response-prompt", "--response-prompt-file", "--response-prompt-stdin", "response prompt"),
                ...
                return await api.PrintPostWithOutputAsync(
                    ProjectAgentsPath(resolvedProjectId, $"/agents/{MohistCliCommands.Escape(agent.Id)}/subscriptions"),
                    body,
                    mode,
                    nameof(MohistCliApi.TableShape.AgentSubscriptionShow),
                    rawJson: true);
            }
        });
        return cmd;
    ...
    private static Command BuildSubscriptionList(MohistCliApi api)
    {
        var cmd = new Command(
            "list",
            "List CloudEvent subscriptions for an Agent. Resolves the agent ref client-side, then GETs .../agents/{agentId}/subscriptions.");
        cmd.Aliases.Add("ls");
        var agentRefArg = new Argument<string>("agent") { Description = "Agent name or id (resolves project-scoped)" };
        var (projectOpt, projectIdOpt) = MohistCliCommands.ProjectRefOption();
        var outputOpt = MohistCliCommands.OutputOption("table");
    ...
    private static Command BuildSubscriptionDelete(MohistCliApi api)
    {
        var cmd = new Command(
            "delete",
            "Delete a CloudEvent subscription by id. Sends DELETE /api/projects/{projectRef}/agents/{agentRef}/subscriptions/{subscriptionId}.");
        cmd.Aliases.Add("rm");
        var agentRefArg = new Argument<string>("agent") { Description = "Agent name or id (resolves project-scoped)" };
        var subscriptionIdArg = new Argument<string>("subscription-id") { Description = "Subscription identity (subs_...) returned by create/list" };
        var (projectOpt, projectIdOpt) = MohistCliCommands.ProjectRefOption();

        cmd.Arguments.Add(agentRefArg);
        cmd.Arguments.Add(subscriptionIdArg);
        cmd.Options.Add(projectOpt);
        cmd.Options.Add(projectIdOpt);
        cmd.SetAction(ctx =>
        {
            var agentRef = ctx.GetValue(agentRefArg);
            var subscriptionId = ctx.GetValue(subscriptionIdArg);
            var project = ctx.GetValue(projectOpt);
            var projectId = ctx.GetValue(projectIdOpt);
            return DeleteAsync();
            ...
            async Task<int> DeleteAsync()
            {
                ...
                var path = ProjectAgentsPath(resolvedProjectId, $"/agents/{MohistCliCommands.Escape(agent.Id)}/subscriptions/{MohistCliCommands.Escape(subscriptionId!)}");
                try
                {
                    ...
                    api.Output.WriteLine($"Subscription {subscriptionId} deleted");
                    return 0;
                }
