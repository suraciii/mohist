...
internal sealed partial class TableRenderer
{
    ...
    private void RenderAgentSubscriptionList(JsonNode? data)
    {
        ...
        var headers = new[] { "id", "name", "filter", "priority", "status" };
        var widths = new[] { IdSoftCap, 24, TitleSoftCap, 10, 12 };

        var cells = new List<string[]>();
        foreach (var row in rows)
        {
            if (row is not JsonObject obj) continue;
            var filterObj = obj["filter"] as JsonObject;
            var filterText = FormatSubscriptionFilterSummary(filterObj);

            cells.Add(new[]
            {
                Truncate(StringOf(obj, "id"), IdSoftCap),
                Truncate(StringOf(obj, "name"), 24),
                Truncate(filterText, TitleSoftCap),
                Truncate(FormatSubscriptionPriority(obj["priority"]), 10),
                Truncate(StringOf(obj, "status"), 12),
            });
        }
    ...
    private void RenderAgentSubscriptionShow(JsonNode? data)
    {
        if (data is null)
        {
            _out.WriteLine("");
        ...
        var filterObj = data["filter"] as JsonObject;

        _out.WriteLine($"id:             {StringOf(data, "id")}");
        _out.WriteLine($"agent id:       {StringOf(data, "agentId")}");
        _out.WriteLine($"name:           {StringOf(data, "name")}");
        _out.WriteLine($"status:         {StringOf(data, "status")}");
        _out.WriteLine($"filter type:    {StringOf(filterObj, "type")}");
        var filterSource = StringOf(filterObj, "source");
        var filterSubject = StringOf(filterObj, "subject");
        _out.WriteLine($"filter source:  {(string.IsNullOrEmpty(filterSource) ? "(any)" : filterSource)}");
        _out.WriteLine($"filter subject: {(string.IsNullOrEmpty(filterSubject) ? "(any)" : filterSubject)}");
        _out.WriteLine($"priority:       {FormatSubscriptionPriority(data["priority"])}");
        _out.WriteLine($"createdAt:      {Truncate(StringOf(data, "createdAt"), TitleSoftCap)}");
        _out.WriteLine($"updatedAt:      {Truncate(StringOf(data, "updatedAt"), TitleSoftCap)}");
        _out.WriteLine($"responsePrompt: {Truncate(StringOf(data, "responsePrompt"), BodySoftCap)}");
    }

    private static string FormatSubscriptionPriority(JsonNode? priority)
    {
        if (priority is null || priority.GetValueKind() == System.Text.Json.JsonValueKind.Null)
            return "default";
        return priority.GetValueKind() switch
        {
            System.Text.Json.JsonValueKind.Number => priority.GetValue<int>().ToString(System.Globalization.CultureInfo.InvariantCulture),
            System.Text.Json.JsonValueKind.String => priority.GetValue<string>() ?? "default",
            _ => priority.ToJsonString(),
        };
    }

    private static string FormatSubscriptionFilter(JsonObject? filter)
    {
        if (filter is null) return "";
        var type = StringOf(filter, "type");
        var source = StringOf(filter, "source");
        var subject = StringOf(filter, "subject");
        var parts = new List<string> { string.IsNullOrEmpty(type) ? "*" : type };
        if (!string.IsNullOrEmpty(source)) parts.Add($"source={source}");
        if (!string.IsNullOrEmpty(subject)) parts.Add($"subject={subject}");
        return string.Join(" ", parts);
    }

    private static string FormatSubscriptionFilterSummary(JsonObject? filter)
    {
        // List view keeps the type matcher on a single line; full source/subject
        // constraints are surfaced in `show` (and via `--output json`).
        if (filter is null) return "";
        return StringOf(filter, "type");
    }
}
