...
export class SlackAdapter {
  ...
  private async drain(runtime: ConnectionRuntime, signal: AbortSignal) {
    ...
    try {
      ...
      while (!signal.aborted) {
        ...
        try {
          this.assertCurrent(runtime, snapshot, signal)
          const ack = await this.mutateDelivery(snapshot.web, delivery, () =>
            this.assertCurrent(runtime, snapshot, signal),
          )
          this.assertCurrent(runtime, snapshot, signal)
  ...
  private async mutateDelivery(
    ...
    if (operation === 'chat_update') {
      ...
      if (response.ok === false) {
        const ack = await this.fallbackAfterUpdateFailure(
          web,
          delivery,
          payload,
    ...
    if (operation === 'reaction_add' || operation === 'reaction_remove') {
      ...
      if (response.ok === false) {
        ...
        if (payload.fallbackText && payload.fallbackDispatchRef) {
          const ack = await this.postFallback(
            web,
            delivery,
            payload,
  ...
  private async fileShareIdentity(
    web: SlackWebClient,
    delivery: Delivery,
  ...
  private async fallbackAfterUpdateFailure(
    web: SlackWebClient,
    delivery: Delivery,
    payload: DeliveryPayload,
    reason: string,
  ...
  private async reconcile(
    ...
    if (
      (payload.operation === 'reaction_add' || payload.operation === 'reaction_remove') &&
      target &&
      payload.reaction
    ) {
  ...
  private async reconcileForUnknownOperation(
    web: SlackWebClient,
    delivery: Delivery,
    ensureCurrent: () => void,
  ): Promise<DeliveryAck> {
...
function isKnownDeliveryOperation(
  operation: unknown,
