...
describe('HttpAdapterTransport', () => {
  it('uses the canonical operator-authenticated lease endpoints for discovery, hello, renew, ingress, and deliveries', async () => {
          ...
          ? [
              {
                ...
                expectedAppId: 'A1',
                active: true,
                appToken: 'xapp-discovery-secret',
              },
              {
                kind: 'manager',
                enrollmentId: 'e',
                ...
                expectedAppId: 'A2',
                active: true,
                appToken: 'xapp-manager-discovery-secret',
              },
            ]
                    ...
                    : url.endsWith('/interactions')
                      ...
                      : url.endsWith('/claim-uncertain')
                        ? {
                            ...
                            threadTs: null,
                            payloadJson: '{"text":"uncertain"}',
                          }
                        : url.endsWith('/claim')
                          ? {
                              id: url.includes('/slack-manager/') ? 'manager-delivery' : 'delivery-1',
                              ownerKind: url.includes('/slack-manager/') ? 'manager' : 'connection',
                              conversationId: 'D1',
                              threadTs: null,
    ...
    await expect(transport.acquireLease(manager, 'runtime', 'a', signal)).resolves.toMatchObject({
      kind: 'runtime',
      leaseId: 'lease',
    })
    ...
    await expect(transport.claimUncertainDelivery(manager, 'lease', 'a', signal)).resolves.toMatchObject({
      id: 'manager-uncertain',
      ownerKind: 'manager',
    })
    await transport.ackDelivery(
      manager,
      { id: 'manager-uncertain', outcome: 'uncertain', adapterId: 'a' },
      'lease',
      signal,
    )
    ...
    expect(JSON.parse(calls[13]!.body)).toEqual({
      id: 'manager-uncertain',
      outcome: 'uncertain',
      adapterId: 'a',
      leaseId: 'lease',
  ...
  it('maps lease conflicts to null and hello outcomes to typed results', async () => {
    ...
    responses.push({
      status: 409,
      body: { success: false, error: 'other conflict', code: 'delivery_claimed_by_another' },
    })
  ...
  it('rejects malformed discovery targets and lease responses without leaking payloads', async () => {
    ...
    await expect(
      transport2.acquireLease({ projectId: 'p', connectionId: 'c' }, 'runtime', 'a', new AbortController().signal),
    ...
    await expect(
      transport3.claimDelivery({ projectId: 'p', connectionId: 'c' }, 'lease', 'a', new AbortController().signal),
