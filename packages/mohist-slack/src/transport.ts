...
export class HttpAdapterTransport implements AdapterTransport {
  ...
  async reportHello(
    ...
    appId: string,
    signal: AbortSignal,
  ): Promise<SlackHelloOutcome> {
    const payload = await this.postEnvelope(
      `${LEASE_ROUTES}/hello`,
      { target: targetBody(ref), leaseId, appId },
      signal,
    )
    if (payload.ok) {
  ...
  async interaction(
    ...
    if (isManagerTarget(ref)) throw new Error('Slack Manager targets do not expose interactions')
    return await this.post<InteractionResult>(
      `${connectionRoute(ref)}/interactions`,
      { ...envelope, leaseId, adapterId },
      signal,
    )
  }
  ...
  async claimUncertainDelivery(
    ...
    const data = await this.postOrNull<unknown>(
      deliveryRoute(ref, 'claim-uncertain'),
      { leaseId, adapterId },
      undefined,
      signal,
    )
