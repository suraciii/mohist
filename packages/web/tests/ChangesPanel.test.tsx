...
import type { IssueDiffResponse, IssueCommitsResponse } from '../src/lib/types'
...
vi.mock('../src/lib/format-time', async () => ({
...
describe('ChangesPanel', () => {
  ...
  describe('Files tab as default', () => {
    it('renders Files button as the first tab', () => {
      ...
      const filesButton = buttons.find(b => b.textContent?.includes('Files'))
  ...
  describe('Lifecycle-specific states', () => {
    it('renders availability metadata when diffData has availability info', () => {
      const diffData = makeDiffResponse({
        available: true,
        summary: { filesChanged: 1, commits: 1, additions: 5, deletions: 3 },
        files: [{ file: 'x.txt', additions: 5, deletions: 3, diff: '', isBinary: false }],
      })
      const commitsData = makeCommitsResponse({ available: true })
