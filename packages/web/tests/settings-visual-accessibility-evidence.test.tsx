...
const configuredEvidenceDir = process.env.MOHIST_SETTINGS_VISUAL_EVIDENCE_DIR
const evidenceDir = configuredEvidenceDir
  ? resolve(configuredEvidenceDir)
  : mkdtempSync(join(tmpdir(), 'mohist-settings-visual-evidence-'))
  ...
  afterAll(() => {
    writeArtifact(
      ...
      [
        ...
        '',
        'Generated by `MOHIST_SETTINGS_VISUAL_EVIDENCE_DIR=<dir> npm run test:run -- settings-visual-accessibility-evidence.test.tsx`.',
    ...
    if (!configuredEvidenceDir) rmSync(evidenceDir, { recursive: true, force: true })
  })

  it('writes before/after visual diff snapshots and contrast audit artifacts', () => {
