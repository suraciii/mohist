...
function timelineFor(phase: Phase) {
  ...
  const settlement = phase === 'unknown' || blocked
    ? {
  ...
  const attention = blocked
    ? { ...settlement, state: 'blocked', reason: 'agent-result-unconfirmed' }
    : null
  return {
    workflowRunId,
    ...
    stages: [{
      ...
      tasks: [{
        ...
        status: phase === 'completed' ? 'completed' : phase === 'stopped' ? 'cancelled' : blocked ? 'blocked' : 'running',
        startedAt: '2026-08-14T10:55:00Z',
        completedAt: phase === 'completed' ? '2026-08-14T11:02:00Z' : null,
        durationMs: null,
        attempts: 1,
...
async function mockApi(page: Page, getPhase: () => Phase) {
  ...
  await page.route('**/api/**', async route => {
    ...
    if (method === 'GET' && path === `/projects/${project.id}/issues`) return route.fulfill({ json: response([issueFor(phase)]) })
    if (method === 'GET' && path === `/projects/${project.id}/issues/${issueNumber}`) return route.fulfill({ json: response(issueFor(phase)) })
    if (method === 'GET' && path === `/projects/${project.id}/issues/${issueNumber}/workflow/status`) return route.fulfill({ json: response({ workflow: timelineFor(phase) }) })
    if (method === 'GET' && path === `/projects/${project.id}/agent/status`) return route.fulfill({ json: response({ running: false, runnerAvailable: true, activeAgents: [], capacity: { active: 0, max: 4 } }) })
    if (method === 'GET' && path.includes('/workflow/tasks/') && path.endsWith('/logs')) return route.fulfill({ json: response({ lines: [], nextCursor: null, truncated: false }) })
    if (method === 'GET' && path === `/workflow-runs/${workflowRunId}/sessions`) return route.fulfill({ json: response([]) })
    ...
    if (method === 'GET' && (path.endsWith('/diff') || path.endsWith('/commits'))) return route.fulfill({ json: response({ available: false, message: 'No fixture' }) })
    if (method === 'GET' && path.endsWith('/workspace-status')) return route.fulfill({ json: response({ exists: false, reason: 'not_started' }) })
    if (method === 'GET' && (path.endsWith('/variables') || path.endsWith('/workflow-profile'))) return route.fulfill({ json: response({ vars: {}, stages: {} }) })
    if (method === 'GET' && path === `/projects/${project.id}/workflow-profile/default`) return route.fulfill({ json: response({ projectId: project.id, defaultWorkflowProfileId: 'mohist/github-pr', disabledWorkflowProfileIds: [] }) })
...
test('unknown Agent result becomes blocked and a late result clears blocked presentation', async ({ page }) => {
  ...
  await expect(page.getByTestId('workflow-agent-result-attention')).toHaveCount(0)
  await page.getByTestId('workflow-stage-bar').getByRole('button', { name: /^Build/ }).click()
