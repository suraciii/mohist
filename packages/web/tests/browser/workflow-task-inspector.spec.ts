...
const longTitle =
  'Complete the responsive workflow inspector while preserving this intentionally long task title on a phone viewport'
const stageTasks = {
  ...
  await page.route('**/api/**', async (route) => {
    ...
    if (method === 'GET' && path === `/projects/${project.id}/agent/status`) {
      return route.fulfill({
        json: apiResponse({ running: false, runnerAvailable: true, activeAgents: [], capacity: { active: 0, max: 4 } }),
      })
    }
    ...
    if (method === 'GET' && path === `/projects/${project.id}/issues/${issueNumber}/workflow/status`)
      return route.fulfill({ json: apiResponse({ workflow: timeline }) })
          ...
          lines: [
            {
              ...
              source: 'action:build',
              text: 'Browser-visible canonical task log',
            },
          ],
          nextCursor: null,
          truncated: false,
    ...
    if (method === 'GET' && path.endsWith('/diff'))
      return route.fulfill({ json: apiResponse({ available: false, message: 'No diff fixture' }) })
    if (method === 'GET' && path.endsWith('/commits'))
      return route.fulfill({ json: apiResponse({ available: false, message: 'No commits fixture' }) })
    ...
    if (method === 'GET' && path.endsWith('/variables'))
      return route.fulfill({ json: apiResponse({ vars: {}, stages: {} }) })
    ...
    if (method === 'GET' && path.endsWith('/opencode/models'))
      return route.fulfill({ json: apiResponse({ models: [], modelVariants: {} }) })
...
test('workflow inspector renders update interruption context without raw runtime transport errors', async ({
  page,
}) => {
  ...
  await expect(page.getByTestId('workflow-agent-interruption-attention')).not.toContainText(
...
test('phone workflow inspector exposes one responsive task list with honest disclosures', async ({ page }) => {
  ...
  const stageBoxes = await stageButtons.evaluateAll((buttons) =>
    buttons.map((button) => {
  ...
  for (const stage of ['Plan', 'Build', 'Check', 'Integrate'] as const) {
    ...
    await expect(
      page.getByText(stageTasks[stage.toLowerCase() as keyof typeof stageTasks], { exact: true }),
    ).toBeVisible()
  ...
  const overflow = await page.evaluate(() => ({
    ...
    stageBar:
      document.querySelector('[data-testid="workflow-stage-bar"]')!.scrollWidth -
