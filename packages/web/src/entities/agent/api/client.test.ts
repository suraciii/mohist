...
describe("unarchiveAgent", () => {
  it("POSTs /api/projects/{ref}/agents/{id}/unarchive and returns the agent payload", async () => {
    ...
    server.use(
      http.post(
        "*/api/projects/:projectId/agents/:agentId/unarchive",
        async ({ request }) => {
    ...
    expect(requestPath(requests[0])).toBe(
      "/api/projects/proj-1/agents/agent-1/unarchive",
    );
...
describe("writeAgentModelAndVariant", () => {
  ...
  it("drops the variant when null is passed", () => {
    expect(
      writeAgentModelAndVariant({ model: "m", variant: "high" }, "m", null),
    ).toEqual({
      model: "m",
      runtime: "opencode",
    });
  ...
  it("returns null when model is null regardless of legacy keys", () => {
    ...
    expect(
      writeAgentModelAndVariant(
        { model: "m", variant: "high", type: "opencode" },
        null,
        null,
      ),
    ).toBeNull();
  ...
  it("preserves a raw variant-only definition", () => {
    expect(
      writeAgentModelAndVariant({ variant: "high" }, null, "high"),
