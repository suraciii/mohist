...
interface TaskLogRegistrationEntry {
  refs: Map<
    symbol,
    { onDelta: (delta: TaskLogDeltaEnvelopeWire) => void; refetch: (signal: AbortSignal) => Promise<unknown> }
  >
...
const MAX_RECONCILIATION_BUFFER_SIZE = 256
const OPEN = 1
...
function isTaskLogDelta(value: unknown): value is TaskLogDeltaEnvelopeWire {
...
export class LiveEventsController implements LiveEventsApi {
  ...
  private fencedGeneration: number | null = null
  ...
  stop(): void {
    ...
    this.clearReconciliationBuffers()
    ...
    return {
      ...
      dispose: () => {
        ...
        if (current.refs.size === 0) {
          current.bufferingGeneration = null
          current.buffered = []
  ...
  private connect(): void {
    ...
    socket.onmessage = (message) => {
      if (
        ...
        this.fencedGeneration === generation ||
  ...
  private receive(raw: string, generation: number): void {
    ...
    } else if (message.method === 'event.task-log' && params && isTaskLogDelta(params.delta)) {
      this.receiveTaskLog(params.delta)
    }
  }
  ...
  private async receiveResponse(response: Record<string, unknown>, generation: number): Promise<void> {
    ...
    try {
      await this.reconcile(generation, reconciliationController.signal)
    } catch {
      reconciliationFailed = true
      if (this.isActiveGeneration(generation)) this.handleReconciliationFailure()
    } finally {
      if (this.reconciliationController === reconciliationController) this.reconciliationController = null
      if (this.isActiveGeneration(generation)) {
        this.inFlightId = null
        if (!reconciliationFailed) this.sendSubscription()
      }
    }
  }

  private async reconcile(generation: number, signal: AbortSignal): Promise<void> {
    if (!this.isActiveGeneration(generation)) return
    const transcriptRegistrations = [...this.transcripts.entries()]
    const taskLogRegistrations = [...this.taskLogs.entries()].map(
      ([key, entry]) => [key, entry, [...entry.refs.entries()]] as const,
    )
    ...
    const queryFilter = (query: { getObserversCount: () => number; queryKey: readonly unknown[] }) =>
      query.getObserversCount() > 0 &&
      hasProjectId(query.queryKey, this.options.projectId) &&
      !isReconciledStreamQuery(query.queryKey)
    const cancelProjectReconciliation = () =>
      void this.options.queryClient.cancelQueries({ predicate: queryFilter }, { silent: true })
    signal.addEventListener('abort', cancelProjectReconciliation, { once: true })
    try {
      ...
      await this.options.queryClient.cancelQueries({ predicate: queryFilter }, { silent: true })
      throw error
    } finally {
      signal.removeEventListener('abort', cancelProjectReconciliation)
    }
    if (!this.isActiveGeneration(generation)) return

    const transcriptReconciliations = transcriptRegistrations.map(async ([token, registration]) => {
      if (!this.isActiveGeneration(generation) || this.transcripts.get(token) !== registration) return
      await registration.reconcile(signal)
    })
    const taskRefetches = taskLogRegistrations.flatMap(([key, entry, refs]) =>
      refs.map(async ([token, registration]) => {
        if (
          !this.isActiveGeneration(generation) ||
          this.taskLogs.get(key) !== entry ||
          entry.refs.get(token) !== registration
        )
          return
        await registration.refetch(signal)
      }),
    )
    await Promise.all([...transcriptReconciliations, ...taskRefetches])
    if (!this.isActiveGeneration(generation)) return
  ...
  private receiveTranscript(event: Record<string, unknown>, generation: number): void {
    ...
    if (buffer.events.length >= MAX_RECONCILIATION_BUFFER_SIZE) {
      this.handleReconciliationOverflow()
      return
    }
  ...
  private receiveTaskLog(delta: TaskLogDeltaEnvelopeWire): void {
    ...
    if (entry.bufferingGeneration === this.generation) {
      if (entry.buffered.length >= MAX_RECONCILIATION_BUFFER_SIZE) {
        this.handleReconciliationOverflow()
        return
      }
  ...
  private deliverTaskLogDelta(entry: TaskLogRegistrationEntry, delta: TaskLogDeltaEnvelopeWire): void {
    for (const { onDelta } of entry.refs.values()) {
      try {
        onDelta(delta)
      } catch {
        // One mounted consumer must not block the others or subscription progress.
      }
    }
  }

  private handleReconciliationOverflow(): void {
    this.fenceReconciliation(4000, 'reconciliation buffer overflow')
  }

  private handleReconciliationFailure(): void {
    this.fenceReconciliation(4001, 'reconciliation failed')
  }

  private fenceReconciliation(code: number, reason: string): void {
    this.fencedGeneration = this.generation
    this.reconciliationController?.abort()
    this.clearReconciliationBuffers()
    this.socket?.close(code, reason)
  }

  private clearReconciliationBuffers(): void {
  ...
  private isActiveGeneration(generation: number): boolean {
    return (
      !this.stopped && this.socket !== null && this.generation === generation && this.fencedGeneration !== generation
    )
  }
}
