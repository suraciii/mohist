...
describe('LiveEventsController', () => {
  ...
  it('does not refetch a replacement transcript registration created during invalidation', async () => {
    const { controller, sockets, queryClient, transcript } = setup()
    const invalidation = deferred()
    vi.spyOn(queryClient, 'invalidateQueries').mockImplementation(() => invalidation.promise)
    const oldRefetch = vi.fn(async () => {})
    const newRefetch = vi.fn(async () => {})
    const oldRegistration = controller.registerTranscriptReconciliation('session-1', 'runtime-old', oldRefetch)
    sockets[0].open()
    const ack = acknowledge(sockets[0])

    oldRegistration.dispose()
    controller.registerTranscriptReconciliation('session-1', 'runtime-new', newRefetch)
    const event = { sessionId: 'session-1', runtimeSessionId: 'runtime-new', sequence: 1 }
    sockets[0].receive({ jsonrpc: '2.0', method: 'event.transcript', params: { event } })
  ...
  it('does not refetch or buffer a replacement task-log registration created during invalidation', async () => {
    const { controller, sockets, queryClient } = setup()
    const invalidation = deferred()
    vi.spyOn(queryClient, 'invalidateQueries').mockImplementation(() => invalidation.promise)
    const oldRefetch = vi.fn(async () => {})
    const newRefetch = vi.fn(async () => {})
    const oldRegistration = controller.registerTaskLogScope(
      { workflowRunId: 'run-1', taskId: 'task-1' },
      vi.fn(),
      oldRefetch,
    )
    ...
    if (oldRegistration.admitted) oldRegistration.dispose()
    const onDelta = vi.fn()
    controller.registerTaskLogScope({ workflowRunId: 'run-1', taskId: 'task-1' }, onDelta, newRefetch)
    const delta = {
      ownerKind: 'workflow',
      ownerId: 'run-1',
      projectId: 'project/one',
  ...
  it('does not refetch a same-scope task-log owner added during invalidation', async () => {
    const { controller, sockets, queryClient } = setup()
    const invalidation = deferred()
    vi.spyOn(queryClient, 'invalidateQueries').mockImplementation(() => invalidation.promise)
    const oldRefetch = vi.fn(async () => {})
    const newRefetch = vi.fn(async () => {})
    controller.registerTaskLogScope({ workflowRunId: 'run-1', taskId: 'task-1' }, vi.fn(), oldRefetch)
    sockets[0].open()
    const ack = acknowledge(sockets[0])

    controller.registerTaskLogScope({ workflowRunId: 'run-1', taskId: 'task-1' }, vi.fn(), newRefetch)
    invalidation.resolve()
    await ack
    await flushPromises()

    expect(oldRefetch).toHaveBeenCalledOnce()
    expect(newRefetch).not.toHaveBeenCalled()
  })
  ...
  it('ignores malformed task-log deltas before invoking consumers', async () => {
    const { controller, sockets } = setup()
    const onDelta = vi.fn()
    controller.registerTaskLogScope({ workflowRunId: 'run-1', taskId: 'task-1' }, onDelta, async () => {})
    sockets[0].open()
    await acknowledge(sockets[0])
    const valid = {
      ownerKind: 'workflow',
    ...
    const invalidDeltas: unknown[] = [
      { ...valid, ownerKind: null },
      { ...valid, ownerId: 1 },
      { ...valid, projectId: {} },
      { ...valid, workId: false },
      { ...valid, taskId: [] },
      { ...valid, entries: {} },
      { ...valid, entries: [{ ...valid.entries[0], seq: 1.5 }] },
      { ...valid, entries: [{ ...valid.entries[0], seq: Number.MAX_SAFE_INTEGER + 1 }] },
      { ...valid, entries: [{ ...valid.entries[0], timestamp: 1 }] },
      { ...valid, entries: [{ ...valid.entries[0], source: null }] },
      { ...valid, entries: [{ ...valid.entries[0], text: {} }] },
  ...
  it('isolates task-log consumers that throw during reconciliation replay', async () => {
    const { controller, sockets } = setup()
    const refetch = deferred()
    const throwingOwner = vi.fn(() => {
      throw new Error('consumer failed')
    })
    const secondOwner = vi.fn()
    controller.registerTaskLogScope({ workflowRunId: 'run-1', taskId: 'task-1' }, throwingOwner, () => refetch.promise)
    controller.registerTaskLogScope({ workflowRunId: 'run-1', taskId: 'task-1' }, secondOwner, async () => {})
    sockets[0].open()
    const ack = acknowledge(sockets[0])
    const delta = {
    ...
    expect(throwingOwner).toHaveBeenCalledWith(delta)
    expect(secondOwner).toHaveBeenCalledWith(delta)
  })

  it('closes failed reconciliation and resumes delivery after reconnect', async () => {
    const { controller, sockets, queryClient, transcript, timers } = setup()
    ...
    controller.registerTranscriptReconciliation('session-1', 'runtime-1', async () => {})
    sockets[0].open()
    const sent = request(sockets[0])
    sockets[0].receive({ jsonrpc: '2.0', id: sent.id, result: {} })
    controller.registerTaskLogScope({ workflowRunId: 'run-1', taskId: 'task-1' }, vi.fn(), async () => {})
    const event = { sessionId: 'session-1', runtimeSessionId: 'runtime-1', sequence: 1 }
    sockets[0].receive({ jsonrpc: '2.0', method: 'event.transcript', params: { event } })

    await flushPromises()

    expect(transcript).not.toHaveBeenCalled()
    expect(sockets[0].closeCalls).toEqual([{ code: 4001, reason: 'reconciliation failed' }])
    expect(cancel).toHaveBeenCalledWith({ predicate: expect.any(Function) }, { silent: true })
    expect(sockets[0].sent).toHaveLength(1)
    sockets[0].receive({ jsonrpc: '2.0', method: 'event.transcript', params: { event } })
    expect(transcript).not.toHaveBeenCalled()
    ...
    timers[0].callback()
    sockets[1].open()
    await acknowledge(sockets[1])
    sockets[1].receive({ jsonrpc: '2.0', method: 'event.transcript', params: { event } })

    expect(transcript).toHaveBeenCalledOnce()
    expect(transcript).toHaveBeenCalledWith(event)
  })

  it('fails fast when one stream reconciliation rejects while another remains pending', async () => {
    const { controller, sockets, transcript } = setup()
    const pending = deferred()
    const failing = deferred()
    const onDelta = vi.fn()
    controller.registerTranscriptReconciliation('session-1', 'runtime-1', () => pending.promise)
    const failedRegistration = controller.registerTaskLogScope(
      { workflowRunId: 'run-1', taskId: 'task-fail' },
      vi.fn(),
      () => failing.promise,
    )
    controller.registerTaskLogScope({ workflowRunId: 'run-1', taskId: 'task-good' }, onDelta, async () => {})
    sockets[0].open()
    const ack = acknowledge(sockets[0])
    await flushPromises()
    const transcriptEvent = { sessionId: 'session-1', runtimeSessionId: 'runtime-1', sequence: 1 }
    const taskDelta = {
      ownerKind: 'workflow',
      ownerId: 'run-1',
      projectId: 'project/one',
    ...
    sockets[0].receive({ jsonrpc: '2.0', method: 'event.transcript', params: { event: transcriptEvent } })
    sockets[0].receive({ jsonrpc: '2.0', method: 'event.task-log', params: { delta: taskDelta } })

    failing.reject(new Error('task-log refetch failed'))
    await flushPromises()
    ...
    pending.resolve()
    if (failedRegistration.admitted) failedRegistration.dispose()
    await ack
  })

  it('closes and clears all transient buffers when a transcript identity exceeds 256 events', async () => {
    const { controller, sockets, transcript, timers } = setup()
    const refetch = deferred()
    controller.registerTranscriptReconciliation('session-1', 'runtime-1', () => refetch.promise)
    sockets[0].open()
    const ack = acknowledge(sockets[0])
    ...
    for (let sequence = 0; sequence <= 256; sequence += 1) {
      sockets[0].receive({
        ...
        params: { event: { sessionId: 'session-1', runtimeSessionId: 'runtime-1', sequence } },
      })
    }

    expect(sockets[0].closeCalls).toEqual([{ code: 4000, reason: 'reconciliation buffer overflow' }])
    expect((controller as unknown as { transcriptBuffers: Map<string, unknown> }).transcriptBuffers.size).toBe(0)
    const event = { sessionId: 'session-1', runtimeSessionId: 'runtime-1', sequence: 257 }
    sockets[0].receive({ jsonrpc: '2.0', method: 'event.transcript', params: { event } })
    expect(transcript).not.toHaveBeenCalled()
    refetch.resolve()
    await ack
    sockets[0].disconnect(4000)
    timers[0].callback()
    sockets[1].open()
    await acknowledge(sockets[1])
    sockets[1].receive({ jsonrpc: '2.0', method: 'event.transcript', params: { event } })
  ...
  it('closes and clears all transient buffers when a task-log scope exceeds 256 deltas', async () => {
    const { controller, sockets, timers } = setup()
    const refetch = deferred()
    const onDelta = vi.fn()
    controller.registerTaskLogScope({ workflowRunId: 'run-1', taskId: 'task-1' }, onDelta, () => refetch.promise)
    sockets[0].open()
    const ack = acknowledge(sockets[0])
      ...
      sockets[0].receive({
        ...
        params: {
          delta: {
            ...
            workId: 'work-1',
            taskId: 'task-1',
            entries: [{ seq, timestamp: '2026-08-20T12:00:00Z', source: 'runner', text: 'line' }],
            truncated: false,
          },
        },
      })
    ...
    expect(sockets[0].closeCalls).toEqual([{ code: 4000, reason: 'reconciliation buffer overflow' }])
    const taskLogs = (controller as unknown as { taskLogs: Map<string, { buffered: unknown[] }> }).taskLogs
    expect([...taskLogs.values()].every((entry) => entry.buffered.length === 0)).toBe(true)
    const delta = {
      ownerKind: 'workflow',
      ownerId: 'run-1',
      projectId: 'project/one',
      workId: 'work-1',
      taskId: 'task-1',
      entries: [{ seq: 257, timestamp: '2026-08-20T12:00:00Z', source: 'runner', text: 'line' }],
      truncated: false,
    }
    sockets[0].receive({ jsonrpc: '2.0', method: 'event.task-log', params: { delta } })
    expect(onDelta).not.toHaveBeenCalled()
    ...
    sockets[1].open()
    await acknowledge(sockets[1])
    sockets[1].receive({ jsonrpc: '2.0', method: 'event.task-log', params: { delta } })
    expect(onDelta).toHaveBeenCalledWith(delta)
  })
  ...
  it('replaces stale transcript buffers when a newer socket generation begins reconciliation', async () => {
    ...
    controller.registerTranscriptReconciliation('session-1', 'runtime-1', (signal) => {
      signals.push(signal)
  ...
  it('invalidates project queries and reconciles every registered stream after acknowledgement', async () => {
    ...
    expect(invalidate).toHaveBeenCalledWith({ predicate: expect.any(Function) }, { throwOnError: true })
    expect(transcriptRefetch).toHaveBeenCalledOnce()
    expect(taskLogRefetch).toHaveBeenCalledOnce()
  })

  it('cancels project reconciliation queries when the socket closes', async () => {
