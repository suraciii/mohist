...
import {
  AgentProfileEditor,
  type AgentProfileEditorOperationsHook,
...
function renderEditor(
  overrides: Partial<Parameters<typeof AgentProfileEditor>[0]> = {},
) {
...
async function openAgentModelSelect() {
  ...
  await waitFor(() => {
    expect(
      screen.getByTestId("agent-model-row-anthropic/claude-variant-high"),
    ).toBeInTheDocument();
  });
}
...
describe("AgentProfileEditor", () => {
  ...
  describe("create flow", () => {
    it('renders create form with title "New Agent"', () => {
      ...
      expect(screen.getByTestId("editor-save")).toHaveTextContent(
        "Create Agent",
      );
    });
    ...
    it("blocks submission with missing name and instructions", async () => {
      ...
      await waitFor(() => {
        expect(screen.getByTestId("editor-name-error")).toHaveTextContent(
          "Name is required",
        );
      });
      expect(screen.getByTestId("editor-instructions-error")).toHaveTextContent(
    ...
    it("calls createAgent with valid form data", async () => {
      ...
      const callArgs = (mocks.createMutation.mutate as ReturnType<typeof vi.fn>)
        .mock.calls[0][0];
    ...
    it("renders model variant chips and persists model plus variant on create", async () => {
      ...
      expect(
        screen.getByTestId("agent-model-row-anthropic/claude-variant-low"),
      ).toHaveTextContent("low");
      expect(
        screen.getByTestId("agent-model-row-anthropic/claude-variant-medium"),
    ...
    it("renders Pi reasoning efforts and persists the selected effort", async () => {
      ...
      const effort = await waitFor(() =>
        screen.getByTestId("agent-model-row-pi/anthropic/claude-variant-high"),
      );
    ...
    it("navigates to the new agent detail page on success", async () => {
      ...
      const onSuccess = (mutateFn as ReturnType<typeof vi.fn>).mock.calls[0][1]
        ?.onSuccess;
  ...
  describe("edit flow", () => {
    ...
    it("renders edit form with pre-filled values", async () => {
      ...
      expect(
        (screen.getByTestId("editor-purpose") as HTMLTextAreaElement).value,
      ).toBe("Review changes");
      expect(
        (screen.getByTestId("editor-description") as HTMLTextAreaElement).value,
      ).toBe("Original purpose");
    ...
    it("calls updateAgent with edited values", async () => {
      ...
      const callArgs = (mocks.updateMutation.mutate as ReturnType<typeof vi.fn>)
        .mock.calls[0][0];
    ...
    it("preserves an effective default as unresolved when saving an unrelated edit", async () => {
      ...
      const updateCall = (
        mocks.updateMutation.mutate as ReturnType<typeof vi.fn>
      ).mock.calls[0][0];
    ...
    it("persists an updated variant and restores its active state from stored agentConfig", async () => {
      renderEditor({ agent: existingAgent });
      await waitFor(() => {
        expect(document.querySelector("#agent-model")).toHaveTextContent(
          "claude · high",
        );
      });
      await openAgentModelSelect();
      expect(
        screen.getByTestId("agent-model-row-anthropic/claude-variant-high"),
      ).toHaveAttribute("data-variant-active", "true");
      fireEvent.click(
        screen.getByTestId("agent-model-row-anthropic/claude-variant-medium"),
      );
      fireEvent.click(screen.getByTestId("editor-save"));
    ...
    it("selecting the model body clears only the stored variant", async () => {
      ...
      fireEvent.click(
        document.querySelector(
          '[data-model-id="anthropic/claude"]',
        ) as HTMLElement,
      );
      fireEvent.click(screen.getByTestId("editor-save"));
    ...
    it("shows inline API error on update failure", async () => {
      ...
      const onError = (mocks.updateMutation.mutate as ReturnType<typeof vi.fn>)
        .mock.calls[0][1]?.onError;
      await act(async () => {
        onError(new Error("UPDATE_FAILED"));
      });
      await waitFor(() => {
        expect(screen.getByTestId("editor-api-error")).toHaveTextContent(
          "UPDATE_FAILED",
        );
      });
    });
    ...
    it("persists agentConfig carrying only {model, variant} — drops legacy keys from spread", async () => {
      ...
      const callArgs = (mocks.updateMutation.mutate as ReturnType<typeof vi.fn>)
        .mock.calls[0][0];
      const agentConfig = callArgs.data.agentConfig as Record<
        string,
        unknown
      > | null;
      expect(agentConfig).not.toBeNull();
      expect(Object.keys(agentConfig ?? {}).sort()).toEqual([
        "model",
        "runtime",
        "variant",
      ]);
    ...
    it("selecting Pi loads Pi models and persists the runtime", async () => {
      ...
      await waitFor(() =>
        expect(document.querySelector("#agent-model")).toBeInTheDocument(),
      );
      fireEvent.click(document.querySelector("#agent-model") as HTMLElement);
      await waitFor(() =>
        expect(
          document.querySelector('[data-model-id="pi/anthropic/claude"]'),
        ).toBeInTheDocument(),
      );
      expect(
      ...
      fireEvent.click(
        document.querySelector(
          '[data-model-id="pi/anthropic/claude"]',
        ) as HTMLElement,
      );
      fillRequiredFields();
      fireEvent.click(screen.getByTestId("editor-save"));
  ...
  describe("archive flow", () => {
    ...
    it("calls archiveAgent on confirm", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByTestId("editor-archive-confirm"),
        ).toBeInTheDocument();
      });
      await act(async () => {
        screen.getByTestId("editor-archive-confirm").click();
      });
      expect(mocks.archiveMutation.mutate).toHaveBeenCalledWith(
        "agent-1",
        expect.any(Object),
      );
    });
    ...
    it("archive confirmation text describes the real effect and contains no pre-fix false phrase", async () => {
      ...
      const confirmDialog = confirmButton.closest(
        '[role="dialog"]',
      ) as HTMLElement;
      expect(confirmDialog).toBeTruthy();
      const description = confirmDialog.querySelector(
  ...
  describe("validation", () => {
    it("blocks submission and shows inline errors for missing required fields", async () => {
      ...
      expect(
        screen.getByTestId("editor-instructions-error"),
      ).toBeInTheDocument();
    });
    ...
    it("shows inline API errors", async () => {
      ...
      const onError = (mutate as ReturnType<typeof vi.fn>).mock.calls[0]?.[1]
        ?.onError;
