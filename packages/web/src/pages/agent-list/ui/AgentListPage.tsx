...
function AgentRow({
  ...
  return (
    <div
      ...
      onClick={() =>
        navigate(toProjectPath(`/agents/${encodeURIComponent(agent.id)}`))
      }
      onKeyDown={(e) => {
        if (e.key === "Enter")
          navigate(toProjectPath(`/agents/${encodeURIComponent(agent.id)}`));
    ...
    >
      <div className="flex items-center gap-4">
        <div className="flex items-center justify-center size-10 rounded-lg bg-muted shrink-0">
          <BotIcon
            className={`size-5 ${isArchived ? "text-muted-foreground" : "text-blue-600"}`}
          />
        </div>
        ...
        <div className="flex min-w-0 flex-1 items-center gap-4">
          <div className="min-w-0 flex-1">
            ...
            <p
              data-testid={`agent-purpose-${agent.id}`}
      ...
      <div className="mt-3 grid grid-cols-1 gap-1.5 border-t border-border/60 pt-3 text-xs sm:grid-cols-3">
        ...
        {leadingGap && (
          <p
            data-testid={`agent-executability-guidance-${agent.id}`}
        ...
        <span
          data-testid={`agent-workload-${agent.id}`}
          className="text-muted-foreground"
        >
          {isArchived
  ...
  return (
    ...
    >
      ...
      <p className="text-xs text-muted-foreground mb-4 max-w-sm mx-auto">
        Start with the work you need done. Mohist will create an Agent for the
        task, or you can configure a profile first.
      </p>
