...
function mockAgents(agents: AgentInfo[]) {
  server.use(
    http.get(AGENTS_PATH, () =>
      HttpResponse.json({ success: true, data: agents }),
    ),
  );
}
  ...
  AgentProfileEditor: ({ agent, open }) =>
    open ? (
      <div
        ...
        data-mode={agent === null ? "create" : "edit"}
      />
    ) : null,
};
...
function makeAvailability(
  overrides: Partial<AgentAvailabilitySummaryEntry> = {},
): AgentAvailabilitySummaryEntry {
  return {
    agentId: "agent-1",
    canStartNow: true,
...
describe("AgentListPage", () => {
  ...
  describe("list rendering", () => {
    ...
    it("renders purpose and the server Executability state distinctly", async () => {
      ...
      expect(
        within(readyRow).getByTestId("agent-purpose-ready"),
      ).toHaveTextContent("Reviews pull requests");
      expect(
        within(readyRow).getByTestId("agent-executability-ready"),
      ).toHaveTextContent("Executability: executable");
      expect(
        within(screen.getByTestId("agent-row-setup")).getByTestId(
          "agent-executability-setup",
        ),
      ).toHaveTextContent("Executability: not-configured");
      expect(
        within(screen.getByTestId("agent-row-unknown")).getByTestId(
          "agent-executability-unknown",
        ),
      ).toHaveTextContent("Executability: unknown");
      expect(
        within(screen.getByTestId("agent-row-unknown")).getByTestId(
          "agent-purpose-unknown",
        ),
      ).toHaveTextContent("No purpose set");
    });
    ...
    it("renders server Availability and active/queued workload without changing executability", async () => {
      ...
      expect(
        within(row).getByTestId("agent-executability-offline"),
      ).toHaveTextContent("Executability: executable");
      expect(
        within(row).getByTestId("agent-availability-offline"),
      ).toHaveTextContent("Availability: Runner offline");
      expect(
        within(row).getByTestId("agent-availability-guidance-offline"),
      ).toHaveAttribute("data-feedback-kind", "runner-offline");
      expect(
        within(row).getByTestId("agent-availability-guidance-offline"),
      ).toHaveTextContent(/connect a runner/i);
      expect(
        within(row).getByTestId("agent-workload-offline"),
      ).toHaveTextContent("Active: 2, Queued: 3");
      expect(
        within(row).getByTestId("agent-executability-offline"),
      ).not.toHaveTextContent("not-configured");
    });
    ...
    it.each([
      ...
      "gives an actionable next step for %s Availability",
      async (waitingReason, nextAction) => {
        mockAgents([
          makeAgent({
            id: "waiting",
            executability: {
        ...
        const guidance = within(row).getByTestId(
          "agent-availability-guidance-waiting",
        );
    ...
    it("uses one list Availability request for multiple Agents", async () => {
      ...
      expect(screen.queryByTestId("agent-availability-a1")).toHaveTextContent(
        "Can start now",
      );
      expect(screen.queryByTestId("agent-availability-a2")).toHaveTextContent(
        "Can start now",
      );
    });
    ...
    it("shows loading Availability while the summary is unresolved", async () => {
      ...
      expect(
        within(row).getByTestId("agent-availability-pending"),
      ).toHaveTextContent("Availability: Loading");
      expect(
        within(row).getByTestId("agent-executability-pending"),
      ).toHaveTextContent("Executability: executable");
      expect(
        within(row).getByTestId("agent-executability-pending"),
      ).not.toHaveTextContent("not-configured");
  ...
  describe("create entry points", () => {
    it("does not render the editor before any entry point is clicked", async () => {
      ...
      expect(
        screen.queryByTestId("agent-profile-editor"),
    ...
    it('opens the profile editor in create mode when the header "New Agent" button is clicked (no route change)', async () => {
      ...
      expect(screen.getByTestId("agent-profile-editor")).toHaveAttribute(
        "data-mode",
        "create",
      );
  ...
  describe("Archived section (agent-archive spec)", () => {
    it('lists archived agents under an "Archived (n)" section whose count matches', async () => {
      ...
      expect(
        within(section).getByTestId("agent-row-a-archived"),
      ).toBeInTheDocument();
      expect(
        within(section).getByTestId("agent-row-b-archived"),
      ).toBeInTheDocument();
    });
