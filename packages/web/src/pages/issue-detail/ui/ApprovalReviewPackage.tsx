    ...
    <div data-testid="send-back-feedback-form" className="mt-3 min-w-0 rounded-md border border-border bg-muted p-3">
      <div className="space-y-2">
        ...
        <div className="text-xs text-muted-foreground">
          Submit with <kbd className="rounded border border-border bg-card px-1 py-0.5 font-mono">Command+Enter</kbd>
        </div>
      </div>
      <div className="mt-2 flex justify-end gap-2">
        ...
        <Button
          type="button"
          size="sm"
          data-testid="send-back-feedback-submit"
...
function findApprovalArtifact(
  ...
  const currentRunArtifacts =
    artifacts?.filter((item) => !workflowRunId || !item.workflowRunId || item.workflowRunId === workflowRunId) ?? []
  return currentRunArtifacts.find((item) => item.path === path) ?? null
}
...
function ArtifactEvidence({
  ...
  if (artifact.kind === 'directory')
    return <div className="text-xs text-warning">Unexpected directory artifact; expected a file.</div>
  ...
  if (!content.data)
    return <div className="text-xs text-muted-foreground">No recorded content for {artifact.path}.</div>
  if (content.data.kind === 'directory')
    return <div className="text-xs text-warning">Unexpected directory content; expected a file.</div>
...
function DiffSummary({ data, isLoading, error }: { data?: IssueDiffResponse; isLoading: boolean; error: unknown }) {
  if (isLoading)
    return (
      <div data-testid="approval-diff-summary" className="text-xs text-muted-foreground">
        Loading diff summary...
      </div>
    )
  if (error)
    return (
      <div data-testid="approval-diff-summary" className="text-xs text-danger">
        Failed to load diff summary.
      </div>
    )
  if (!data || data.available === false) {
    return (
      <div data-testid="approval-diff-summary" className="text-xs text-muted-foreground">
        Diff summary is unavailable: {data?.message ?? 'No comparison is available.'}
      </div>
    )
  }
  ...
  return (
    <section
      data-testid="approval-diff-summary"
      className="min-w-0 space-y-2 rounded-md border border-border bg-muted/50 p-3 text-sm"
    >
      ...
      <div className="flex min-w-0 flex-wrap gap-x-3 gap-y-1 text-xs text-muted-foreground">
        <span className="min-w-0 break-all">
          {data.head} compared with {data.base}
        </span>
...
function SecondaryActions({
  ...
  return (
    <div className="relative" data-testid="approval-more-actions">
      ...
      {open && (
        <div
          className="absolute bottom-full right-0 mb-2 min-w-52 rounded-md border border-border bg-popover p-1 shadow-lg"
          role="menu"
        >
          {actions.map((action) => {
...
export function ApprovalReviewPackage({
  ...
  const handleSendBackSubmit = () => {
    if (!sendBack || !sendBack.enabled || controller.pendingKind !== null || !draft.category || !draft.body.trim())
      return
  ...
  const evidence: ReactNode =
    approvalStage === 'plan' ? (
      ...
      <div
        id="artifacts"
        data-testid="approval-review-evidence"
        className="mt-4 scroll-mt-20 rounded-md border border-border bg-muted px-3 py-2 text-xs text-muted-foreground"
      >
  ...
  return (
    ...
    >
      {isNarrowViewport ? (
        <div className="min-w-0 rounded-lg border border-l-4 border-warning bg-card p-4 shadow-sm">
          ...
          <div className="mt-3 flex flex-wrap items-center gap-2 text-xs font-medium uppercase tracking-wide text-muted-foreground">
            ...
            <span className="normal-case font-normal text-card-foreground">{nextAction}</span>
          </div>
          {evidence}
        </div>
      ) : (
      ...
      {isNarrowViewport && (
        <div
          data-testid="approval-mobile-action-bar"
          className="fixed inset-x-0 bottom-0 z-50 px-3 pb-[calc(0.5rem+env(safe-area-inset-bottom))]"
        >
          <div className="mx-auto grid w-full max-w-md grid-cols-[1fr_1fr_auto] gap-2 rounded-xl border border-border bg-popover/95 p-2 shadow-lg backdrop-blur">
            <Button
              type="button"
              data-testid="approval-mobile-approve"
              aria-describedby={approveReason ? 'approval-mobile-approve-reason' : undefined}
              disabled={!approve?.enabled || controller.pendingKind !== null}
              onClick={() => approve && controller.runAction(approve)}
              className="min-h-11"
            >
            ...
            <Button
              type="button"
              variant="destructive"
              data-testid="approval-mobile-send-back"
              aria-describedby={sendBackReason ? 'approval-mobile-send-back-reason' : undefined}
              disabled={!sendBack?.enabled || controller.pendingKind !== null}
              onClick={handleSendBackOpen}
              className="min-h-11"
            >
              Send back
            </Button>
          ...
          {approveReason && (
            <p id="approval-mobile-approve-reason" className="sr-only">
              {approveReason}
            </p>
          )}
          {sendBackReason && (
            <p id="approval-mobile-send-back-reason" className="sr-only">
              {sendBackReason}
            </p>
          )}
          {sendBackForm}
