...
describe('FtrTrendChart', () => {
  ...
  it('renders empty state with next action when trend is missing', async () => {
    mockQualityResponse({
      window: makeWindow(20, 0.65),
  ...
  it('renders empty state when all trend points have zero samples (graceful older server)', async () => {
  ...
  it('treats an undefined trend as empty (graceful degradation against older server)', async () => {
    mockQualityResponse({
      window: makeWindow(20, 0.65),
    } as QualityMetricsResponse)
  ...
  it('renders resolved chart content with accessibility wrapper', async () => {
    mockQualityResponse(buildTrendData())
  ...
  it('renders one marker per non-null FTR bucket sourced from the per-bucket series', async () => {
    mockQualityResponse(buildTrendData())
  ...
  it('null-bucket produces no marker and gaps the line (no 0%/100% plotted)', async () => {
    mockQualityResponse(buildTrendData())
  ...
  it('FTR line uses stroke-chart-5 theme token', async () => {
    mockQualityResponse(buildTrendData())
  ...
  it('axis renders with fixed 0/25/50/75/100 percentage ticks', async () => {
    mockQualityResponse(buildTrendData())
  ...
  it('hides the legend when overlay is off (single series)', async () => {
    mockQualityResponse(buildTrendData())
  ...
  it('legend distinguishes FTR from rework by shape (line vs dashedLine)', async () => {
    mockQualityResponse(buildTrendData())

    renderChart()

    await waitFor(() => {
      expect(screen.getByLabelText('Toggle rework rate overlay')).toBeInTheDocument()
    })
  ...
  it('SR summary describes the window, first/last FTR, and peak rework day', async () => {
    mockQualityResponse(buildTrendData())
  ...
  it('SVG has role=img and aria-label naming the FTR trend', async () => {
    mockQualityResponse(buildTrendData())
  ...
  it('does not render a second LineSeries when overlay is off', async () => {
    mockQualityResponse(buildTrendData())
  ...
  it('toggling the overlay renders the rework LineSeries on the same axis', async () => {
    mockQualityResponse(buildTrendData())
  ...
  it('rework LineSeries uses stroke-chart-4, a dashed path, and per-bucket rates from the series', async () => {
    mockQualityResponse(buildTrendData())
  ...
  it('overlay toggle is disabled when the series carries no rework samples', async () => {
  ...
  it('overlay toggle persists across re-renders via local state', async () => {
    mockQualityResponse(buildTrendData())
  ...
  it('line markers apply transition (transform-based motion) by default', async () => {
    mockQualityResponse(buildTrendData())
  ...
  it('line markers disable animation when prefers-reduced-motion: reduce', async () => {
  ...
  it('renders day labels for the trailing buckets', async () => {
    mockQualityResponse(buildTrendData())
  ...
  it('renders a window badge derived from trend.from/trend.to when trend is present', async () => {
    mockQualityResponse(buildTrendData())
  ...
  it('hides the window badge in the empty state when trend is missing', async () => {
    mockQualityResponse({
      window: makeWindow(20, 0.65),
    } as QualityMetricsResponse)
