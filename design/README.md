...
- [agent-subscriptions.md](agent-subscriptions.md) — Agent 事件订阅（**WIP**）：Agent 监听 CloudEvent、按订阅响应提示词自动启动。归属 Agent 上下文，消费 PL；handler 只读信封、Agent 用 mo workflow show 自拉上下文；前置依赖 mo workflow 命令套件已由 issue #381 关闭。
