...
resource rather than a Project membership. Three kinds exist:

- `admin`: exactly one, representing the owner of the self-hosted deployment.
- `service`: one per built-in component, separating trusted local machines and adapters from the
  administrator.
- `agent`: one per Mohist Agent, preserving Agent attribution without giving the Agent a login
  credential.
...
or recovery path. Five kinds exist:
...
Scope limits which class of operation a credential can perform. Four Scopes
exist:
...
- `callerKeyId` is the Credential ID. It is stable across retries and never supplied or returned by
  the caller.
- `principalId` is the Principal used for attribution.
- `scopes` are the route capabilities granted to the PAT.
- `projectGrant` is either `explicit` or `operator_all`.
- `allowedProjectIds` is a non-empty Project set when the grant is `explicit`.
...
1. Compare the deployment root credentials in constant time.
2. Otherwise hash the token and resolve a stored Credential.
3. Reject expired or revoked credentials.
4. Resolve the Principal and Scope.
5. For a direct Agent PAT, also resolve the ExternalAgentCaller and its Project grant.
...
1. Bearer PAT authentication.
2. ExternalAgentCaller resolution.
3. Scope authorization.
4. Project grant authorization.
5. Canonical resource ownership check.
6. Request validation and normalization.
7. Idempotency lookup and fingerprint comparison.
8. Admission and durable effects.
...
Display aliases such as `--display-name` never become ownership evidence. Agent
