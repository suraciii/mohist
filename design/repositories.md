...
- 让 Repository `GitUrl` / `BaseBranch` update 经过现有 `IssueRepositoryCoordinator`；
- 复用删除路径的未完成 Issue blocker 查询；
- 覆盖 update 与并发 Issue create/reassign/reopen 的串行化，以及 default 切换不受影响。

这一步独立可部署，先建立后续删除 snapshot 所依赖的不变量。

### 2. 删除控制平面副本

- 从 `WorkflowRun`、`WorkflowStartSnapshot` 与 `IssueWorkStarted` 删除 Repository/workspace
  snapshot；
- 让 dispatch context resolver 通过 `ProjectId + IssueNumber` 读取 Issue target 与 Project
  Repository；
- 删除 `AuthoritativeRoutingOverlay` 中的 Repository/workspace 替换，只保留真正属于 run
  identity 的 context；
- workspace、rebase、diff 与 cleanup API 改为以 `WorkflowRunId` 寻址，base branch 在请求时
  从 Project Repository 解析。

### 3. 原子切换 Runner workspace 协议

- path 与 branch 直接由 `WorkflowRunId` 推导；
- marker 缩减为 `workflowRunId`；registry 缩减为 run lifecycle；
- 删除 workspace query 中的 Project、Issue、Repository、fingerprint、version、path 与 branch；
- 删除 `git-remote-identity.ts`，复用 workspace 内的精确 origin 检查；
- bootstrap 优先恢复远端同名 run branch；
- cleanup 只验证 runner-root containment 与 marker run ID。

Server 与 Runner 的 wire shape 在同一个提交集完成并一起部署，不保留双协议分支。

### 4. 删除旧测试形状，保留行为测试

删除只断言 fingerprint、marker 字段和 DTO shape 的测试。保留并强化以下行为：

- 未完成 Issue 阻止 Repository 执行属性变化；
- dispatch 始终使用 Issue 绑定的 Project Repository；
- Project default 变化不重定向已有 Issue；
- workspace 不能跨 run 复用；
- origin 或 branch 被修改时，任何 Git 写操作都在 mutation 前失败；
- registry 丢失后可以从 marker 重建，workspace 丢失后可以从远端 run branch 重建；
- cleanup 不删除 runner root 外或 marker 不匹配的目录。
